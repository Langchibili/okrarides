...
export default function OSMMapDisplay({
    ...
    controlsRef.current = {
      ...
      updateDriverLocation(lat, lng, heading) {
        ...
        if (marker) {
          ...
          if (heading != null) marker.getElement().style.transform += ` rotate(${heading}deg)`;
        }
        // Pan to keep driver in view when they drift off screen — no zoom change.
  ...
  useEffect(() => {
    ...
    // fitBounds on STATIC markers only (pickup / dropoff).
    // Driver marker is excluded — it moves every location tick via
    // updateDriverLocation which already handles keep-in-view via easeTo.
    // Including the driver here would re-trigger fitBounds every tick.
    const staticMarkers = all.filter((m) => m.type !== 'driver');
    if (staticMarkers.length >= 2) {
      const lngs = staticMarkers.map((m) => m.lng);
      const lats = staticMarkers.map((m) => m.lat);
      map.fitBounds(
        [[Math.min(...lngs), Math.min(...lats)], [Math.max(...lngs), Math.max(...lats)]],
        { padding: { top: 80, bottom: 220, left: 60, right: 60 }, duration: 800, maxZoom: 17 },
      );
    } else if (staticMarkers.length === 1) {
      map.flyTo({ center: [staticMarkers[0].lng, staticMarkers[0].lat], zoom: 16, duration: 800 });
    }
  }, [markers, pickupLocation, dropoffLocation]);
  ...
  useEffect(() => {
    ...
    // Zoom to fit the full route — same padding as IframeMap srcdoc.
    // bottom:220 leaves room for the bottom sheet, top:80 for the status bar.
