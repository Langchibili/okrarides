// // //Okrarides\delivery\components\Map\MapIframe.jsx
// // 'use client';
...
// //   return R * 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a));
// // }
// // PATH: Okra/Okrarides/delivery/components/Map/MapIframe.jsx
// 'use client';
...
// // ─────────────────────────────────────────────────────────────────────────────
// // NativeMapWrapper — unchanged from delivery original
// // ─────────────────────────────────────────────────────────────────────────────
// function NativeMapWrapper({
//   NativeComponent, prioritizedMap, center, zoom, markers,
//   pickupLocation, dropoffLocation, onRouteCalculated, onMapClick,
...
// // ─────────────────────────────────────────────────────────────────────────────
// // IframeMap
// // Changes from delivery original:
// //   1. mapsProvider added to read getRoute()
// //   2. Route useEffect: calls mapsProvider.getRoute() → sends SET_ROUTE_GEOMETRY
// //      instead of DRAW_ROUTE (iframe no longer fetches routes itself)
// //   3. generateIframeContent: DRAW_ROUTE replaced with SET_ROUTE_GEOMETRY handler
// //   All other delivery-specific logic (routeLoaded fitBounds fix, FIX1/FIX2
// //   UPDATE_MARKERS, maxZoom:17, driver marker, pan-to-keep-in-view) preserved.
// // ─────────────────────────────────────────────────────────────────────────────
// const IframeMap = memo(({
//   center          = { lat: -15.4167, lng: 28.2833 },
//   zoom            = 13,
...
//   // Added: read mapsProvider so route fetching respects prioritizedRoute
//   const mapsProvider = useSafeMapProvider();
...
//   // ── Marker sync — unchanged from delivery original ────────────────────────
//   useEffect(() => {
//     if (!iframeLoaded) return;
//     const markerData = [];
//     if (pickupLocation)  markerData.push({ id: 'pickup',  position: pickupLocation,  type: 'pickup',  title: 'Pickup'  });
...
//   // prioritizedRoute. The iframe only renders the geometry it receives.
//   const pickupLat  = pickupLocation?.lat;
//   const pickupLng  = pickupLocation?.lng;
//   const dropoffLat = dropoffLocation?.lat;
//   const dropoffLng = dropoffLocation?.lng;
...
//   // generateIframeContent kept inline as per delivery original pattern.
//   // Only change: DRAW_ROUTE handler replaced with SET_ROUTE_GEOMETRY.
//   // All other delivery-specific fixes (FIX1 routeLoaded guard, FIX2 maxZoom:17,
//   // driver marker, pan-to-keep-in-view) are preserved exactly.
//   const generateIframeContent = () => `<!DOCTYPE html>
// <html>
// <head>
//   <meta charset="utf-8">
...
//   // FIX 2 (preserved): maxZoom:17 prevents over-zooming on short routes.
//   function renderRouteCoords(coords, distanceKm, durationMin) {
//     if (!coords || coords.length < 2) return;
//     const geojson = { type: 'Feature', geometry: { type: 'LineString', coordinates: coords } };
//     if (map.getSource('route')) {
...
//     switch (type) {
//       // FIX 1 (preserved): fitBounds only when !routeLoaded
//       case 'UPDATE_MARKERS': {
//         Object.entries(markers).forEach(([id, m]) => {
//           if (id !== 'current-location') { m.remove(); delete markers[id]; }
//         });
...
//       // ── SET_ROUTE_GEOMETRY ──────────────────────────────────────────────────
//       // Replaces DRAW_ROUTE. React fetches the route via MapsProvider.getRoute()
//       // (respects prioritizedRoute) and sends pre-computed geometry here.
//       // The iframe never fetches routes itself.
...
// // ─────────────────────────────────────────────────────────────────────────────
// // HELPERS — unchanged from delivery original
// // ─────────────────────────────────────────────────────────────────────────────
...
// ── FIX: generateIframeContent moved OUTSIDE the component ───────────────────
// Previously defined as a closure inside IframeMap and called inline as
// srcDoc={generateIframeContent()}, which regenerated the full HTML string on
// every render and caused the iframe to fully reload (blank map flash, lost
// state) on any state change in the parent. Now it's a pure function at module
// scope, memoized once with useMemo([]) inside IframeMap.
//
// Changes vs. delivery original:
//   • createMarkerEl now accepts a `custom` config object (same shape as driver
//     app) so PACKAGE_MARKER / TRUCK_MARKER are actually applied
//   • CONFIGURE_DRIVER_MARKER handler added — lets React set the truck icon once
//     after map load, same pattern as driver/ride MapIframe
//   • UPDATE_MARKERS passes md.custom through to createMarkerEl
//   • SET_ROUTE_GEOMETRY replaces DRAW_ROUTE (React fetches route, iframe renders)
//   • All delivery-specific fixes preserved: FIX1 routeLoaded fitBounds guard,
//     FIX2 maxZoom:17, pan-to-keep-driver-in-view, UPDATE_DRIVER_LOCATION
...
(function() {
  ...
  // ── FIX: createMarkerEl now accepts an optional custom config object ────────
  // Shape: { bg: string, label: string, size: number, isPulse?: boolean }
  // If custom is provided it overrides the type preset entirely — this is how
  // PACKAGE_MARKER and TRUCK_MARKER from active-delivery actually get applied.
  ...
  window.addEventListener('message', async (event) => {
    ...
    switch (type) {
      ...
      // ── FIX: UPDATE_MARKERS passes md.custom to createMarkerEl ───────────
      // FIX 1 (preserved): fitBounds only when !routeLoaded
      case 'UPDATE_MARKERS': {
        Object.entries(markers).forEach(([id, m]) => {
          if (id !== 'current-location') { m.remove(); delete markers[id]; }
      ...
      // ── FIX: CONFIGURE_DRIVER_MARKER added ───────────────────────────────
      // active-delivery calls c.configureDriverMarker(TRUCK_MARKER) in onMapLoad.
      // Without this handler the truck icon was silently ignored and the driver
      // marker fell back to the default car emoji.
      ...
      case 'UPDATE_DRIVER_LOCATION': {
        ...
        if (driverMarker) {
          ...
          // Use driverMarkerConfig if set (truck icon), otherwise default
          const el = createMarkerEl('driver', driverMarkerConfig);
          driverMarker = new maplibregl.Marker({ element: el, rotationAlignment: 'map' })
            .setLngLat(lngLat).addTo(map);
        }
        ...
        // Preserved: pan map to keep driver in view
        if (map && lngLat[0] != null && lngLat[1] != null) {
          if (!map.getBounds().contains(lngLat)) {
            map.easeTo({ center: lngLat, duration: 600 });
          }
      ...
      // ── SET_ROUTE_GEOMETRY — React fetches route, iframe only renders ─────
      case 'SET_ROUTE_GEOMETRY': {
        const coords = data.geometry;
        if (!coords || coords.length < 2) break;
        const run = () => renderRouteCoords(
...
const IframeMap = memo(({
  ...
  // ── FIX: memoized once — iframe HTML never regenerates on re-render ───────
  ...
  // Previously depended on iframeLoaded state, which made sendToIframe
  // change on every render after load, cascading into createMapControls
  // and the onMapLoad effect re-firing repeatedly.
  ...
  }, [mapId]); // mapId is stable — no iframeLoaded dep
  ...
  // ── Marker sync ───────────────────────────────────────────────────────────
  // ── FIX: overriddenIds guard — prevents ghost marker trail ────────────────
  // When accepted, active-delivery passes pickupLocation = driverLoc (moves
  // every 2s) AND markers has { id: 'pickup', position: actualPickup }.
  // Without the guard, both get added to the iframe; only the second is
  // tracked by id; the first orphans at each old driver position = ghost trail.
  // If markers already claims an id, skip the prop-based entry for that id.
  useEffect(() => {
    if (!iframeLoaded) return;
  ...
  // ── FIX: callbacks accessed via refs — removed from effect deps ───────────
  const onRouteCalculatedRef = useRef(onRouteCalculated);
  useEffect(() => { onRouteCalculatedRef.current = onRouteCalculated; }, [onRouteCalculated]);

  const getRoute = mapsProvider?.getRoute; // extract stable fn ref, not whole object
  ...
  // getRoute is a stable extracted fn ref; onRouteCalculated via ref — not a dep
  }, [pickupLat, pickupLng, dropoffLat, dropoffLng, iframeLoaded, sendToIframe, getRoute]);

  // ── Message listener ──────────────────────────────────────────────────────
  // ── FIX: callbacks via refs — listener only registers once on mount ───────
  ...
  useEffect(() => {
    const handleMessage = (event) => {
      ...
      switch (type) {
        case 'MAP_LOADED':       setIframeLoaded(true); setIsLoading(false); break;
        case 'ROUTE_CALCULATED': onRouteCalculatedRef.current?.(data); break;
        case 'MAP_CLICKED':      onMapClickRef.current?.(data.location); break;
        case 'MAP_ERROR':        console.error('Map iframe error:', data.error); setError(data.error); setIsLoading(false); break;
      }
    };
    window.addEventListener('message', handleMessage);
    return () => window.removeEventListener('message', handleMessage);
  }, [mapId]); // stable — no callback deps needed
