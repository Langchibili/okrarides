...
export default function AddDeliveryVehiclePage() {
  ...
  const handleUpdateVehicle = async () => {
    ...
    try {
      ...
      // Step 3 — reflect the new values locally
      setExistingVehicle((prev) => ({ ...prev, ...payload }));
  ...
  const handleSubmit = async (e) => {
    e.preventDefault(); setError(null); setSuccess(null);
    if (!hasBasicInfoSaved) { setError('Please save vehicle information first'); return; }
    if (!areAllDocumentsUploaded()) { setError('Please upload all required documents'); return; }
    try {
      setLoading(true);
      const res = await submitForVerification(); // → /delivery-driver/onboarding/submit
      if (res.success) {
          ...
          <Box sx={{ p: 3 }}>
            ...
            <Box sx={{ opacity: !vehicleId ? 0.5 : 1, pointerEvents: !vehicleId ? 'none' : 'auto' }}>
              ...
              {isInsuranceRequired && (
                <Box sx={{ mb: 3 }}>
                  <DocumentUploadCard
                    title="Insurance Disk"
                    description="Upload your valid insurance disk"
                    onUpload={(f) => handleDocumentUpload(f, 'insuranceCertificate')}
                    uploadedFile={uploadedDocs.insuranceCertificate}
                    onRemove={() => setUploadedDocs(p => ({ ...p, insuranceCertificate: null }))}
                  />
                </Box>
              )}
            </Box>
            ...
            {(isEditMode || hasBasicInfoSaved) && (
              <>
                ...
                {vehicleId && !areAllDocumentsUploaded() && <Typography variant="caption" color="text.secondary" sx={{ display: 'block', textAlign: 'center', mt: 2 }}>Please upload all required documents before submitting</Typography>}
              </>
            )}
          </Box>
        </Box>
