...
export default function OkraPayModal({
  ...
  // Only these two functions change inside OkraPayModal:
  ...
  const submitMobileMoney = async () => {
    ...
    try {
      ...
      // For floatadd with no pre-created intent, create one inline before calling OkraPay.
      // The backend's /float-topups endpoint sets driver or partner based on the auth token.
      let effectiveRelatedEntityId = relatedEntityId;
      if (purpose === 'floatadd' && !effectiveRelatedEntityId) {
        const intent = await apiClient.post('/float-topups', { data: { amount, partner: user?.id, driver: null } });
        effectiveRelatedEntityId = intent?.data?.id ?? intent?.id;
        if (!effectiveRelatedEntityId) throw new Error('Could not create top-up record. Please try again.');
      }
      ...
      const res = await apiClient.post('/okrapay/initiate', {
        ...
        amount,
        currency,
        relatedEntityId: effectiveRelatedEntityId,
  ...
  const submitCard = async () => {
    ...
    try {
      ...
      // Same inline intent creation for card payments
      let effectiveRelatedEntityId = relatedEntityId;
      if (purpose === 'floatadd' && !effectiveRelatedEntityId) {
        const intent = await apiClient.post('/float-topups', { data: { amount, partner: user?.id, driver: null } });
        effectiveRelatedEntityId = intent?.data?.id ?? intent?.id;
