...
import { getFloatColor, formatDateTime, savedCurrencyCode, savedPhoneCode } from '@/lib/utils/format';
...
export default function FloatPage() {
  const { user } = useAuth(); // partner's logged-in user — user.id is the partner user ID
  const {
  ...
  const [floatQuickAmount, setFloatQuickAmount] = useState(null); // ← new
  ...
  // ── Create float-topup record, then open OkraPayModal ─────────────────────
  const handleBuyFloat = async () => {
    ...
    setInitiating(true);
    try {
      // Explicitly link to the partner user and clear driver so the backend
      // routes this through handleFloatTopupSuccess → partner credit path.
      const res = await apiClient.post('/float-topups', {
        data: {
          amount: numBuyAmount,
          partner: user?.id,   // partner's user ID
          driver: null,
          topupId: `FT-${Date.now()}`,
      ...
      if (!id) throw new Error('Could not create top-up record. Please try again.');

      setTopupId(id);
      setModalOpen(true);
    } catch (e) {
      setBuyError(e.message || 'Failed to start top-up. Please try again.');
    } finally {
      setInitiating(false);
    }
  };
  ...
  return (
    <Box sx={{ p: { xs: 2, md: 3 }, minHeight: '100vh' }}>
        ...
        <Box>
          <Paper sx={{ p: 3, borderRadius: 3, mb: 3 }}>
            ...
            <Button
              ...
              disabled={numBuyAmount <= 0 || initiating || !user?.id}
                ...
                return (
                  <Box key={entry.id} sx={{ display: 'flex', alignItems: 'center', gap: 2, py: 1.5, borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                    ...
                    <Typography sx={{
                      ...
                      color: isCredit ? '#10B981' : '#EF4444', fontSize: '0.85rem', flexShrink: 0,
                    }}>
            ...
            {driversLoading ? (
              ...
              drivers.map((driver) => (
                <Paper key={driver.id} sx={{
                  ...
                  {/* Quick amount chips — now open FloatModal pre-filled instead of sending directly */}
                  <Box sx={{ display: 'flex', gap: 0.75, flexShrink: 0 }}>
                    {[50, 100, 200].map((v) => (
                      <Chip
                        ...
                        variant="outlined"
                        onClick={() => {
                          setFloatQuickAmount(v);
                          setFloatModal(driver);
                        }}
                  ...
                  <Button
                    ...
                    variant="outlined"
                    onClick={() => {
                      setFloatQuickAmount(null); // no pre-fill for custom
                      setFloatModal(driver);
                    }}
      ...
      {floatModal && (
        <FloatModal
          open={!!floatModal}
          onClose={() => { setFloatModal(null); setFloatQuickAmount(null); }}
          driver={floatModal}
          partnerFloatBalance={displayBalance}
          defaultAction="CREDIT"
          defaultAmount={floatQuickAmount}   // ← pre-fills the amount field
          currency={currency}
          onSuccess={() => {
            setFloatModal(null);
            setFloatQuickAmount(null);
            refreshDashboard();
            loadDrivers();
      ...
      {/* OkraPay modal — relatedEntityId is the float-topup record created above */}
      <OkraPayModal
        open={modalOpen}
        onClose={() => { setModalOpen(false); setTopupId(null); }}
        amount={numBuyAmount}
        purpose="floatadd"
        relatedEntityId={topupId}
        currency={currencyCode ?? savedCurrencyCode()}
        phoneCode={phoneCode ?? savedPhoneCode()}
