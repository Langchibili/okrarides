  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0, bgcolor: bgPage, width: '100%', boxSizing: 'border-box', overflow: 'hidden' }}>
      ...
      <Box sx={{
        ...
        p: 2, pb: 1, boxSizing: 'border-box', bgcolor: bgPage
      }}>
        {/* Delivery mode */}
                  ...
                  <Paper onClick={() => setSelectedClass(est)} elevation={0} sx={{
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                      ...
                      <Box sx={{ textAlign: 'right', flexShrink: 0 }}>
                        <Typography variant="h6" fontWeight={800} sx={{ color: AMBER, fontSize: '1rem' }}>{savedCurrencySymbol()}{(est.totalFare ?? 0).toFixed(2)}</Typography>
            ...
            <Box sx={{
              ...
              <Box sx={{ mb: 1.5 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.4 }}>
                  ...
                  <Typography variant="body2" fontWeight={600} color="text.primary">{savedCurrencySymbol()}{(selectedClass.baseFare ?? 0).toFixed(2)}</Typography>
                </Box>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.4 }}>
                  <Typography variant="body2" color="text.secondary">Distance & time</Typography>
                  <Typography variant="body2" fontWeight={600} color="text.primary">{savedCurrencySymbol()}{((selectedClass.distanceFare ?? 0) + (selectedClass.timeFare ?? 0)).toFixed(2)}</Typography>
                </Box>
                {selectedClass.fragileSurcharge > 0 && (
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.4 }}>
                    <Typography variant="body2" color="text.secondary">Fragile handling</Typography>
                    <Typography variant="body2" fontWeight={600} color="text.primary">{savedCurrencySymbol()}{(selectedClass.fragileSurcharge ?? 0).toFixed(2)}</Typography>
                  </Box>
                )}
                <Divider sx={{ my: 0.75 }} />
                <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                  <Typography variant="h6" fontWeight={800} color="text.primary">Total</Typography>
                  <Typography variant="h6" fontWeight={800} sx={{ color: AMBER }}>{savedCurrencySymbol()}{(selectedClass.totalFare ?? 0).toFixed(2)}</Typography>
                </Box>
              </Box>
              ...
              <Button fullWidth variant="contained" size="large" onClick={handleConfirm} disabled={!canConfirm} sx={{
                ...
                {loading ? <CircularProgress size={22} color="inherit" /> : `Confirm Delivery · ${savedCurrencySymbol()}${(selectedClass.totalFare ?? 0).toFixed(2)}`}
              </Button>
