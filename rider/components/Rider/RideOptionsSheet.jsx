...
export function RideOptionsSheet({
  ...
  return (
    ...
    >
      ...
      {/* ② SCROLL AREA — height:0 + flex:1 + maxHeight fallback for when
          SwipeableBottomSheet doesn't propagate a definite height to children */}
      <Box
        sx={{
          flex: 1,
          height: 0,
          minHeight: 0,
          maxHeight: 'calc(80svh - 140px)',
          overflowY: 'auto',
          overflowX: 'hidden',
          scrollbarWidth: 'none',
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, width: '100%', minWidth: 0 }}>
                      ...
                      <Box sx={{ textAlign: 'right', flexShrink: 0 }}>
                        <Typography variant="h6" fontWeight={700} color="primary.main" sx={{ fontSize: '1rem' }}>
                          {savedCurrencySymbol()}{(estimate.subtotal || 0).toFixed(2)}
                        </Typography>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', minWidth: 0 }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, minWidth: 0 }}>
                    ...
                    <Box sx={{ minWidth: 0 }}>
                      ...
                      <Typography variant="caption" color="text.secondary">{promoDiscount?.message || `Save ${savedCurrencySymbol()}${promoDiscount?.discount?.toFixed(2)}`}</Typography>
                    </Box>
                  </Box>
              ...
              {[{ value: 'cash', label: 'Cash', Icon: WalletIcon }, { value: 'okrapay', label: 'OkraPay', Icon: CardIcon }].map(({ value, label, Icon }) => (
                <Paper
                  ...
                  sx={{
                    ...
                    width: '100%', boxSizing: 'border-box', '&:focus': { outline: 'none' },
                  }}
                >
                  <FormControlLabel
            ...
            >
              <Box sx={{ mb: 1.5 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.4 }}>
                  ...
                  <Typography variant="body2" fontWeight={600}>{savedCurrencySymbol()}{(selectedRideClass.subtotal || 0).toFixed(2)}</Typography>
                </Box>
                {promoApplied && promoDiscount && (
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.4 }}>
                    <Typography variant="body2" color="success.main">Promo Discount</Typography>
                    <Typography variant="body2" fontWeight={600} color="success.main">-{savedCurrencySymbol()}{(promoDiscount.discount || 0).toFixed(2)}</Typography>
                  </Box>
                )}
                <Divider sx={{ my: 0.75 }} />
                <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                  <Typography variant="h6" fontWeight={800}>Total</Typography>
                  <Typography variant="h6" fontWeight={800} color="primary.main">{savedCurrencySymbol()}{calculateTotalFare().toFixed(2)}</Typography>
                </Box>
              </Box>
              ...
              >
                {loading ? <CircularProgress size={22} color="inherit" /> : `Confirm Ride · ${savedCurrencySymbol()}${calculateTotalFare().toFixed(2)}`}
              </Button>
            </Box>
          </motion.div>
        )}
