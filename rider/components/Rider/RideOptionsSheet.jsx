...
export function RideOptionsSheet({
  ...
  onConfirmRide,
  loading,
  isSelectingLocation = false, // New prop to indicate location selection mode
}) {
  // Local state
  const [paymentMethod, setPaymentMethod] = useState('cash');
  const [passengerCount, setPassengerCount] = useState(1);
  ...
  const [sheetExpandedHeight, setSheetExpandedHeight] = useState(null);

  // Validate locations
  const hasLocations = pickupLocation && dropoffLocation;

  // Effect to handle sheet expansion when selecting locations
  useEffect(() => {
    if (isSelectingLocation) {
      setSheetExpandedHeight('80%');
    } else {
      setSheetExpandedHeight(null);
    }
  }, [isSelectingLocation]);

  // Effect to show floating button when both locations are set
  useEffect(() => {
    if (hasLocations && !selectedRideClass) {
      setButtonFloating(true);
    } else {
      setButtonFloating(false);
    }
  }, [hasLocations, selectedRideClass]);
  ...
  // Handle swipe down - make button fade back in (reduce z-index)
        ...
        {hasLocations && !selectedRideClass && (
          ...
          >
            <Button
              ...
              size="large"
              onClick={() => {
                // Scroll to ride options or trigger ride options view
                setButtonFloating(false);
              }}
