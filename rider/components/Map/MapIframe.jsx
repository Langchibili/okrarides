...
// ── Route-aware check (mirrors MapsProvider) ──────────────────────────────────
...
// This is intentionally outside any component/hook. The string is generated
// once per set of stable initialisation values and never again.
...
(function() {
  ...
  async function nominatimSearch(query) {
    try {
      ...
      results.sort((a, b) => (/\\d/.test(a.main_text) ? 0 : 1) - (/\\d/.test(b.main_text) ? 0 : 1));
      return results;
    } catch { return []; }
  }
  ...
  async function initMap() {
    ...
    const initialCenter = savedGeo ? [savedGeo.lng, savedGeo.lat] : await getIPLocation();
    ...
    map.on('load', async () => {
      ...
      if (geoOnLoad) placeCurrentLocationDot([geoOnLoad.lng, geoOnLoad.lat]);
  ...
  window.addEventListener('message', async (event) => {
    ...
    switch (type) {
      ...
      case 'UPDATE_CURRENT_LOCATION': {
        ...
        if (!mapReady) { setTimeout(() => placeCurrentLocationDot(lngLat), 600); break; }
      ...
      case 'SET_ROUTE_GEOMETRY': {
        if (!mapReady) {
          setTimeout(() => renderRouteGeometry(data.geometry, data.distance, data.duration, data.distanceValue, data.durationValue), 500);
          break;
        }
        renderRouteGeometry(data.geometry, data.distance, data.duration, data.distanceValue, data.durationValue);
        break;
      }
          ...
          if (r.ok) {
            ...
            if (d?.length > 0) {
              ...
              sendMessage('PLACE_DETAILS', {
                ...
                location: { lat: parseFloat(item.lat), lng: parseFloat(item.lon), address: item.display_name, name: item.display_name?.split(',')[0], place_id: data.placeId },
              });
              break;
            }
          }
...
function NativeMapWrapper({
  ...
  // Use primitive deps to avoid re-running when object identity changes
  ...
  useEffect(() => {
    ...
    async function fetchRoute() {
      if (!mapsProvider?.getRoute) return;
      const result = await mapsProvider.getRoute(
  ...
  }, [pickupLat, pickupLng, dropoffLat, dropoffLng]); // eslint-disable-line
...
const IframeMap = memo(({
  ...
  // Stable IDs and config — never changes after mount
  ...
  // ── FIX: Only read getRoute from the provider; do NOT subscribe to the
  //    full provider object. This means provider re-renders (e.g. from
  //    logRequest's setProviders) do NOT re-render IframeMap.
  const mapsProvider = useSafeMapProvider();
  const getRouteRef = useRef(mapsProvider?.getRoute);
  useEffect(() => { getRouteRef.current = mapsProvider?.getRoute; }, [mapsProvider?.getRoute]);

  // ── srcDoc is memoised on mount-time values only. The iframe NEVER
  //    reloads because center/zoom are only used for the initial view.
  const srcDoc = useMemo(
    () => generateIframeContent({ mapId, center, zoom, localMapServerUrl }),
    [], // eslint-disable-line react-hooks/exhaustive-deps
  );

  // ── iframeLoaded ref so sendToIframe is stable ────────────────────────
  ...
  // ── Map controls factory — stable reference ───────────────────────────
  ...
  // ── onMapLoad fires exactly once after the iframe is ready ────────────
  ...
  // ── Push saved geo-location dot once after iframe ready ───────────────
  useEffect(() => {
    if (!iframeLoaded) return;
    try {
      if (typeof window !== 'undefined' && localStorage.getItem('geo_loc_status') === 'success') {
        const geo = JSON.parse(localStorage.getItem('geo_loc_data') || 'null');
  ...
  // ── Route calculation ─────────────────────────────────────────────────
  // Uses primitive lat/lng deps + getRouteRef so this never fires because
  // the provider object changed reference.
  const pickupLat = pickupLocation?.lat;
  const pickupLng = pickupLocation?.lng;
  const dropoffLat = dropoffLocation?.lat;
  const dropoffLng = dropoffLocation?.lng;
  ...
  useEffect(() => {
    ...
    async function fetchAndSendRoute() {
      // Try MapsProvider first (via ref — never creates a new effect)
      try {
        if (getRouteRef.current) {
          const result = await getRouteRef.current(origin, destination);
          if (cancelled) return;
          if (result?.type === 'deeplink') {
          ...
          if (result?.geometry?.length) {
            ...
            onRouteCalculatedRef.current?.({
              ...
              distanceValue: result.distanceValue ?? 0, durationValue: result.durationValue ?? 0,
            });
            return;
          }
        }
      } catch (err) {
        console.warn('[IframeMap] getRoute failed:', err?.message);
      }
      ...
      try {
        ...
        if (res.ok && !cancelled) {
          ...
          if (route) {
            ...
            onRouteCalculatedRef.current?.({
              distance: `${km.toFixed(1)} km`, duration: `${Math.round(min)} min`,
              distanceValue: route.distance, durationValue: route.duration,
            });
            return;
          }
        }
      ...
      onRouteCalculatedRef.current?.({
        distance: `${km.toFixed(1)} km`, duration: 'N/A',
        distanceValue: Math.round(km * 1000), durationValue: 0,
      });
    }
  ...
  // Note: getRouteRef and onRouteCalculatedRef are intentionally omitted —
  // they are refs, not reactive values.
    ...
    const handleMessage = (event) => {
      ...
      switch (type) {
        ...
        case 'MAP_CLICKED':
          ...
          break;
        case 'MAP_ERROR':
          console.error('[IframeMap] iframe error:', data.error);
          setError(data.error);
          setIsLoading(false);
          break;
      }
