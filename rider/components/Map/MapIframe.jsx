...
//   // ── FIX: placeCurrentLocationDot ─────────────────────────────────────────
//   // Extracted helper so both browser geolocation AND the UPDATE_CURRENT_LOCATION
//   // message can place/replace the pulsing blue dot in a single consistent way.
...
//     map.on('load', async () => {
//       mapReady = true;
//       // Browser geolocation — best-effort only; in React Native WebView this may
//       // never fire. The app-level position is pushed via UPDATE_CURRENT_LOCATION.
//       if (navigator.geolocation) {
//         navigator.geolocation.getCurrentPosition((pos) => {
//           const loc = [pos.coords.longitude, pos.coords.latitude];
//           map.flyTo({ center: loc, zoom: 15, duration: 1200 });
//           // Only place browser dot if no app-provided dot exists yet
//           if (!markers['current-location']) {
...
//       // ── FIX: UPDATE_CURRENT_LOCATION ──────────────────────────────────────
//       // Replaces the pulsing blue "you are here" dot with the app-provided GPS
//       // coordinates. This is the authoritative current-location update — it takes
//       // priority over the browser geolocation dot placed by initMap, which often
//       // fails to fire in React Native WebView environments.
...
//   // ── FIX: current-location pin ─────────────────────────────────────────────
//   // Root cause of the missing pin:
//   //   The iframe places the pulsing blue "you are here" dot via
//   //   navigator.geolocation.getCurrentPosition inside initMap(). In React Native
//   //   WebView this API is unreliable — it often never fires, fires very late, or
//   //   returns stale/inaccurate coordinates. There was no path for the React layer
//   //   (which has the authoritative native GPS position in pickupLocation) to push
//   //   that position into the iframe's current-location dot.
//   //
//   // Fix:
//   //   Whenever pickupLocation is the rider's actual current location
//   //   (isCurrentLocation === true), send UPDATE_CURRENT_LOCATION to the iframe.
//   //   The iframe handler (added above) properly removes the old dot — whether it
//   //   was placed by browser geolocation or a previous UPDATE_CURRENT_LOCATION —
//   //   and places a fresh one at the correct coordinates.
//   //
//   //   Uses primitive lat/lng deps (not the whole object) so it only re-fires
//   //   when the position actually changes, not on every object reference change.
//   useEffect(() => {
//     if (!iframeLoaded) return;
