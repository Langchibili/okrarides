...
//   // Single helper used by initMap (geo_loc_data), browser geolocation, AND
//   // the UPDATE_CURRENT_LOCATION message so they all behave identically.
//   function placeCurrentLocationDot(lngLat) {
//     if (markers['current-location']) {
//       markers['current-location'].remove();
//       delete markers['current-location'];
//     }
//     const el = createMarkerEl('current');
//     markers['current-location'] = new maplibregl.Marker({ element: el })
//       .setLngLat(lngLat)
//       .addTo(map);
//   }

//   // ── readSavedGeoLoc — read geo_loc_data from localStorage ────────────────
//   // The srcdoc iframe shares the parent page's origin, so localStorage is
//   // fully accessible here.
//   function readSavedGeoLoc() {
//     try {
...
//     // First try saved geolocation (fastest, no network request needed)
//     const saved = readSavedGeoLoc();
...
//     try {
//       // Build viewbox from saved geo for biased results
//       let extraParams = '';
//       const geo = readSavedGeoLoc();
//       if (geo) {
//         const delta = 0.5;
//         extraParams =
//           '&viewbox=' + (geo.lng - delta) + ',' + (geo.lat + delta) + ',' +
//           (geo.lng + delta) + ',' + (geo.lat - delta) + '&bounded=0';
//       }

//       const r = await fetch(
//         'https://nominatim.openstreetmap.org/search?q=' + encodeURIComponent(query) +
//         '&format=json&limit=10&addressdetails=1' + extraParams,
...
//       // Float number-coded entries to the top
//       results.sort((a, b) => {
//         const aNum = /\\d/.test(a.main_text) ? 0 : 1;
//         const bNum = /\\d/.test(b.main_text) ? 0 : 1;
//         return aNum - bNum;
//       });
...
//     // ── 1. Prefer saved IP-geolocation over a live IP lookup ─────────────
//     let initialCenter = null;
//     const savedGeo = readSavedGeoLoc();
//     if (savedGeo) {
//       initialCenter = [savedGeo.lng, savedGeo.lat];
//     } else {
//       initialCenter = await getIPLocation();
//     }
...
//       // ── Place dot from saved geolocation immediately (fast, no permission) ──
//       let geoLocDotPlaced = false;
//       const geoOnLoad = readSavedGeoLoc();
//       if (geoOnLoad) {
//         placeCurrentLocationDot([geoOnLoad.lng, geoOnLoad.lat]);
//         geoLocDotPlaced = true;
//       }

//       // ── Browser geolocation for higher accuracy (may not fire in WebView) ──
//       if (navigator.geolocation) {
//         navigator.geolocation.getCurrentPosition((pos) => {
//           const loc = [pos.coords.longitude, pos.coords.latitude];
//           map.flyTo({ center: loc, zoom: 15, duration: 1200 });
//           // Always update — browser GPS is more accurate than IP geolocation
//           placeCurrentLocationDot(loc);
//         }, () => {});
//       }
...
//       // Replaces the pulsing blue dot with the app-provided GPS coordinates.
//       // Takes priority over the browser geolocation dot placed by initMap,
//       // which often fails to fire in React Native WebView environments.
...
//         ];
//         if (lngLat[0] == null || lngLat[1] == null) break;
//         if (!mapReady) {
//           // Map not ready yet — defer until it is
//           setTimeout(() => placeCurrentLocationDot(lngLat), 600);
//           break;
//         }
//         placeCurrentLocationDot(lngLat);
//         break;
//       }
...
//             placeCurrentLocationDot([loc.lng, loc.lat]);
//           },
...
//   // ── Memoize HTML once — iframe never reloads on re-render ────────────────
//   const srcDoc = useMemo(
//     () => generateIframeContent({ mapId, center, zoom, localMapServerUrl }),
//     [] // eslint-disable-line react-hooks/exhaustive-deps
//   );

//   // ── sendToIframe reads iframeLoaded from a ref — stable reference ────────
...
//   // ── onMapLoad via ref — effect fires exactly once on load ─────────────────
//   const onMapLoadRef = useRef(onMapLoad);
//   useEffect(() => { onMapLoadRef.current = onMapLoad; }, [onMapLoad]);

//   useEffect(() => {
//     if (iframeLoaded) onMapLoadRef.current?.(createMapControls());
//   }, [iframeLoaded, createMapControls]);

//   // ── Marker sync with ghost-marker guard ───────────────────────────────────
//   useEffect(() => {
...
//   // ── Push saved IP-geolocation as the initial current-location dot ─────────
//   // Fires once after the iframe is ready. Ensures the blue "you are here" dot
//   // appears at the user's approximate location without requiring any browser
//   // permission prompt. This is a React-layer backup — the iframe also reads
//   // geo_loc_data directly, but this guarantees it even in restrictive WebViews.
//   useEffect(() => {
//     if (!iframeLoaded) return;
//     try {
//       if (typeof window !== 'undefined' &&
//         localStorage.getItem('geo_loc_status') === 'success') {
//         const geo = JSON.parse(localStorage.getItem('geo_loc_data') || 'null');
//         if (geo?.lat != null && geo?.lng != null) {
//           sendToIframe('UPDATE_CURRENT_LOCATION', { location: { lat: geo.lat, lng: geo.lng } });
//         }
//       }
//     } catch { }
//   }, [iframeLoaded, sendToIframe]);

//   // ── Live GPS current-location pin ─────────────────────────────────────────
//   // When pickupLocation carries isCurrentLocation === true (the rider's live
//   // GPS position), push it into the iframe so the pulsing dot tracks movement.
//   // Uses primitive lat/lng deps so it only fires when the position changes.
//   useEffect(() => {
//     if (!iframeLoaded) return;
//     if (!pickupLocation?.isCurrentLocation) return;
//     const lat = pickupLocation.lat ?? pickupLocation.latitude;
//     const lng = pickupLocation.lng ?? pickupLocation.longitude;
//     if (lat == null || lng == null) return;
//     sendToIframe('UPDATE_CURRENT_LOCATION', { location: { lat, lng } });
//   }, [
//     pickupLocation?.lat,
//     pickupLocation?.lng,
...
function NativeMapWrapper({
  ...
  // FIX: Stable ref for getRoute so the effect below doesn't re-fire when
  // MapsProvider re-renders and produces a new getRoute function reference.
  const getRouteRef = useRef(mapsProvider?.getRoute);
  useEffect(() => { getRouteRef.current = mapsProvider?.getRoute; }, [mapsProvider?.getRoute]);
  ...
  useEffect(() => {
    ...
    async function fetchRoute() {
      if (getRouteRef.current) {
        const result = await getRouteRef.current(pickupLocation, dropoffLocation);
        if (cancelled) return;
        if (result?.type === 'deeplink') {
          onRouteCalculated?.({ distance: 'N/A', duration: 'N/A', distanceValue: 0, durationValue: 0, viaApp: prioritizedMap });
        } else if (result?.geometry) {
    ...
    fetchRoute();
    return () => { cancelled = true; };
    // FIX: getRoute intentionally excluded — stable via ref above.
    // Only re-fetch when the actual coordinates change.
  }, [pickupLocation?.lat, pickupLocation?.lng, dropoffLocation?.lat, dropoffLocation?.lng]); // eslint-disable-line
...
const IframeMap = memo(({
  ...
  // FIX: Store getRoute in a ref so it never appears in the route effect's
  // dep array. Every MapsProvider re-render (e.g. from a failed PUT) creates
  // a new getRoute function reference. Without this ref, that re-render would
  // re-fire the route fetch effect, call Google again, update MapsProvider
  // state, and loop indefinitely — which is exactly what the logs showed.
  const getRouteRef = useRef(mapsProvider?.getRoute);
  useEffect(() => { getRouteRef.current = mapsProvider?.getRoute; }, [mapsProvider?.getRoute]);
  ...
  // ── Route: fetch via MapsProvider then send geometry to iframe ────────────
  // FIX: Extract primitive lat/lng values so the effect dep array contains
  // only numbers, never object references. Object references change on every
  // render even when the coordinates are identical, causing the effect to
  // re-fire, call getRoute again, and loop with MapsProvider.
  const pickupLat = pickupLocation?.lat;
  const pickupLng = pickupLocation?.lng;
  const dropoffLat = dropoffLocation?.lat;
  const dropoffLng = dropoffLocation?.lng;

  // FIX: Coord deduplication guard — skips the fetch entirely when the same
  // coords were already successfully fetched. Protects against MapsProvider
  // re-renders that don't change getRoute behaviour but do produce a new
  // getRoute reference, which would previously re-fire this effect and hit
  // Google again even though the route hasn't changed.
  const lastFetchedRouteKey = useRef(null);
  ...
  useEffect(() => {
    ...
    if (pickupLat == null || pickupLng == null || dropoffLat == null || dropoffLng == null) {
      sendToIframe('CLEAR_ROUTE');
      lastFetchedRouteKey.current = null;
      return;
    }

    // FIX: Skip fetch if these exact coords were already fetched successfully.
    // This prevents a MapsProvider re-render (e.g. from a failed PUT to Strapi)
    // from re-triggering a Google Routes API call for the same route.
    const routeKey = `${pickupLat},${pickupLng}|${dropoffLat},${dropoffLng}`;
    if (routeKey === lastFetchedRouteKey.current) return;
    ...
    async function fetchAndSendRoute() {
      try {
        let result = null;
        // FIX: Read from ref — never re-fires this effect when getRoute
        // reference changes due to MapsProvider internal state updates.
        if (getRouteRef.current) {
          result = await getRouteRef.current(origin, destination);
        }
        if (cancelled) return;
    ...
    // FIX: getRoute deliberately excluded from deps — accessed via getRouteRef.
    // Including it would cause this effect to re-fire every time MapsProvider
    // re-renders (e.g. after a failed Strapi PUT), hitting Google on each tick.
  }, [pickupLat, pickupLng, dropoffLat, dropoffLng, iframeLoaded, sendToIframe]); // eslint-disable-line
