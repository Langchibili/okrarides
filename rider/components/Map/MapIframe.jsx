...
const WazeMapDisplay   = lazy(() => import('../APIProviders/MapDisplays/WazeMapDisplay'));
...
function NativeMapWrapper({
  ...
  console.error(
    '[NativeMapWrapper] RENDER — pickupLocation:', pickupLocation,
    '| dropoffLocation:', dropoffLocation,
    '| pickupLat:', pickupLocation?.lat,
    '| dropoffLat:', dropoffLocation?.lat,
  );

  // Stable ref to mapsProvider — lets the route effect always read the latest
  // value without adding mapsProvider to deps (which would re-run on every
  // provider state update, causing route flicker).
  const mapsProviderRef = useRef(mapsProvider);
  useEffect(() => { mapsProviderRef.current = mapsProvider; }, [mapsProvider]);

  // Primitive dep values — extracted here so they're visible in the render log
  ...
  // Draw route whenever both pickup + dropoff lat/lng primitives change.
  // Also re-runs when mapsProvider becomes ready (handles the case where
  // pickup/dropoff were set before providers finished loading).
  const mapsReady = mapsProvider?.ready ?? false;
  useEffect(() => {
    console.error('[NativeMapWrapper] ROUTE EFFECT FIRED — pickupLat:', pickupLat, '| dropoffLat:', dropoffLat, '| mapsReady:', mapsReady);
    if (!pickupLat || !pickupLng || !dropoffLat || !dropoffLng) { setRoute(null); return; }
    let cancelled = false;

    async function fetchRoute() {
      const mp = mapsProviderRef.current;
      console.error('[NativeMapWrapper] fetchRoute — mp ready:', !!mp, '| hasGetRoute:', !!mp?.getRoute);

      if (mp?.getRoute) {
        const result = await mp.getRoute(
          { lat: pickupLat,  lng: pickupLng  },
          { lat: dropoffLat, lng: dropoffLng },
        );
        if (cancelled) return;
        console.error('[NativeMapWrapper] getRoute result type:', result?.type, '| hasGeometry:', !!result?.geometry);
        ...
        console.warn('[NativeMapWrapper] getRoute returned null — no local map server. Using straight-line fallback.');
      }
  ...
  }, [pickupLat, pickupLng, dropoffLat, dropoffLng, mapsReady]); // mapsReady re-triggers if providers load after pickup/dropoff were set
...
const IframeMap = memo(({
  ...
  useEffect(() => {
    ...
    if (pickupLocation && dropoffLocation) {
      sendToIframe('DRAW_ROUTE', { pickup: pickupLocation, dropoff: dropoffLocation });
    } else {
      sendToIframe('CLEAR_ROUTE');
    }
  }, [markers, pickupLocation, dropoffLocation, iframeLoaded, showRoute, sendToIframe]);
...
(function() {
  ...
  let lastRouteKey = null, routeFailures = 0;
  ...
  async function drawRoute(pickup, dropoff) {
    ...
    if (LOCAL_SERVER && routeFailures < ROUTE_MAX_FAILURES) {
      try {
        ...
        routeFailures = 0;
        renderRouteCoords(data.geometry || [], data.distanceValue / 1000, data.durationValue / 60);
        return;
      } catch(err) {
        routeFailures++;
        console.warn('[MapIframe] Local route failed (' + routeFailures + '):', err.message);
      }
    }

    // 2. Public OSRM (free, no key required)
    try {
      ...
      renderRouteCoords(coords, distKm, durMin);
      return;
    } catch(err) {
      console.warn('[MapIframe] OSRM failed, using straight-line:', err.message);
    }
