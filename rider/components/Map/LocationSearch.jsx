...
//   // ── Theme-aware dropdown values ────────────────────────────────────────────
//   // Shadow: dark mode needs a heavier shadow since the background is already dark.
//   // Divider between "Use Current Location" and predictions: subtle in both modes.
//   // Current-location hover: amber tint works on both light and dark surfaces.
//   const dropdownShadow  = isDark
//     ? '0 8px 40px rgba(0,0,0,0.65), 0 2px 12px rgba(0,0,0,0.4)'
//     : '0 8px 40px rgba(0,0,0,0.20)';
//   const currentLocHover = isDark
//     ? 'rgba(255,193,7,0.12)'
//     : 'rgba(255,193,7,0.08)';
//   const currentLocFocus = currentLocHover;
//   // Paper bgcolor is handled by MUI elevation overlay in dark mode automatically,
//   // but we set it explicitly so the portal element never inherits a transparent bg.
...
//             // Subtle border in dark mode so the sheet edge reads against the map
...
//                 '&:focus': { outline: 'none', bgcolor: currentLocFocus },
//               }}
//             >
...
export const LocationSearch = ({
  ...
  return (
    <Box
      ref={containerRef}
      sx={{ position: 'relative', width: '100%', maxWidth: '100%', boxSizing: 'border-box' }}
    >
      <TextField
        fullWidth size="small"
        value={query}
        onChange={handleInputChange}
        onFocus={() => { setFocused(true); measureDropdown(); externalOnFocus?.(); }}
        onBlur={() => setTimeout(() => { setFocused(false); externalOnBlur?.(); }, 200)}
        placeholder={placeholder}
        autoFocus={autoFocus}
        InputProps={{
