...
// // // // // Strapi enum values (no trailing "map" / "maps"):
...
// // // // // Providers that navigate via deep-link — they cannot geocode
// // // // const DEEP_LINK_PROVIDERS = new Set([PROVIDER.WAZE, PROVIDER.APPLE]);

// // // // // Providers whose "instance" is null because they are handled inline
// // // // // (Nominatim for openstreet, local OSRM for local)
// // // // const INLINE_PROVIDERS = new Set([PROVIDER.OPENSTREET, PROVIDER.LOCAL]);

// // // // // ─────────────────────────────────────────────────────────────────────────────
// // // // // DEFAULT SERVICE → API METHOD MAP PER PROVIDER
// // // // //
// // // // // Tells each service call which API endpoint to use within a provider.
// // // // // Overridable via a Strapi `serviceApis` JSON field on the priority-map type.
// // // // //
...
// // // //   // Inline and deep-link providers never exhaust a quota
...
// // // //     // openstreet and local are handled inline — no class instance needed
...
// // // // // ─────────────────────────────────────────────────────────────────────────────
// // // // // PUBLIC OSRM (OpenStreetMap routing — final fallback for all routing calls)
// // // // // ─────────────────────────────────────────────────────────────────────────────
...
// // // //   // ── Explicit per-service provider assignments (from Strapi) ───────────────
...
// // // //   // ── Per-provider API method config (defaults + optional Strapi overrides) ─
...
// // // //     const failKey = record.providerName;
// // // //     if ((initFailures.current[failKey] || 0) >= 5) {
// // // //       console.warn(`[MapsProvider] initProviderData: ${failKey} failed 5+ times — skipping`);
...
// // // //       if (!res.ok) {
// // // //         initFailures.current[failKey] = (initFailures.current[failKey] || 0) + 1;
// // // //         console.warn(`[MapsProvider] initProviderData PUT failed for ${failKey}: ${res.status}`);
// // // //       } else {
// // // //         initFailures.current[failKey] = 0;
// // // //       }
// // // //     } catch {
...
// // // //   // ── Load priority map + provider records from Strapi ─────────────────────
...
// // // //       try {
// // // //         // 1. Single-type priority map
// // // //         const pmRes  = await fetch(`${apiUrl}/api-providers-priority-map?populate=*`, { headers: authHeaders() });
// // // //         const pmJson = await pmRes.json();
// // // //         const pmRaw  = pmJson?.data ?? {};
// // // //         const attrs  = pmRaw.attributes ?? pmRaw; // handle both v4 and v5 response shapes
       
// // // //         // ── Explicit service assignments ────────────────────────────────────
// // // //         // Note: Strapi schema uses the typo `priotized*` — match it exactly.
...
// // // //         console.log('[MapsProvider] service assignments — map:', pMap, '| route:', pRoute, '| distance:', pDistance, '| eta:', pEta);
...
// // // //         // ── Optional Strapi serviceApis override ────────────────────────────
// // // //         // Strapi may expose a `serviceApis` JSON field with the same shape as
// // // //         // DEFAULT_SERVICE_APIS. Any keys present are deep-merged over the defaults.
// // // //         const strapiApis = attrs.serviceApis ?? null;
// // // //         if (strapiApis && typeof strapiApis === 'object') {
// // // //           setServiceApis((prev) => {
// // // //             const merged = { ...prev };
// // // //             for (const [provName, apis] of Object.entries(strapiApis)) {
// // // //               merged[provName] = { ...(prev[provName] || {}), ...apis };
// // // //             }
// // // //             return merged;
// // // //           });
// // // //         }
...
// // // //         const list = attrs.mapsProviders ?? [];
// // // //         console.log('[MapsProvider] priority list:', list);
// // // //         setPriorityList(list);
// // // //         if (!list.length) { setReady(true); return; }
...
// // // //           // Deep-link + inline providers don't need quota initialisation
// // // //           if (!DEEP_LINK_PROVIDERS.has(name) && !INLINE_PROVIDERS.has(name)) {
// // // //             initProviderData(record).then((updated) => {
// // // //               setProviders((prev) => {
// // // //                 if (!prev[name]) return prev;
// // // //                 return { ...prev, [name]: { ...prev[name], record: updated, budget: computeBudget(updated) } };
...
// // // //             budget:   computeBudget(record),
// // // //           };
// // // //         }

// // // //         // Synthesise entries for providers in the priority list that have no
// // // //         // Strapi record yet, so they still work without a manual CMS step.
// // // //         for (const name of list) {
...
// // // //           };
// // // //           console.log(`[MapsProvider.load] ${name}: no Strapi record — synthesised default`);
// // // //         }

// // // //         setProviders(providerMap);

// // // //         const anyActive = list.some(
// // // //           (n) => providerMap[n]?.instance != null || INLINE_PROVIDERS.has(n),
// // // //         );
// // // //         setAllExhausted(!anyActive);
...
// // // //   // ── Log a request (fire-and-forget PUT to Strapi) ─────────────────────────
...
// // // //   // PROVIDER RESOLUTION
// // // //   //
// // // //   // resolveServiceProvider(explicitName, serviceType, failedSet)
// // // //   //
// // // //   // Resolution order:
// // // //   //   1. Try the explicitly assigned provider (if not already failed)
// // // //   //   2. Walk priorityList top-to-bottom, skipping failed providers
// // // //   //   3. Special substitution: if 'local' appears and has failed →
// // // //   //      immediately try 'openstreet' in its place before continuing the list
// // // //   //
// // // //   // Returns: { name, instance, record } | null
// // // //   // ─────────────────────────────────────────────────────────────────────────
...
// // // //     //  - Has an instance OR is an inline provider (openstreet/local)
// // // //     //  - Has remaining quota (skipped for deep-link and inline providers)
// // // //     const isUsable = (name) => {
// // // //       if (failedSet.has(name)) return false;
// // // //       if (INLINE_PROVIDERS.has(name)) return true; // always usable inline
...
// // // //       console.log(`[MapsProvider.resolve] ✅ explicit: ${explicitName} for ${serviceType}`);
...
// // // //       if (isUsable(name)) {
// // // //         console.log(`[MapsProvider.resolve] ✅ fallback: ${name} for ${serviceType}`);
// // // //         return toEntry(name);
// // // //       }
// // // //     }

// // // //     console.log(`[MapsProvider.resolve] ❌ no provider available for ${serviceType}`);
// // // //     return null;
// // // //   }, [priorityList, providers]);
...
// // // //   //
// // // //   // Drives the retry loop used by getRoute / calculateDistance / calculateEta.
// // // //   // Calls `handler(providerEntry)` — the handler returns a result or null/throws.
// // // //   // On null/throw the provider is added to failedSet and the loop continues.
// // // //   // finalFallback() is called if every provider fails.
// // // //   // ─────────────────────────────────────────────────────────────────────────
...
// // // //     // +2 gives room for the explicit provider try AND the openstreet substitution
// // // //     const maxAttempts = priorityList.length + 2;

// // // //     for (let i = 0; i < maxAttempts; i++) {
// // // //       // Only pass explicit on the first attempt; after it fails it goes into failed set
...
// // // //         // null result = provider returned nothing useful
// // // //         console.log(`[MapsProvider.${serviceType}] ${p.name} returned no result`);
// // // //         failed.add(p.name);
// // // //       } catch (err) {
...
// // // //     console.warn(`[MapsProvider.${serviceType}] all providers failed — running final fallback`);
// // // //     return finalFallback();
// // // //   }, [priorityList, resolveServiceProvider]);

// // // //   // ─────────────────────────────────────────────────────────────────────────
// // // //   // PUBLIC API: searchPlaces
// // // //   //
// // // //   // Uses the priority list (no explicit per-search field in Strapi yet).
// // // //   // Skips deep-link providers that advertise no autoComplete API.
// // // //   // Falls back to Nominatim when every provider fails.
// // // //   // ─────────────────────────────────────────────────────────────────────────
...
// // // //       handler: async (p) => {
// // // //         // Skip deep-link providers that have no autoComplete API configured
// // // //         if (DEEP_LINK_PROVIDERS.has(p.name) && !serviceApis[p.name]?.autoComplete) {
// // // //           return null;
// // // //         }

// // // //         // Inline providers → Nominatim
...
// // // //     });
// // // //   }, [runWithFallback, serviceApis, logRequest]);
...
// // // //   const getPlaceDetails = useCallback(async (placeId, fallbackData = null) => {
// // // //     // Fast path: caller already has coords
...
// // // //       explicitProvider: null,
// // // //       handler: async (p) => {
// // // //         if (INLINE_PROVIDERS.has(p.name)) return fallbackData; // can't resolve a place_id via Nominatim
// // // //         if (!p.instance?.getPlaceDetails) return null;
// // // //         const result = await p.instance.getPlaceDetails(placeId, fallbackData);
...
// // // //         const result = await p.instance.reverseGeocode(lat, lng);
// // // //         if (result) { logRequest(p.name, 'GeocoderHTTPAPI'); return result; }
// // // //         return null;
// // // //       },
...
// // // //   // PUBLIC API: getRoute
// // // //   //
// // // //   // Uses `prioritizedRoute` as the first-choice provider.
// // // //   // local failure → openstreet substitution is handled by resolveServiceProvider.
// // // //   // Absolute final fallback: public OSRM (openstreetmap).
// // // //   // ─────────────────────────────────────────────────────────────────────────

// // // //   const getRoute = useCallback(async (origin, destination) => {
// // // //     return runWithFallback({
// // // //       serviceType:      'routing',
// // // //       explicitProvider: prioritizedRoute,
// // // //       handler: async (p) => {
// // // //         // Deep-link providers open the native navigation app
...
// // // //         // OpenStreetMap — public OSRM
// // // //         if (p.name === PROVIDER.OPENSTREET) {
// // // //           return osrmRoute(origin, destination);
// // // //         }

// // // //         // Provider-specific route API
// // // //         if (!p.instance?.getRoute) return null;
// // // //         const result = await p.instance.getRoute(origin, destination);
// // // //         if (result) { logRequest(p.name, 'GeocoderHTTPAPI'); return result; }
// // // //         return null;
// // // //       },
// // // //       // If absolutely everything fails, try public OSRM one last time
// // // //       finalFallback: () => osrmRoute(origin, destination),
// // // //     });
// // // //   }, [prioritizedRoute, runWithFallback, fetchLocalRoute, logRequest]);

// // // //   // ─────────────────────────────────────────────────────────────────────────
// // // //   // PUBLIC API: calculateDistance
// // // //   //
// // // //   // Uses `prioritizedDistance`. Falls back through the priority list.
// // // //   // local failure → openstreet. Final fallback: haversine straight-line.
// // // //   // ─────────────────────────────────────────────────────────────────────────

// // // //   const calculateDistance = useCallback(async (origin, destination) => {
// // // //     return runWithFallback({
// // // //       serviceType:      'distance',
// // // //       explicitProvider: prioritizedDistance,
// // // //       handler: async (p) => {
// // // //         // Inline providers — use OSRM distance
// // // //         if (p.name === PROVIDER.LOCAL) {
// // // //           const route = await fetchLocalRoute(origin, destination);
// // // //           return route ? { distanceValue: route.distanceValue, distance: route.distance } : null;
// // // //         }
// // // //         if (p.name === PROVIDER.OPENSTREET) {
...
// // // //         const result = await p.instance.calculateDistance(origin, destination);
// // // //         if (result != null) { logRequest(p.name, 'GeocoderHTTPAPI'); return result; }
// // // //         return null;
// // // //       },
// // // //       finalFallback: async () => {
// // // //         // Haversine straight-line — always available, always works
// // // //         const R    = 6371;
...
// // // //   // ─────────────────────────────────────────────────────────────────────────
// // // //   // PUBLIC API: calculateEta
// // // //   //
// // // //   // Uses `prioritizedEta`. Falls back through the priority list.
// // // //   // local failure → openstreet.
// // // //   // ─────────────────────────────────────────────────────────────────────────

// // // //   const calculateEta = useCallback(async (origin, destination) => {
// // // //     return runWithFallback({
// // // //       serviceType:      'eta',
// // // //       explicitProvider: prioritizedEta,
// // // //       handler: async (p) => {
// // // //         if (p.name === PROVIDER.LOCAL) {
// // // //           const route = await fetchLocalRoute(origin, destination);
// // // //           return route ? { durationValue: route.durationValue, duration: route.duration } : null;
// // // //         }
// // // //         if (p.name === PROVIDER.OPENSTREET) {
// // // //           const route = await osrmRoute(origin, destination);
// // // //           return route ? { durationValue: route.durationValue, duration: route.duration } : null;
...
// // // //         const result = await p.instance.calculateEta(origin, destination);
// // // //         if (result != null) { logRequest(p.name, 'GeocoderHTTPAPI'); return result; }
// // // //         return null;
// // // //       },
// // // //       finalFallback: async () => null, // ETA has no haversine equivalent
// // // //     });
...
// // // //     // Display
// // // //     prioritizedMap,
// // // //     // Explicit service assignments (exposed so consumers can read them)
// // // //     prioritizedRoute,
// // // //     prioritizedDistance,
// // // //     prioritizedEta,
// // // //     // Service API config
...
// // // // Providers handled inline (no SDK instance needed)
// // // const INLINE_PROVIDERS = new Set([PROVIDER.OPENSTREET, PROVIDER.LOCAL]);
...
// // // //
// // // // Strapi stores mapsProviders as a flat alternating JSON array:
// // // //   ["google:", { routing: "routeApi", ... }, "waze:", {}, "apple:", {}, ...]
// // // //
// // // // We need to turn that into:
// // // //   - providerNames: ["google", "waze", "apple", ...]   (clean, no colon)
// // // //   - perProviderApis: { google: { routing: "routeApi", ... }, ... }
// // // //
// // // // Rules:
// // // //   - String entries are provider names; strip the trailing colon if present.
// // // //   - The object immediately following a string is that provider's API config.
// // // //   - Plain objects that appear WITHOUT a preceding string are skipped.
// // // //   - Duplicate names are allowed (Strapi quirk) — last config wins.
// // // // ─────────────────────────────────────────────────────────────────────────────

// // // function parseMapsProviders(rawList) {
// // //   const providerNames  = []; // ordered, deduplicated
...
// // //     // Skip plain objects — they are config blocks, handled below
// // //     if (typeof item !== 'string') continue;

// // //     // Strip trailing colon: "google:" → "google"
// // //     const name = item.replace(/:$/, '').trim();
// // //     if (!name) continue;

// // //     // Collect config: the very next element if it's a plain object
...
// // //     // Add to ordered list (first occurrence wins for ordering)
// // //     if (!seen.has(name)) {
// // //       providerNames.push(name);
// // //       seen.add(name);
// // //     }

// // //     // Merge config (last config wins for duplicate names)
...
// // //     case PROVIDER.GEOAPIFY:   return new GeoapifyProvider(process.env.NEXT_PUBLIC_GEOAPIFY_API_KEY || '');
// // //     case PROVIDER.OPENSTREET:
// // //     case PROVIDER.LOCAL:      return null; // handled inline
// // //     default:
...
// // //         // ── Parse mapsProviders ─────────────────────────────────────────────
// // //         // Strapi returns this as a flat alternating array:
// // //         //   ["google:", {routing:"routeApi",...}, "waze:", {}, ...]
// // //         // parseMapsProviders extracts clean names and per-provider API configs.
// // //         const rawList = attrs.mapsProviders ?? [];
// // //         const { providerNames: list, perProviderApis } = parseMapsProviders(rawList);

// // //         console.log('[MapsProvider] parsed priority list:', list);
// // //         console.log('[MapsProvider] per-provider API overrides:', perProviderApis);

// // //         // Merge Strapi per-provider configs over the defaults
// // //         if (Object.keys(perProviderApis).length > 0) {
// // //           setServiceApis((prev) => {
// // //             const merged = { ...prev };
...
// // //         // Also check for a top-level serviceApis JSON override (optional)
// // //         const strapiApis = attrs.serviceApis ?? null;
// // //         if (strapiApis && typeof strapiApis === 'object') {
// // //           setServiceApis((prev) => {
// // //             const merged = { ...prev };
...
// // //         // Only query for names that have a real API key-based provider —
// // //         // inline providers (openstreet, local) don't have Strapi records.
...
// // //         // ── Synthesise entries for providers with no Strapi record ──────────
// // //         for (const name of list) {
// // //           if (providerMap[name]) continue;
...
// // //           console.log(`[MapsProvider.load] "${name}": no Strapi record — synthesised default | instance: ${providerMap[name].instance ? 'created' : 'inline/null'}`);
// // //         }

// // //         setProviders(providerMap);

// // //         // Summary log
// // //         console.log('[MapsProvider.load] providerMap:');
// // //         for (const [k, v] of Object.entries(providerMap)) {
// // //           const isInline = INLINE_PROVIDERS.has(k);
// // //           console.log(`  ${k}: instance=${!!v.instance} | inline=${isInline} | budget.monthly=${v.budget?.monthlyRemaining}`);
// // //         }
...
// // //       explicitProvider: null,
// // //       handler: async (p) => {
// // //         if (DEEP_LINK_PROVIDERS.has(p.name) && !serviceApis[p.name]?.autoComplete) return null;
// // //         if (INLINE_PROVIDERS.has(p.name)) {
// // //           const results = await nominatimSearch(query, countryCode);
// // //           return results.length ? results : null;
// // //         }
...
// // // ─────────────────────────────────────────────────────────────────────────────
// // // PROVIDER NAME CONSTANTS
// // // Match Strapi enum values exactly (no trailing "map" / "maps").
// // // ─────────────────────────────────────────────────────────────────────────────
...
// // // DEFAULT SERVICE → API METHOD MAP PER PROVIDER
// // //
// // // These are the fallback defaults when Strapi doesn't supply a config object
// // // for a provider.  When Strapi does supply one (e.g. { routing: 'routeApi' })
// // // it is deep-merged over these defaults during load.
// // //
// // // service keys:  routing | distance | eta | autoComplete | location
// // // ─────────────────────────────────────────────────────────────────────────────
...
// // //
// // // Produces:
// // //   providerNames:   ["google", "waze", "apple", ...]   ordered, no colon
// // //   perProviderApis: { google: { routing: "routeApi", ... }, ... }
// // // ─────────────────────────────────────────────────────────────────────────────
...
// //     // Non-empty config objects are stored; last one wins for duplicate names
// //     if (Object.keys(config).length > 0) {
// //       perProviderApis[name] = { ...(perProviderApis[name] || {}), ...config };
// //     }
// //   }
...
// //   // serviceApis[providerName][serviceType] = apiMethodString
...
// //         // ── Parse the mixed provider list ───────────────────────────────────
// //         // ["google:", { routing: "routeApi", ... }, "waze:", {}, ...]
// //         //   → providerNames: ["google", "waze", ...]
// //         //   → perProviderApis: { google: { routing: "routeApi", ... } }
// //         const rawList = attrs.mapsProviders ?? [];
// //         const { providerNames: list, perProviderApis } = parseMapsProviders(rawList);

// //         console.log('[MapsProvider] parsed priority list:', list);
// //         console.log('[MapsProvider] per-provider api overrides from Strapi:', perProviderApis);

// //         // Merge Strapi per-provider configs over the defaults.
// //         // This is what makes { routing: 'routeApi' } from Strapi take effect.
// //         setServiceApis((prev) => {
// //           const merged = { ...prev };
// //           for (const [provName, apis] of Object.entries(perProviderApis)) {
// //             merged[provName] = { ...(DEFAULT_SERVICE_APIS[provName] || {}), ...(prev[provName] || {}), ...apis };
// //           }
// //           return merged;
// //         });

// //         // Also support a top-level serviceApis JSON override (optional Strapi field)
// //         const strapiApis = attrs.serviceApis ?? null;
// //         if (strapiApis && typeof strapiApis === 'object') {
// //           setServiceApis((prev) => {
// //             const merged = { ...prev };
...
// //         // Synthesise entries for providers not in Strapi yet
// //         for (const name of list) {
// //           if (providerMap[name]) continue;
// //           const defaultRecord = {
...
// //             budget: computeBudget(defaultRecord),
// //           };
// //           console.log(`[MapsProvider.load] "${name}": synthesised default | instance: ${providerMap[name].instance ? 'created' : 'inline/null'}`);
// //         }
...
// //   // ── logRequest (circuit-breaks after 3 consecutive 404s per provider) ─────
...
// //               res.status === 404 ? 'Enable update permission for api-providers in Strapi → Settings → Roles.' : `HTTP ${res.status}`,
// //             );
// //           }
// //         } else {
...
// //   // PROVIDER RESOLUTION
// //   //
// //   // Flow for an explicit provider (e.g. prioritizedRoute = 'google'):
// //   //   1. Skip the priority list entirely
// //   //   2. Look up 'google' in providers map
// //   //   3. Check its quota
// //   //   4. Return it with its apiMethod from serviceApis['google']['routing']
// //   //
// //   // Flow when explicit provider fails or is absent:
// //   //   1. Walk priorityList top-to-bottom
// //   //   2. local failure → substitute openstreet before continuing
// //   //
...
// //     // Build the full entry including the api method for this service
// //     const toEntry = (name) => ({
// //       name,
// //       instance:  providers[name]?.instance ?? null,
// //       record:    providers[name]?.record   ?? null,
// //       // Resolve apiMethod: Strapi-merged config first, then defaults, then null
// //       apiMethod: serviceApis[name]?.[serviceType] ?? DEFAULT_SERVICE_APIS[name]?.[serviceType] ?? null,
// //     });

// //     // 1. Explicit provider — jump straight to it, bypass the list
// //     if (explicitName && isUsable(explicitName)) {
// //       console.log(`[MapsProvider.resolve] ✅ explicit: "${explicitName}" for ${serviceType} | apiMethod: ${toEntry(explicitName).apiMethod}`);
// //       return toEntry(explicitName);
// //     }

// //     // 2. Priority list walk
// //     for (const name of priorityList) {
// //       if (name === explicitName) continue; // already tried
...
// //       if (isUsable(name)) {
// //         console.log(`[MapsProvider.resolve] ✅ fallback: "${name}" for ${serviceType} | apiMethod: ${toEntry(name).apiMethod}`);
// //         return toEntry(name);
// //       }
// //     }
...
// //   // GENERIC SERVICE RUNNER
// //   //
// //   // handler(providerEntry) where providerEntry = { name, instance, record, apiMethod }
// //   //
// //   // apiMethod is derived from serviceApis[name][serviceType] and tells the
// //   // provider which internal implementation to use.  Handlers pass it through
// //   // to the provider method:
// //   //   p.instance.getRoute(origin, destination, p.apiMethod)
// //   //   p.instance.searchPlaces(query, countryCode, p.apiMethod)
// //   //   etc.
// //   // ─────────────────────────────────────────────────────────────────────────
...
// //       try {
// //         // handler receives the full entry INCLUDING apiMethod
// //         const result = await handler(p);
// //         if (result != null) return result;
// //         console.log(`[MapsProvider.${serviceType}] "${p.name}" (${p.apiMethod}) returned no result`);
// //         failed.add(p.name);
// //       } catch (err) {
...
// //     return runWithFallback({
// //       serviceType:      'autoComplete',
// //       explicitProvider: null, // no dedicated Strapi field for autoComplete yet
// //       handler: async (p) => {
// //         // Skip deep-link providers with no autocomplete implementation
// //         if (DEEP_LINK_PROVIDERS.has(p.name) && !p.apiMethod) return null;

// //         // Inline providers → Nominatim (apiMethod = 'nominatim')
// //         if (INLINE_PROVIDERS.has(p.name)) {
// //           const results = await nominatimSearch(query, countryCode);
// //           return results.length ? results : null;
// //         }
...
// //         // Pass apiMethod so the provider can dispatch to the right implementation
// //         // e.g. 'placesApi' → GoogleMapsProvider._searchViaAutoCompleteNew()
...
// //         if (INLINE_PROVIDERS.has(p.name)) return fallbackData;
// //         if (!p.instance?.getPlaceDetails) return null;
// //         // Pass apiMethod: e.g. 'geocoding' → GoogleMapsProvider._placeDetailsViaGeocoding()
...
// //         if (INLINE_PROVIDERS.has(p.name)) return nominatimReverse(lat, lng);
// //         if (!p.instance?.reverseGeocode) return null;
// //         // Pass apiMethod: e.g. 'geocoding' → GoogleMapsProvider._reverseGeocodeViaGeocoding()
...
// //   // PUBLIC API: getRoute
// //   //
// //   // explicitProvider = prioritizedRoute (e.g. 'google')
// //   // → resolveServiceProvider finds 'google', reads serviceApis['google']['routing']
// //   //   = 'routeApi', attaches it as p.apiMethod
// //   // → handler calls p.instance.getRoute(origin, destination, 'routeApi')
// //   // → GoogleMapsProvider dispatches to _routeViaRoutesApi()
// //   // ─────────────────────────────────────────────────────────────────────────
...
// //         if (p.name === PROVIDER.OPENSTREET) return osrmRoute(origin, destination);
// //         if (!p.instance?.getRoute) return null;
// //         // p.apiMethod = 'routeApi' (from Strapi config or default)
...
// //         }
// //         if (!p.instance?.calculateDistance) return null;
// //         // p.apiMethod = 'routeApi' → GoogleMapsProvider delegates to _routeViaRoutesApi internally
