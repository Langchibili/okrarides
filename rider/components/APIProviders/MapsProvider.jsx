...
// //     distance:     'routeApi',
// //     eta:          'routeApi',
// //     // ── Updated: use the GeoSuggest API for autocomplete ──────────────────
// //     // Falls back to 'suggest' inside YandexMapsProvider when the key is absent.
...
// // /**
// //  * Maps an apiType string to the daily-count bucket key.
// //  *
// //  * Bucket  │ apiType values
// //  * ────────┼──────────────────────────────────────────
// //  * 'js'    │ 'JavaScriptAPI'
// //  * 'geo'   │ 'GeocoderHTTPAPI', 'GeoSuggestAPI', and everything else
// //  *
// //  * GeoSuggestAPI is billed like a Geocoder HTTP call (per-request REST),
// //  * so it goes into the 'geo' bucket — matching the existing requestCounts data:
// //  *   { "GeoSuggestAPI": 0, "GeocoderHTTPAPI": 835 }
// //  */
// // function dailyBucketKey(apiType) {
// //   return apiType === 'JavaScriptAPI' ? 'js' : 'geo';
// // }
...
// //     // ── YANDEX ─────────────────────────────────────────────────────────────
// //     // Two keys are needed:
// //     //   NEXT_PUBLIC_YANDEX_MAPS_API_KEY      → Geocoder + Router
// //     //   NEXT_PUBLIC_YANDEX_GEOSUGGESTAPIKEY  → GeoSuggest (autocomplete)
// //     //
// //     // Both are passed into YandexMapsProvider.  If the GeoSuggest key is absent
// //     // the provider falls back to the basic Suggest API automatically.
// //     case PROVIDER.YANDEX: {
// //       const geocoderKey    = process.env.NEXT_PUBLIC_YANDEX_MAPS_API_KEY     || '';
// //       const geoSuggestKey  = process.env.NEXT_PUBLIC_YANDEX_GEOSUGGESTAPIKEY || '';

// //       L.info('createProviderInstance', '"yandex"', {
// //         geocoderKeyPresent:   !!geocoderKey,
// //         geoSuggestKeyPresent: !!geoSuggestKey,
// //       });
// //       if (!geocoderKey)   L.warn('createProviderInstance', '"yandex" geocoderKey missing  — NEXT_PUBLIC_YANDEX_MAPS_API_KEY not set');
// //       if (!geoSuggestKey) L.warn('createProviderInstance', '"yandex" geoSuggestKey missing — NEXT_PUBLIC_YANDEX_GEOSUGGESTAPIKEY not set (will fall back to basic suggest)');
...
// //         L.info('searchPlaces', `"${p.name}" returned ${results?.length ?? 0} results`);
// //         if (results?.length) {
// //           // ── Use the resolved apiMethod as the billing key ─────────────────
// //           // Previously this was hardcoded to 'JavaScriptAPI' for every provider.
// //           // Now we pass the actual method so that Yandex GeoSuggest calls are
// //           // recorded as 'GeoSuggestAPI' (→ 'geo' daily bucket) in Strapi, which
// //           // matches the existing requestCounts schema:
// //           //   { "GeoSuggestAPI": N, "GeocoderHTTPAPI": N }
// //           const billingType = p.apiMethod || 'JavaScriptAPI';
// //           L.trace('searchPlaces', `logRequest "${p.name}" as "${billingType}"`);
// //           logRequest(p.name, billingType);
// //           return results;
// //         }
// //         L.warn('searchPlaces', `"${p.name}" no results`); return null;
// //       },
...
//           priotizedDistanceCalculator: attrs.priotizedDistanceCalculator,
//           priotizedEtaCalculator:      attrs.priotizedEtaCalculator,
//           priotizedAutocompleteApi:    attrs.priotizedAutocompleteApi,
//           mapsProviders:               attrs.mapsProviders,
//         });
...
//         L.info('load()', 'Step 1: resolved priorities', { pMap, pRoute, pDistance, pEta, pAutocomplete });
//         if (!pRoute)        L.warn('load()', 'priotizedRouteDrawer is null/unset in Strapi');
//         if (!pDistance)     L.warn('load()', 'priotizedDistanceCalculator is null/unset in Strapi');
//         if (!pEta)          L.warn('load()', 'priotizedEtaCalculator is null/unset in Strapi');
//         if (!pAutocomplete) L.warn('load()', 'priotizedAutocompleteApi is null/unset in Strapi — autocomplete will walk the priority list');
...
//         // ── Step 3: auto-include any prioritized service providers missing from list ──
//         L.info('load()', 'Step 3: ensuring prioritized service providers are in the list');
//         const serviceProviders = [pRoute, pDistance, pEta, pAutocomplete].filter(Boolean);
//         L.info('load()', 'Step 3: service providers from Strapi priority fields', serviceProviders);
//         for (const name of serviceProviders) {
//           if (!list.includes(name)) {
//             list.push(name);
...
//   const searchPlaces = useCallback(async (query, countryCode = null) => {
//     L.info('searchPlaces', 'called', { query, countryCode, prioritizedAutocomplete });
//     if (!query || query.length < 2) { L.trace('searchPlaces', 'query too short'); return []; }
//     return runWithFallback({
//       // Use the Strapi-configured autocomplete provider as the explicit first
//       // choice.  resolveServiceProvider will fall through to the priority list
//       // if that provider is unavailable or returns no results.
...
//   L.trace('MapsProvider', 'rendering context provider', { ready, allExhausted, prioritizedMap, prioritizedRoute, prioritizedAutocomplete });

//   return <MapsContext.PrAovider value={value}>{children}</MapsContext.Provider>;
// }
...
function createProviderInstance(name) {
  ...
  switch (name) {
    ...
    // Two keys are needed:
    //
    //   NEXT_PUBLIC_YANDEX_MAPS_GEOCODER_AND_JAVASCRIPITAPI_KEY
    //     → Geocoder HTTP API + Router API
    //       Used to resolve suggest selections → lat/lng, and for routing.
    //
    //   NEXT_PUBLIC_YANDEX_GEOSUGGESTAPIKEY
    //     → GeoSuggest API (autocomplete)
    //       If absent the provider falls back to the basic Suggest API.
    //
    // The geocoder key is passed as the FIRST constructor argument.
    // ─────────────────────────────────────────────────────────────────────
    case PROVIDER.YANDEX: {
      ...
      L.info('createProviderInstance', '"yandex"', {
        geocoderKeyPresent:   !!geocoderKey,
        geoSuggestKeyPresent: !!geoSuggestKey,
        geocoderEnvVar:       'NEXT_PUBLIC_YANDEX_MAPS_GEOCODER_AND_JAVASCRIPITAPI_KEY',
        suggestEnvVar:        'NEXT_PUBLIC_YANDEX_GEOSUGGESTAPIKEY',
      });

      if (!geocoderKey) {
        L.warn('createProviderInstance',
          '"yandex" geocoderKey missing — ' +
          'NEXT_PUBLIC_YANDEX_MAPS_GEOCODER_AND_JAVASCRIPITAPI_KEY not set. ' +
          'Geocoder and Router calls will fail.'
        );
      }
      if (!geoSuggestKey) {
        L.warn('createProviderInstance',
          '"yandex" geoSuggestKey missing — ' +
          'NEXT_PUBLIC_YANDEX_GEOSUGGESTAPIKEY not set. ' +
          'Will fall back to basic Suggest API using the geocoder key.'
        );
      }
