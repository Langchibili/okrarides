...
export default function DeliveryTrackingPage() {
  ...
  useEffect(() => {
    ...
    on('delivery:driver:arrived', onArrived);
    on('delivery:started',        onStarted);
    on('delivery:payment:requested', onPayReq);
    on('delivery:completed',      onComplete);
    on('delivery:cancelled',      onCancelled);

    return () => {
      off('delivery:driver:arrived');
      off('delivery:started');
      off('delivery:payment:requested');
      off('delivery:completed');
      off('delivery:cancelled');
    };
  }, [connected, id, on, off, router]);

  // ─── Live deliverer location + ETA ───────────────────────────────────────
  useEffect(() => {
    ...
    const updateLoc = async () => {
      ...
      try {
        ...
        // One-time route draw per status
  ...
  const stepIdx  = STATUS_IDX[rideStatus] ?? 0;
