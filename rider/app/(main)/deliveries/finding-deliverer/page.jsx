// // PATH: rider/app/(main)/deliveries/finding-deliverer/page.jsx
// 'use client';
...
//   { label: 'Waiting for acceptance…', duration: null }, // open-ended
// ];
...
//         // If already accepted (e.g. fast acceptance), redirect immediately
//         if (data?.rideStatus === 'accepted') {
//           router.replace(`/deliveries/${deliveryId}/tracking`);
//         }
//         if (data?.rideStatus === 'cancelled' || data?.rideStatus === 'no_drivers_available') {
//           setNoDrivers(true);
//         }
//       } catch (err) {
...
//   // ─── Status polling (fallback to socket) ─────────────────────────────────
//   useEffect(() => {
...
//       if (!mountedRef.current) return;
//       setElapsed(s => s + 1);
//       setProgress(p => Math.min(p + (100 / 120), 99)); // fills over ~2min
//     }, 1000);
...
//           {noDrivers ? (
//             <motion.div key="no-drivers" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
//               <Box sx={{ textAlign: 'center', py: 1 }}>
//                 <Typography sx={{ fontSize: 40, mb: 1 }}>😔</Typography>
//                 <Typography variant="h6" fontWeight={700} sx={{ mb: 0.75 }}>No Deliverers Available</Typography>
//                 <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
//                   No nearby deliverers accepted your request. Try again in a few minutes.
//                 </Typography>
//                 <Button variant="contained" fullWidth onClick={() => router.push('/deliveries/send')}
//                   sx={{ height: 52, borderRadius: 3, fontWeight: 700, background: `linear-gradient(135deg,${AMBER} 0%,#D97706 100%)`, color: '#fff', mb: 1.5 }}>
//                   Try Again
//                 </Button>
//                 <Button variant="outlined" fullWidth onClick={() => router.replace('/deliveries')}
...
//                   '& .MuiLinearProgress-bar': { background: `linear-gradient(90deg,${AMBER},#D97706)`, borderRadius: 2.5 },
//                 }}
//               />

//               {/* Stage label */}
//               <AnimatePresence mode="wait">
//                 <motion.div key={stage} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }}>
//                   <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 2 }}>
//                     <CircularProgress size={22} sx={{ color: AMBER }} />
//                     <Typography variant="subtitle1" fontWeight={600}>{STAGES[stage]?.label}</Typography>
//                     <Box sx={{ flex: 1 }} />
...
//                     <Box sx={{ mt: 1.5, pt: 1.5, borderTop: `1px solid ${alpha(isDark?'#fff':'#000',0.07)}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
//                       <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75 }}>
//                         <PackageIcon sx={{ fontSize: 14, color: AMBER }} />
//                         <Typography variant="caption" color="text.secondary" textTransform="capitalize">
//                           {delivery.package?.packageType ?? 'Package'}
//                         </Typography>
//                         {delivery.package?.isFragile && <Chip label="Fragile" size="small" color="error" sx={{ height: 16, fontSize: '0.6rem', fontWeight: 700 }} />}
//                       </Box>
