...
export default function SendPackagePage() {
  ...
  // ── Location hooks ─────────────────────────────────────────────────────
  const { location, loading: locationLoading, refresh: refreshWebLocation } = useGeolocation({ watch: true });
  const { isNative, reconnectDeviceSocket, stopLocationTracking, startLocationTracking, getCurrentLocation: getNativeLocation } = useReactNative();
  ...
  //
  // Root cause (same as HomePage):
  //   fetchAndApplyNativeLocation calls fetchServerStoredLocation without await,
  //   so both run concurrently. fetchServerStoredLocation may set pickupLocation
  //   to a geocoded address first, but applyLocationFix's initial raw-coordinate
  //   setPickupLocation call then unconditionally overwrites it. By the time
  //   applyLocationFix tries to geocode, geoCodeCounterRef is already at 2 and
  //   reverseGeocodeCoords returns null — leaving raw coords displayed forever.
  //
  // Fix:
  //   Stamp `_geocoded: true` on every location object produced by a real
  //   geocoding call. applyLocationFix's initial raw-coordinate write checks
  //   for that flag and skips the overwrite if the existing isCurrentLocation
  //   is already geocoded.
  // ─────────────────────────────────────────────────────────────────────────
  const applyLocationFix = useCallback(async (lat, lng) => {
    if (lat == null || lng == null) return;
    ...
    if (geocoded) {
      setPickupLocation((prev) => {
        ...
        return {
          ...
          lng: geocoded.longitude,
          address: geocoded.address || `${lat.toFixed(5)}, ${lng.toFixed(5)}`,
          name: geocoded.name || geocoded.streetAddress || geocoded.address?.split(',')[0] || `(${geocoded.latitude.toFixed(2)},${geocoded.longitude.toFixed(2)})`,
          placeId: geocoded.placeId || `geo_${lat}_${lng}`,
          city: geocoded.city,
          country: geocoded.country,
          state: geocoded.state,
  ...
  // FIX: stamps _geocoded: true on the location it writes, so applyLocationFix
  //      won't overwrite it with raw coordinates when both run concurrently.
  const fetchServerStoredLocation = async (userId) => {
    if (userId) {
      if (geoCodeCounterRef.current >= 2) return; // ← ref, always current
      const res = await apiClient.get(`/users/${userId}`);
  ...
  const handlePickupSelect = useCallback((loc) => {
    setPickupLocation(loc); saveToRecent(loc);
    if (dropoffLocation) { hideNav(); }
    if (mapControls) mapControls.animateToLocation(loc, 15);
    setTimeout(() => setFocusedInput(null), 150);
  }, [mapControls, saveToRecent, dropoffLocation]);

  const handleDropoffSelect = useCallback((loc) => {
    setDropoffLocation(loc); saveToRecent(loc);
    if (pickupLocation) { hideNav(); }
    if (mapControls) mapControls.animateToLocation(loc, 15);
    setTimeout(() => setFocusedInput(null), 150);
  }, [mapControls, saveToRecent, pickupLocation]);
