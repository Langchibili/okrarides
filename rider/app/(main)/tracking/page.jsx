...
export default function TrackingPage() {
  ...
  // Stable ride ID for comparison
  const currentRideId = currentRide?.id || rideId;

  const initialMapCenter = useMemo(() => (
    normalizeCoords(currentRide?.pickupLocation) || { lat: -15.4167, lng: 28.2833 }
  ), [currentRideId]);

  // ── EFFECT 1: mount/unmount & manual load ─────────────────────────────────
  useEffect(() => {
    mountedRef.current = true;
    stopLocationTracking();

    // MANUAL LOAD: Load active ride when component mounts
    loadActiveRide();
  ...
  }, [loadActiveRide, stopLocationTracking, stopTracking]);

  // ── EFFECT 2: Setup socket listeners and start tracking when ride is active ──
  useEffect(() => {
    if (!currentRideId) return;

    // MANUAL SETUP: Setup socket listeners for this ride
    setupSocketListeners(currentRideId);

    // MANUAL TRACKING: Start tracking when we have an active ride
    startTracking(currentRideId, (trackingData) => {
      // Optional callback for tracking updates
      console.log('Tracking update:', trackingData);
    });

    return () => {
      // Cleanup is handled by useRide's internal cleanup
    };
  }, [currentRideId, setupSocketListeners, startTracking]);

  // ── EFFECT 3: Reset route on status change ───────────────────────────────
  useEffect(() => {
    if (!rideStatus) return;
    if (DBG().BREAK_ROUTE_RESET) return;
  ...
  // ── EFFECT 4: Driver location polling (fallback when RN bridge not available) ──
  useEffect(() => {
    if (!rideId || !currentRide || ['completed', 'cancelled'].includes(rideStatus)) {
      return;
    }
    if (DBG().BREAK_POLL) return;
    ...
    const poll = async () => {
      ...
      try {
        ...
        if (loc) {
          ...
          // Update ETA and distance based on status
          if (rideStatus === 'accepted') {
            const pickup = normalizeCoords(td.ride?.pickupLocation ?? currentRide.pickupLocation);
            if (pickup) {
              const km = calculateDistance(loc, pickup);
              setDistance(prev => Math.abs(prev - km) < 0.01 ? prev : km);
              setEta(prev => {
                const newEta = estimateDuration(km);
                return prev === newEta ? prev : newEta;
              });
            }
          ...
          // Update route for accepted status
          if (rideStatus === 'accepted') {
            const pickup = normalizeCoords(td.ride?.pickupLocation ?? currentRide.pickupLocation);
            if (pickup) {
              ...
              if (movedEnough) {
                ...
                setRoute(prev => {
                  if (prev.pickup?.lat === loc.lat && prev.pickup?.lng === loc.lng) return prev;
                  return { pickup: loc, dropoff: pickup, ready: true };
                });
              }
            }
          }

          // Update route for onboard status
        ...
        // Stop polling if ride is complete
  ...
  }, [rideId, rideStatus, currentRide?.pickupLocation, currentRide?.dropoffLocation]);

  // ── EFFECT 5: React Native live location bridge ───────────────────────────
  useEffect(() => {
    if (DBG().BREAK_RN_BRIDGE) return;
    ...
    const unsub = rnOn('DRIVER_LOCATION_UPDATED', (payload) => {
      ...
      if (payload.eta != null) {
        setEta(prev => prev === payload.eta ? prev : payload.eta);
      }
      if (payload.distance != null) {
        setDistance(prev => prev === payload.distance ? prev : payload.distance);
      }
    });
  ...
  useEffect(() => {
    ...
    const fetchPic = async () => {
      if (currentRide?.driver?.id) {
        const pic = await loadRideDriverProfilePicUrl(currentRide.driver.id);
        if (mountedRef.current) setDriverProfilePic(pic);
      }
    };
    fetchPic();
  }, [currentRideId, loadRideDriverProfilePicUrl]);

  // ── EFFECT 7: Ride status side-effects (redirects) ────────────────────────
  useEffect(() => {
    if (!currentRide) return;
    if (DBG().BREAK_STATUS_FX) return;
    ...
    if (_redirectedRideIds.has(guardKey)) return;
  ...
  }, [currentRide?.rideStatus, currentRide?.id, rideId, router, stopTracking]);
