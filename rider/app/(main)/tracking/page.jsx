...
// //                   ? process.env.NEXT_PUBLIC_UPLOAD_PUBLIC_API_URL + getImageUrl(driverProfilePic, 'thumbnail') 
// //                   : undefined
// //                 }
...
//   // ── Redirect guard: prevents duplicate redirects firing from stale effect
//   //    closures or unstable deps (e.g. stopTracking changing every render) ────
...
//   // ── Reset redirect guard when the rideId changes ──────────────────────────
//   useEffect(() => {
//     hasRedirectedRef.current = false;
//   }, [rideId]);

//   // ── Reset route state whenever status changes ─────────────────────────────
//   useEffect(() => {
//     log('status-changed-reset-route', {
//       newStatus:    rideStatus,
//       prevRouteRef: routeStatusRef.current,
...
//   // ── Ride-status side-effects (redirects) ──────────────────────────────────
//   // IMPORTANT: router and stopTracking are intentionally omitted from deps.
//   // Including them causes this effect to re-fire on every render (because
//   // they are new references each time), which stacks dozens of setTimeout
//   // redirect calls. We read stopTracking via stopTrackingRef instead, and
//   // router.push is stable in Next.js App Router so reading it from closure
//   // at effect-call time is safe. hasRedirectedRef prevents any duplicate
//   // redirects that might slip through regardless.
//   useEffect(() => {
//     if (!currentRide) return;
...
//     // Block duplicate redirects no matter how many times this effect fires.
//     if (hasRedirectedRef.current) {
//       log('status-effect-redirect-blocked-by-guard', { status });
//       return;
//     }
...
//     }
//   // eslint-disable-next-line react-hooks/exhaustive-deps
//   }, [currentRide?.rideStatus, rideId]);
//   // ^ router and stopTracking deliberately excluded — see comment above.
...
//         {/* Floating ETA card — accepted (driver → pickup) */}
//         {rideStatus === 'accepted' && (
//           <motion.div
//             initial={{ y: -80, opacity: 0 }} animate={{ y: 0, opacity: 1 }}
//             transition={{ delay: 0.15, type: 'spring', stiffness: 260, damping: 22 }}
...
const ssKey = (rideId) => `tracking-redirected-${rideId}`;
const hasSessionRedirected  = (id) => { try { return !!id && sessionStorage.getItem(ssKey(id)) === 'true'; } catch { return false; } };
const markSessionRedirected = (id) => { try { if (id) sessionStorage.setItem(ssKey(id), 'true'); } catch {} };
...
function LicensePlate({ plate, size = 'large' }) {
  ...
  return (
    <Box sx={{
      ...
      borderRadius: isLarge ? 2.5 : 1.5, overflow: 'hidden',
      background: 'linear-gradient(180deg, #FAFAFA 0%, #F0F0F0 100%)',
      border: '2.5px solid #111',
      boxShadow: isLarge
        ? '0 6px 24px rgba(0,0,0,0.45), 0 2px 8px rgba(0,0,0,0.2), inset 0 1px 0 rgba(255,255,255,0.9)'
      ...
      <Box sx={{ px: isLarge ? 2 : 1.25, pb: isLarge ? 0.75 : 0.4, pt: isLarge ? 0.5 : 0.3 }}>
        <Typography sx={{
          fontFamily: '"Courier New", "Lucida Console", "Liberation Mono", monospace',
          fontWeight: 900, fontSize: isLarge ? '1.5rem' : '0.9rem',
          letterSpacing: isLarge ? 4 : 2.5, color: '#0A0A0A', lineHeight: 1,
          textTransform: 'uppercase', textShadow: '0 1px 0 rgba(255,255,255,0.6)',
...
// ── Inner component — always client-side, never SSR ──────────────────────────
function TrackingPageInner() {
  ...
  // FIX 1: In-flight guard — prevents overlapping poll requests on slow networks.
  // On 3G a request can take 10-15s while the interval fires every 5s, causing
  // 3+ concurrent requests that all resolve together and fire 12-18 setState
  // calls in a burst, cascading into re-renders and effect restarts.
  const pollInFlightRef = useRef(false);

  // FIX 2: rideStatus ref — keeps poll reading the latest status without
  // making rideStatus a dep of the polling effect. When rideStatus is in the
  // dep array, any re-render that momentarily changes it (even to the same
  // value) tears down and recreates the interval, restarting the route state
  // and compounding the re-render cascade from Fix 1.
  ...
  // Keep rideStatusRef in sync with the latest derived value every render.
  // This is intentionally outside a useEffect so the ref is always current
  // before any effect that runs in the same render cycle reads it.
  rideStatusRef.current = rideStatus;

  log('render', {
    rideId, rideStatus, currentRideId: currentRide?.id,
    sourceOfRide: activeRide ? 'activeRide' : 'ride',
    loading, routeReady, sessionGuarded: hasSessionRedirected(rideId),
    driverLocation: driverLocation ? `${driverLocation.lat},${driverLocation.lng}` : null,
  });

  // ── Mount / unmount ─────────────────────────────────────────────────────────
  // FIX 3: stopTracking moved here so it only fires on true component unmount,
  // not on every dep-change restart of the startTracking effect. Previously,
  // the startTracking cleanup called stopTracking on every restart, which could
  // clear activeRide in useRide → brief "no ride" state → more re-renders.
  useEffect(() => {
    mountedRef.current = true;
    log('mounted', { rideId, sessionGuarded: hasSessionRedirected(rideId) });
    return () => {
      mountedRef.current = false;
      clearInterval(locPollRef.current);
  ...
  // ── Driver location polling ─────────────────────────────────────────────────
  // FIX 2 (continued): deps reduced to [rideId] only.
  //   - rideStatus  → read via rideStatusRef.current inside poll (never a dep)
  //   - currentRide?.driver?.id → removed; this value can be undefined during
  //     loading states, causing the interval to restart and briefly clearing
  //     route/ETA state on every slow-network response.
  // FIX 1 (continued): poll body is guarded by pollInFlightRef so only one
  //   request is in-flight at a time regardless of how fast the interval fires.
  useEffect(() => {
    // Reset in-flight guard whenever this effect (re-)runs so a stale true
    // from a previous effect instance never blocks the new one.
    pollInFlightRef.current = false;

    const status = rideStatusRef.current;
    if (!rideId || !currentRide || ['completed', 'cancelled'].includes(status)) {
      log('poll-skipped', { rideId, rideStatus: status, hasCurrentRide: !!currentRide });
      return;
    }
    log('poll-started', { rideId, rideStatus: status });

    const poll = async () => {
      // Skip if a request is already in-flight or component has unmounted.
      if (pollInFlightRef.current || !mountedRef.current) return;
      pollInFlightRef.current = true;

      try {
        const result = await ridesAPI.trackRide(rideId);
        // Read the latest status from the ref — never close over the stale
        // value from when the effect was created.
        const currentStatus = rideStatusRef.current;

        log('poll-response', {
          success: result.success, backendStatus: result.data?.ride?.rideStatus,
          hasDriverLoc: !!result.data?.driver?.currentLocation,
        });
        if (!result.success || !result.data || !mountedRef.current) return;

        const td  = result.data;
        const loc = normalizeCoords(td.driver?.currentLocation);
        ...
        if (loc) {
          ...
          if (currentStatus === 'passenger_onboard' && routeStatusRef.current !== 'passenger_onboard') {
            routeStatusRef.current = 'passenger_onboard';
            const pickup  = normalizeCoords(td.ride?.pickupLocation  ?? currentRide.pickupLocation);
            const dropoff = normalizeCoords(td.ride?.dropoffLocation ?? currentRide.dropoffLocation);
            if (pickup && dropoff) {
    ...
    return () => {
      clearInterval(locPollRef.current);
      // Release guard on cleanup so a fresh effect instance starts unblocked.
      pollInFlightRef.current = false;
    };
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [rideId]); // rideStatus deliberately excluded — read via rideStatusRef inside poll
  ...
  useEffect(() => {
    const unsub = rnOn('DRIVER_LOCATION_UPDATED', (payload) => {
      const raw = payload.location ?? payload;
      if (!raw) return;
      const loc = normalizeCoords({ lat: raw.lat ?? raw.latitude, lng: raw.lng ?? raw.longitude, accuracy: raw.accuracy, heading: raw.heading, speed: raw.speed });
      if (!loc) return;
      setDriverLocation(loc);
      mapControlsRef.current?.updateDriverLocation(loc);
      if (payload.eta      != null) setEta(payload.eta);
  ...
  // ── startTracking ───────────────────────────────────────────────────────────
  // FIX 3 (continued): stopTracking removed from this cleanup. It now lives
  // exclusively in the mount/unmount effect above so it only fires once on
  // true unmount. Calling it here also fired on dep-change restarts (e.g. if
  // rideId ever changed), which could clear ride state in useRide mid-session.
  useEffect(() => {
    if (!rideId || !currentRide) return;
    const cleanup = startTracking(rideId, () => {});
  ...
  useEffect(() => {
    ...
    if (currentRide.rideStatus === 'accepted') {
      if (currentRide.currentDriverLocation && currentRide.pickupLocation) {
        const dl = normalizeCoords(currentRide.currentDriverLocation);
        const pk = normalizeCoords(currentRide.pickupLocation);
        if (dl && pk) { const km = calculateDistance(dl, pk); setDistance(km); setEta(estimateDuration(km)); }
      }
    } else {
    ...
    const fetchPic = async () => { setDriverProfilePic(await loadRideDriverProfilePicUrl(currentRide?.driver?.id)); };
    fetchPic();
  }, [currentRide?.id]); // eslint-disable-line
  ...
  useEffect(() => {
    ...
    const status       = currentRide.rideStatus;
    const alreadyOnPay = typeof window !== 'undefined' && window.location.pathname.includes(`/trips/${id}/pay`);
    const guarded      = hasSessionRedirected(id);

    log('status-effect-fired', { status, rideId: id, alreadyOnPay, guarded });

    if (guarded) { log('status-effect-blocked-by-session-guard', { status }); return; }

    if (status === 'awaiting_payment') {
      if (!alreadyOnPay) {
        markSessionRedirected(id);
        setSnackbar({ open: true, message: 'Payment required. Redirecting…', severity: 'warning' });
        setTimeout(() => router.push(`/trips/${id}/pay`), 1200);
      }
      return;
  ...
  const markers = useMemo(() => {
    if (!currentRide) return [];
    const pl = normalizeCoords(currentRide.pickupLocation);
    const dl = normalizeCoords(currentRide.dropoffLocation);
    if (rideStatus === 'accepted') return pl ? [{ id: 'pickup', position: pl, type: 'pickup' }] : [];
    if (rideStatus === 'passenger_onboard' || rideStatus === 'awaiting_payment') {
      return [pl && { id: 'pickup', position: pl, type: 'pickup' }, dl && { id: 'dropoff', position: dl, type: 'dropoff' }].filter(Boolean);
    }
    return [];
  }, [currentRide, rideStatus]);
  ...
  const handleCancelConfirmYes = async () => {
    ...
    try {
      const res = await apiClient.get('/cancellation-reasons?filters[isActive][$eq]=true&filters[applicableFor][$in][0]=rider&filters[applicableFor][$in][1]=both&sort=displayOrder:asc');
      if (mountedRef.current) setCancelReasons(res?.data ?? res ?? []);
    } catch { if (mountedRef.current) setCancelReasons([]); }
    finally { if (mountedRef.current) setCancelReasonsLoading(false); }
  };

  const handleCancelWithReason = async () => {
    const isOther = selectedCancelReason === '__other__';
    const reason  = isOther ? (cancelOtherText.trim() || 'Other') : (cancelReasons.find(r => String(r.id) === String(selectedCancelReason))?.reason || selectedCancelReason);
    if (!reason) return;
    setCanceling(true);
    try {
      const result = await cancelRide(rideId, reason, reason);
  ...
  const handleCloseCancelReasons = () => {
    if (canceling) return;
    setShowCancelReasons(false); setCancelReasons([]); setSelectedCancelReason(''); setCancelOtherText('');
  };

  const canSubmitCancel = selectedCancelReason && (selectedCancelReason !== '__other__' || cancelOtherText.trim().length > 0);
  ...
  return (
    <Box sx={{ height: '100vh', width: '100%', position: 'relative', overflow: 'hidden' }}>
      ...
      <Box sx={{ position: 'absolute', inset: 0, zIndex: 1 }}>
        <MapIframe
          center={driverLocation || normalizeCoords(currentRide?.pickupLocation) || { lat: -15.4167, lng: 28.2833 }}
          zoom={15} markers={markers}
          pickupLocation={routePickup} dropoffLocation={routeDropoff}
          showRoute={routeReady} onMapLoad={handleMapLoad}
        />
      </Box>

      {/* Status Bar */}
      <motion.div initial={{ y: -60, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ type: 'spring', stiffness: 260, damping: 24 }} style={{ position: 'absolute', top: 0, left: 0, right: 0, zIndex: 10 }}>
        <Box sx={{ background: statusGradient, color: 'white', py: 1.5, px: 2, display: 'flex', alignItems: 'center', gap: 1.5, boxShadow: '0 4px 20px rgba(0,0,0,0.25)' }}>
          <Box sx={{ width: 38, height: 38, borderRadius: '50%', bgcolor: 'rgba(255,255,255,0.18)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.3rem', flexShrink: 0 }}>{statusConfig.icon}</Box>
          <Box sx={{ flex: 1 }}>
            <Typography variant="body2" sx={{ fontWeight: 800, lineHeight: 1.2 }}>{statusConfig.title}</Typography>
            <Typography variant="caption" sx={{ opacity: 0.85 }}>{statusConfig.description}</Typography>
          </Box>
      ...
      {/* Floating ETA card — accepted */}
      {rideStatus === 'accepted' && (
        <motion.div initial={{ y: -80, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.15, type: 'spring', stiffness: 260, damping: 22 }} style={{ position: 'absolute', top: 70, left: '50%', transform: 'translateX(-50%)', zIndex: 9 }}>
          <Paper elevation={8} sx={{ px: 3, py: 1.5, borderRadius: 4, display: 'flex', alignItems: 'center', gap: 2.5, background: isDark ? 'rgba(30,30,30,0.95)' : 'rgba(255,255,255,0.96)', backdropFilter: 'blur(20px)', border: '1px solid', borderColor: isDark ? 'rgba(16,185,129,0.3)' : 'rgba(16,185,129,0.35)', boxShadow: isDark ? '0 8px 32px rgba(0,0,0,0.5)' : '0 8px 32px rgba(16,185,129,0.18)' }}>
            <Box sx={{ textAlign: 'center' }}>
              <Typography variant="caption" sx={{ color: 'text.disabled', fontWeight: 700, fontSize: '0.6rem', letterSpacing: 0.6, textTransform: 'uppercase' }}>ETA to Pickup</Typography>
              <Typography variant="h6" sx={{ fontWeight: 800, color: '#10B981', lineHeight: 1.1 }}>{eta != null ? formatETA(eta) : '—'}</Typography>
            </Box>
            <Divider orientation="vertical" flexItem />
            <Box sx={{ textAlign: 'center' }}>
              <Typography variant="caption" sx={{ color: 'text.disabled', fontWeight: 700, fontSize: '0.6rem', letterSpacing: 0.6, textTransform: 'uppercase' }}>Distance</Typography>
              <Typography variant="h6" sx={{ fontWeight: 800, lineHeight: 1.1 }}>{distance != null ? `${typeof distance === 'number' ? distance.toFixed(1) : distance} km` : '—'}</Typography>
            </Box>
          </Paper>
        </motion.div>
      )}
      ...
      {/* In-transit ETA card */}
      {rideStatus === 'passenger_onboard' && eta != null && (
        <motion.div initial={{ y: -80, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.1, type: 'spring', stiffness: 260, damping: 22 }} style={{ position: 'absolute', top: 70, left: '50%', transform: 'translateX(-50%)', zIndex: 9 }}>
          <Paper elevation={8} sx={{ px: 3, py: 1.5, borderRadius: 4, display: 'flex', alignItems: 'center', gap: 2.5, background: isDark ? 'rgba(30,30,30,0.95)' : 'rgba(255,255,255,0.96)', backdropFilter: 'blur(20px)', border: '1px solid', borderColor: isDark ? 'rgba(255,193,7,0.3)' : 'rgba(255,193,7,0.4)', boxShadow: isDark ? '0 8px 32px rgba(0,0,0,0.5)' : '0 8px 32px rgba(255,140,0,0.2)' }}>
            <Box sx={{ textAlign: 'center' }}>
              <Typography variant="caption" sx={{ color: 'text.disabled', fontWeight: 700, fontSize: '0.6rem', letterSpacing: 0.6, textTransform: 'uppercase' }}>Arrival</Typography>
              <Typography variant="h6" sx={{ fontWeight: 800, color: '#FF8C00', lineHeight: 1.1 }}>{formatETA(eta)}</Typography>
            </Box>
            {distance != null && (<><Divider orientation="vertical" flexItem /><Box sx={{ textAlign: 'center' }}><Typography variant="caption" sx={{ color: 'text.disabled', fontWeight: 700, fontSize: '0.6rem', letterSpacing: 0.6, textTransform: 'uppercase' }}>Remaining</Typography><Typography variant="h6" sx={{ fontWeight: 800, lineHeight: 1.1 }}>{typeof distance === 'number' ? `${distance.toFixed(1)} km` : `${distance} km`}</Typography></Box></>)}
          </Paper>
        </motion.div>
      )}
      ...
      {rideStatus === 'awaiting_payment' && (
        <motion.div initial={{ y: -80, opacity: 0 }} animate={{ y: 0, opacity: 1 }} style={{ position: 'absolute', top: 70, left: 16, right: 16, zIndex: 9 }}>
          <Alert severity="warning" sx={{ borderRadius: 3, fontWeight: 600, boxShadow: '0 8px 24px rgba(255,152,0,0.3)' }} action={<Button size="small" variant="contained" color="warning" onClick={() => router.push(`/trips/${currentRide.id ?? rideId}/pay`)} sx={{ fontWeight: 700, borderRadius: 2 }}>Pay Now</Button>}>
            Payment required — please pay the driver
          </Alert>
        </motion.div>
      )}
      ...
      {/* Centre on driver */}
      {driverLocation && (
        <IconButton onClick={() => mapControlsRef.current?.animateToLocation(driverLocation, 16)} sx={{ position: 'absolute', right: 16, top: 80, zIndex: 5, bgcolor: isDark ? 'rgba(30,30,30,0.9)' : 'rgba(255,255,255,0.95)', backdropFilter: 'blur(10px)', boxShadow: '0 4px 16px rgba(0,0,0,0.2)', border: '1px solid', borderColor: isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.08)', '&:hover': { boxShadow: '0 6px 20px rgba(0,0,0,0.3)', transform: 'scale(1.06)' }, transition: 'all 0.18s ease' }}>
          <MyLocationIcon color="primary" />
        </IconButton>
      )}
      ...
      <SwipeableBottomSheet open initialHeight={rideStatus === 'arrived' ? 500 : 420} maxHeight="85%" minHeight={200} draggable>
        ...
        <Box sx={{ px: 3, pb: 3, flex: 1, display: 'flex', flexDirection: 'column', overflowY: 'auto', '&::-webkit-scrollbar': { display: 'none' }, scrollbarWidth: 'none' }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', mb: 2.5, flexShrink: 0 }}>
            <Box sx={{ position: 'relative', mr: 2 }}>
              <Avatar
                src={driverProfilePic ? process.env.NEXT_PUBLIC_UPLOAD_PUBLIC_API_URL + getImageUrl(driverProfilePic, 'thumbnail') : undefined}
                sx={{ width: 64, height: 64, border: '3px solid', borderColor: 'primary.main', boxShadow: '0 4px 16px rgba(255,193,7,0.3)' }}
              >
                {driver?.firstName?.[0]}
              </Avatar>
            ...
            <Box sx={{ flex: 1 }}>
              ...
              <Chip size="small" label={rideStatus === 'awaiting_payment' ? 'Payment Required' : rideStatus === 'arrived' ? 'Arrived' : rideStatus === 'passenger_onboard' ? 'In Transit' : 'On the way'} color={rideStatus === 'awaiting_payment' ? 'warning' : rideStatus === 'arrived' ? 'success' : 'primary'} sx={{ fontWeight: 700, height: 22, fontSize: '0.7rem' }} />
            </Box>
            <Box sx={{ display: 'flex', gap: 1, flexShrink: 0 }}>
              <IconButton onClick={() => { const raw = /^\+?\d[\d\s\-]{6,}$/.test(driver?.username) ? driver.username : driver?.phoneNumber; const digits = (raw || '').replace(/\D/g, ''); window.location.href = `tel:${digits.length > 10 ? digits.slice(-10) : digits}`; }} sx={{ bgcolor: 'success.main', color: '#fff', width: 40, height: 40, '&:hover': { bgcolor: 'success.dark', transform: 'scale(1.08)' }, transition: 'all 0.16s ease' }}>
                <PhoneIcon sx={{ fontSize: 18 }} />
              </IconButton>
              <IconButton onClick={() => { const raw = /^\+?\d[\d\s\-]{6,}$/.test(driver?.username) ? driver.username : driver?.phoneNumber; window.open(`https://wa.me/${(raw || '').replace(/\D/g, '')}`, '_blank'); }} sx={{ background: 'linear-gradient(135deg, #FFC107 0%, #FF8C00 100%)', color: '#000', width: 40, height: 40, '&:hover': { transform: 'scale(1.08)' }, transition: 'all 0.16s ease' }}>
                <MessageIcon sx={{ fontSize: 18 }} />
              </IconButton>
            </Box>
          </Box>
              ...
              <motion.div key="en-route" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }}>
                <Box sx={{ p: 2, borderRadius: 3, background: 'linear-gradient(135deg,rgba(16,185,129,0.12) 0%,rgba(5,150,105,0.06) 100%)', border: '1px solid rgba(16,185,129,0.25)', mb: 2, flexShrink: 0 }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 0.8 }}>
                    ...
                    {eta != null && (
                      <Box sx={{ flex: 1, display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: 1.5 }}>
                        {distance != null && <Typography variant="caption" sx={{ fontWeight: 700, color: 'text.secondary' }}>{typeof distance === 'number' ? `${distance.toFixed(1)} km` : `${distance} km`}</Typography>}
                        <Typography variant="caption" sx={{ fontWeight: 800, color: '#10B981' }}>{formatETA(eta)}</Typography>
                      </Box>
                    )}
                  </Box>
                  <Typography variant="body2" sx={{ color: 'text.secondary', mb: eta != null ? 1.5 : 0 }}>Your driver is heading to your pickup location.</Typography>
                  {eta != null && <LinearProgress variant="indeterminate" sx={{ borderRadius: 2, height: 4, bgcolor: 'rgba(16,185,129,0.15)', '& .MuiLinearProgress-bar': { background: 'linear-gradient(90deg,#10B981,#059669)' } }} />}
                </Box>
              </motion.div>
            )}
          </AnimatePresence>
            ...
            {rideStatus === 'passenger_onboard' && (
              <motion.div key="in-progress" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }}>
                <Box sx={{ p: 2, borderRadius: 3, background: 'linear-gradient(135deg,rgba(255,193,7,0.15) 0%,rgba(255,140,0,0.08) 100%)', border: '1px solid rgba(255,193,7,0.25)', mb: 2, flexShrink: 0 }}>
                  ...
                  <Typography variant="body2" sx={{ color: 'text.secondary', mb: eta ? 1.5 : 0 }}>Sit back and relax. You'll arrive soon!</Typography>
                  {eta && (<Box><Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}><Typography variant="caption" sx={{ fontWeight: 600 }}>Estimated Arrival</Typography><Typography variant="caption" sx={{ fontWeight: 800, color: '#FF8C00' }}>{formatETA(eta)}</Typography></Box><LinearProgress variant="indeterminate" sx={{ borderRadius: 2, height: 5, bgcolor: 'rgba(255,193,7,0.15)', '& .MuiLinearProgress-bar': { background: 'linear-gradient(90deg,#FFC107,#FF8C00)' } }} /></Box>)}
                </Box>
              </motion.div>
            )}
          </AnimatePresence>
          ...
          <Box sx={{ p: 2, borderRadius: 3, border: '1px solid', borderColor: isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.07)', bgcolor: isDark ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.02)', mb: 2, flexShrink: 0 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
              <CarIcon sx={{ color: 'text.disabled', fontSize: 18 }} />
              <Typography variant="caption" sx={{ fontWeight: 700, fontSize: '0.68rem', letterSpacing: 0.5, textTransform: 'uppercase', color: 'text.disabled' }}>Your Vehicle</Typography>
            </Box>
            {vehicle?.numberPlate && <Box sx={{ display: 'flex', justifyContent: 'center', mb: 2 }}><LicensePlate plate={vehicle.numberPlate} size="large" /></Box>}
            <Box sx={{ display: 'grid', gridTemplateColumns: vehicle?.color ? '1fr 1fr' : '1fr', gap: 1.5, mb: vehicle?.color && colorInfo ? 1.5 : 0 }}>
              <Box>
                <Typography variant="caption" sx={{ color: 'text.disabled', fontSize: '0.65rem', display: 'block', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.4, mb: 0.25 }}>Model</Typography>
                <Typography variant="body2" sx={{ fontWeight: 700 }}>{vehicle?.make} {vehicle?.model}</Typography>
              </Box>
              {vehicle?.color && (
                <Box>
                  <Typography variant="caption" sx={{ color: 'text.disabled', fontSize: '0.65rem', display: 'block', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.4, mb: 0.25 }}>Colour</Typography>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75 }}>
                    <Box sx={{ width: 20, height: 20, borderRadius: '50%', flexShrink: 0, bgcolor: colorInfo?.body ?? '#888', border: `2.5px solid ${colorInfo?.outline ?? '#666'}`, boxShadow: `0 2px 6px rgba(0,0,0,0.25), inset 0 1px 2px rgba(255,255,255,0.5)` }} />
                    <Typography variant="body2" sx={{ fontWeight: 700, textTransform: 'capitalize' }}>{colorInfo?.label ?? vehicle.color}</Typography>
                  </Box>
                </Box>
              )}
            </Box>
            {vehicle?.color && <Box sx={{ mt: 1.5, p: 1.5, borderRadius: 2.5, bgcolor: isDark ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.025)', border: '1px solid', borderColor: isDark ? 'rgba(255,255,255,0.07)' : 'rgba(0,0,0,0.06)', display: 'flex', justifyContent: 'center' }}><VanIconSmall colorKey={vehicle.color} size={140} /></Box>}
            {driverLocation?.speed && <Box sx={{ mt: 1.25, display: 'flex', alignItems: 'center', gap: 0.75 }}><SpeedIcon sx={{ fontSize: 15, color: '#FF8C00' }} /><Typography variant="caption" sx={{ fontWeight: 700, color: '#FF8C00' }}>{Math.round(driverLocation.speed)} km/h</Typography></Box>}
          </Box>
            ...
            <AnimatePresence mode="wait">
              {rideStatus === 'arrived' && (
                <motion.div key="confirm-pickup" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }}>
                  ...
                  <Button fullWidth variant="contained" size="large" onClick={handleConfirmPickup} disabled={loading} sx={{ height: 56, fontWeight: 800, fontSize: '1rem', borderRadius: 3.5, mb: 1, background: 'linear-gradient(135deg,#4CAF50 0%,#2E7D32 100%)', boxShadow: '0 6px 20px rgba(76,175,80,0.4)', '&:hover': { transform: 'translateY(-1px)', boxShadow: '0 8px 24px rgba(76,175,80,0.5)' }, transition: 'all 0.2s ease' }}>
                    {loading ? <CircularProgress size={24} color="inherit" /> : "✓ I'm in the car — Start Trip"}
                  </Button>
                </motion.div>
              )}
              {rideStatus === 'awaiting_payment' && (
                <motion.div key="awaiting-payment" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
                  <Button fullWidth variant="contained" color="warning" size="large" onClick={() => router.push(`/trips/${currentRide.id ?? rideId}/pay`)} sx={{ height: 56, fontWeight: 800, borderRadius: 3.5, mb: 1 }}>💳 Pay Now</Button>
                </motion.div>
              )}
            </AnimatePresence>
            {rideStatus !== 'passenger_onboard' && rideStatus !== 'awaiting_payment' && (
              <Button fullWidth variant="outlined" color="error" onClick={() => setShowCancelDialog(true)} disabled={canceling} sx={{ height: 48, fontWeight: 700, borderRadius: 3.5, borderWidth: 1.5, '&:hover': { borderWidth: 1.5 } }}>
                Cancel Ride
              </Button>
            )}
          </Box>
      ...
      <Dialog open={showCancelDialog} onClose={() => setShowCancelDialog(false)} PaperProps={{ sx: { borderRadius: 4, maxWidth: 400, border: '1px solid', borderColor: isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.08)' } }}>
        <DialogTitle sx={{ fontWeight: 700 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
            <Box sx={{ width:40, height:40, borderRadius:2, flexShrink:0, background:'linear-gradient(135deg,#ef4444 0%,#dc2626 100%)', display:'flex', alignItems:'center', justifyContent:'center', boxShadow:'0 4px 12px rgba(239,68,68,0.35)' }}><WarningIcon sx={{ fontSize:22, color:'#fff' }} /></Box>
            Cancel Ride?
          </Box>
          <IconButton onClick={() => setShowCancelDialog(false)} sx={{ position: 'absolute', right: 8, top: 8 }}><CloseIcon /></IconButton>
        </DialogTitle>
      ...
      <Dialog open={showCancelReasons} onClose={handleCloseCancelReasons} PaperProps={{ sx: { borderRadius: 4, maxWidth: 400, mx: 2, width: '100%', border: '1px solid', borderColor: isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.08)' } }}>
        <DialogTitle sx={{ fontWeight: 700, pb: 0.5 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
            <Box sx={{ width:40, height:40, borderRadius:2, flexShrink:0, background:'linear-gradient(135deg,#ef4444 0%,#dc2626 100%)', display:'flex', alignItems:'center', justifyContent:'center', boxShadow:'0 4px 12px rgba(239,68,68,0.35)' }}><CancelIcon sx={{ fontSize:22, color:'#fff' }} /></Box>
            Reason for Cancellation
          </Box>
        </DialogTitle>
        <DialogContent sx={{ pt: 2, pb: 1 }}>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 2.5, lineHeight: 1.6 }}>Please let us know why you're cancelling so we can improve your experience.</Typography>
          {cancelReasonsLoading ? (
            <Box sx={{ display: 'flex', justifyContent: 'center', py: 3 }}><CircularProgress size={28} /></Box>
          ) : (
            <FormControl fullWidth size="medium">
              <InputLabel id="cancel-reason-label">Select a reason</InputLabel>
              <Select labelId="cancel-reason-label" value={selectedCancelReason} label="Select a reason" onChange={(e) => { setSelectedCancelReason(e.target.value); if (e.target.value !== '__other__') setCancelOtherText(''); }} sx={{ borderRadius: 2 }}>
                {cancelReasons.map((r) => (<MenuItem key={r.id} value={String(r.id)}>{r.reason}</MenuItem>))}
                <MenuItem value="__other__">Other</MenuItem>
              </Select>
            </FormControl>
          )}
        ...
        <DialogActions sx={{ px: 2.5, pb: 1.5, gap: 1, flexDirection: 'column' }}>
          ...
          <Divider sx={{ width:'100%', my:0.5 }}><Typography variant="caption" color="text.disabled" sx={{ px:1 }}>or</Typography></Divider>
          <Button fullWidth variant="text" size="medium" disabled={canceling} onClick={handleCancelRide} sx={{ height:44, borderRadius:2.5, fontWeight:600, color:'text.secondary', textTransform:'none', fontSize:'0.875rem', '&:hover':{ bgcolor:'action.hover' } }}>
            Cancel without reason
          </Button>
        </DialogActions>
