...
// // // // Lives outside the component so it survives remounts. Keyed by rideId.
// // // // Prevents duplicate redirects even when the component mounts multiple times
// // // // before the setTimeout fires (e.g. when useRide auto-navigates back to tracking).
// // // const _redirectedRideIds = new Set();
...
// // //   const locPollRef = useRef(null);
// // //   const mountedRef = useRef(true);
// // //   // FIX: timer ref so pending redirects can be cancelled on unmount
// // //   const redirectTimerRef = useRef(null);
// // //   // FIX: keep stopTracking callable from effects without making it a dep
// // //   const stopTrackingRef = useRef(stopTracking);
// // //   useEffect(() => { stopTrackingRef.current = stopTracking; }, [stopTracking]);
...
// // //   // ── FIX: single mount/unmount effect ──────────────────────────────────────
// // //   // stopLocationTracking is called exactly once here (empty deps), NOT in a
// // //   // [router]-dep effect. [router] can change reference on every render in some
// // //   // Next.js builds, causing the native bridge to be hammered continuously,
// // //   // which contributes to the reload cycle.
// // //   useEffect(() => {
// // //     mountedRef.current = true;
// // //     log('mounted', { rideId });

// // //     // Stop any lingering GPS tracking from the previous screen immediately
// // //     // on mount — before any re-renders can trigger.
...
// // //       // FIX: cancel any pending redirect timer so stale timers from a
// // //       // previous mount don't fire after this instance has already unmounted.
...
// // //       log('unmounted', { rideId });
// // //     };
// // //   }, []); // eslint-disable-line — intentionally empty: run once on mount only

// // //   // ── Reset route state whenever ride status changes ────────────────────────
// // //   useEffect(() => {
// // //     log('status-changed-reset-route', {
// // //       newStatus: rideStatus,
// // //       prevRouteRef: routeStatusRef.current,
...
// // //           // ETA/distance for the correct leg
// // //           if (rideStatus === 'accepted') {
// // //             const pickup = normalizeCoords(td.ride?.pickupLocation ?? currentRide.pickupLocation);
// // //             if (pickup) {
// // //               const km = calculateDistance(loc, pickup);
...
// // //           // Route: onboard — pickup → dropoff (set once)
// // //           if (rideStatus === 'passenger_onboard' && routeStatusRef.current !== 'passenger_onboard') {
// // //             routeStatusRef.current = 'passenger_onboard';
// // //             const pickup = normalizeCoords(td.ride?.pickupLocation ?? currentRide.pickupLocation);
// // //             const dropoff = normalizeCoords(td.ride?.dropoffLocation ?? currentRide.dropoffLocation);
// // //             if (pickup && dropoff) {
// // //               log('route-set-onboard', { pickup: coordKey(pickup), dropoff: coordKey(dropoff) });
// // //               lastRouteKeyRef.current = `${coordKey(pickup)}->${coordKey(dropoff)}`;
// // //               setRoutePickup(pickup);
// // //               setRouteDropoff(dropoff);
// // //               setRouteReady(true);
...
// // //   // ── Seed initial data from currentRide ───────────────────────────────────
// // //   useEffect(() => {
// // //     if (!currentRide) return;
// // //     log('seed-initial-data', {
// // //       currentRideId: currentRide.id,
...
// // //     if (currentRide.rideStatus === 'accepted') {
// // //       // Driver → pickup leg: seed from live driver location
// // //       if (currentRide.currentDriverLocation && currentRide.pickupLocation) {
// // //         const driverLoc = normalizeCoords(currentRide.currentDriverLocation);
// // //         const pickup = normalizeCoords(currentRide.pickupLocation);
// // //         if (driverLoc && pickup) {
...
// // //       }
// // //     } else {
// // //       // All other statuses: pickup → dropoff leg
// // //       if (currentRide.estimatedDuration !== undefined) {
// // //         setEta(currentRide.estimatedDuration);
// // //       } else if (currentRide.pickupLocation && currentRide.dropoffLocation) {
// // //         setEta(estimateDuration(calculateDistance(currentRide.pickupLocation, currentRide.dropoffLocation)));
...
// // //   //   1. Guard is _redirectedRideIds (module-level Set) — survives remounts.
// // //   //      useRef(false) reset to false on every remount, allowing duplicate
// // //   //      timers to accumulate and fire all at once.
// // //   //   2. router.replace() removes /tracking from history so useRide's
// // //   //      auto-navigate-back-to-tracking doesn't remount this page again.
// // //   //   3. redirectTimerRef holds any setTimeout so the unmount cleanup can
// // //   //      cancel timers that haven't fired yet.
// // //   //   4. router and stopTracking are intentionally excluded from deps —
// // //   //      both are read via stable refs/closure. Including them causes the
// // //   //      effect to re-fire on every render in some Next.js builds.
// // //   useEffect(() => {
// // //     if (!currentRide) return;
...
// // //     const currentPath = typeof window !== 'undefined' ? window.location.pathname : 'ssr';
// // //     const alreadyOnPay = typeof window !== 'undefined' &&
// // //       window.location.pathname.includes(`/trips/${id}/pay`);
...
// // //       currentPath,
// // //       alreadyOnPay,
// // //       redirectGuarded: _redirectedRideIds.has(guardKey),
// // //       sourceOfRide: activeRide ? 'activeRide' : 'ride',
// // //       hookLoading: loading,
// // //     });

// // //     // Module-level guard: blocks all duplicate redirects across remounts.
// // //     if (_redirectedRideIds.has(guardKey)) {
// // //       log('status-effect-redirect-blocked', { status, guardKey });
// // //       return;
// // //     }

// // //     if (status === 'awaiting_payment') {
// // //       if (!alreadyOnPay) {
// // //         _redirectedRideIds.add(guardKey);
// // //         log('redirect-to-pay', { rideId: id, from: currentPath });
// // //         setSnackbar({ open: true, message: 'Payment required. Redirecting…', severity: 'warning' });
// // //         // replace() so /tracking is gone from history
// // //         redirectTimerRef.current = setTimeout(() => router.replace(`/trips/${id}/pay`), 1200);
// // //       } else {
// // //         log('redirect-to-pay-skipped-already-there', { rideId: id, currentPath });
// // //       }
// // //       return;
...
// // //       log('redirect-trip-summary', { rideId: id, from: currentPath });
// // //       stopTrackingRef.current?.();
// // //       router.replace(`/trip-summary?rideId=${id}`);
// // //     } else if (status === 'cancelled') {
// // //       _redirectedRideIds.add(guardKey);
// // //       log('redirect-home-cancelled', { rideId: id, from: currentPath });
// // //       stopTrackingRef.current?.();
...
// // //       setSnackbar({
// // //         open: true,
// // //         message: result.success ? 'Trip started!' : (result.error || 'Failed'),
...
// // //         '/cancellation-reasons?filters[isActive][$eq]=true&filters[applicableFor][$in][0]=rider&filters[applicableFor][$in][1]=both&sort=displayOrder:asc'
// // //       );
// // //       const reasons = res?.data ?? res ?? [];
// // //       log('cancel-reasons-loaded', { count: reasons.length });
// // //       if (mountedRef.current) setCancelReasons(reasons);
...
// // //         {/* Floating ETA card — accepted (driver → pickup, green) */}
// // //         {rideStatus === 'accepted' && (
// // //           <motion.div
// // //             initial={{ y: -80, opacity: 0 }} animate={{ y: 0, opacity: 1 }}
// // //             transition={{ delay: 0.15, type: 'spring', stiffness: 260, damping: 22 }}
...
// // //         {/* Floating ETA card — in transit (amber) */}
// // //         {rideStatus === 'passenger_onboard' && eta != null && (
// // //           <motion.div
// // //             initial={{ y: -80, opacity: 0 }} animate={{ y: 0, opacity: 1 }}
// // //             transition={{ delay: 0.1, type: 'spring', stiffness: 260, damping: 22 }}
...
// // //                         rideStatus === 'passenger_onboard' ? 'In Transit' : 'On the way'
// // //                   }
// // //                   color={rideStatus === 'awaiting_payment' ? 'warning' : rideStatus === 'arrived' ? 'success' : 'primary'}
// // //                   sx={{ fontWeight: 700, height: 22, fontSize: '0.7rem' }}
// // //                 />
...
// // //                       onClick={handleConfirmPickup} disabled={loading}
// // //                       sx={{ height: 56, fontWeight: 800, fontSize: '1rem', borderRadius: 3.5, mb: 1, background: 'linear-gradient(135deg,#4CAF50 0%,#2E7D32 100%)', boxShadow: '0 6px 20px rgba(76,175,80,0.4)', '&:hover': { transform: 'translateY(-1px)', boxShadow: '0 8px 24px rgba(76,175,80,0.5)' }, transition: 'all 0.2s ease' }}
// // //                     >
// // //                       {loading ? <CircularProgress size={24} color="inherit" /> : "✓ I'm in the car — Start Trip"}
// // //                     </Button>
...
// // //                   onClick={() => setShowCancelDialog(true)} disabled={canceling}
// // //                   sx={{ height: 48, fontWeight: 700, borderRadius: 3.5, borderWidth: 1.5, '&:hover': { borderWidth: 1.5 } }}
// // //                 >
// // //                   Cancel Ride
// // //                 </Button>
...
// // //               startIcon={canceling ? <CircularProgress size={16} color="inherit" /> : <CancelIcon />}
...
// // //               disabled={canceling} onClick={handleCancelRide}
...
// // // FIX: Minimum driver movement (km) required to trigger a route redraw.
// // // Prevents MapIframe from re-rendering on every 5-second poll tick while
// // // the driver is nearly stationary.
// // const ROUTE_UPDATE_THRESHOLD_KM = 0.05; // 50 metres
...
// //   // FIX: Single route object replaces three separate state values
// //   // (routePickup + routeDropoff + routeReady). One setState call = one
// //   // re-render instead of three consecutive re-renders per poll tick.
// //   const [route, setRoute] = useState({ pickup: null, dropoff: null, ready: false });

// //   // ── Refs ──────────────────────────────────────────────────────────────────
// //   const routeStatusRef = useRef(null);
// //   // FIX: replaces lastRouteKeyRef. Route now only redraws when the driver
// //   // has physically moved more than ROUTE_UPDATE_THRESHOLD_KM rather than
// //   // on every poll tick (driver always has a new coord = old key always differs).
// //   const lastRouteDriverLocRef = useRef(null);
// //   const mapControlsRef = useRef(null);
// //   const locPollRef = useRef(null);
// //   const mountedRef = useRef(true);
// //   const redirectTimerRef = useRef(null);
...
// //   // FIX: stable boolean for the polling effect dep array. Replaces
// //   // currentRide?.driver?.id which can flip between partial/full API responses
// //   // on slow connections and unnecessarily restart the 5-second interval.
// //   const hasCurrentRide = !!currentRide;

// //   // FIX: stable map center derived from pickup location only, recomputed
// //   // only when the ride changes (never when the driver moves). Previously
// //   // `center={driverLocation || ...}` changed the MapIframe center prop on
// //   // every location event, potentially re-initialising the map iframe.
// //   // Driver position is now pushed exclusively via the imperative API:
// //   // mapControlsRef.current.updateDriverLocation(loc)
// //   const initialMapCenter = useMemo(() => (
// //     normalizeCoords(currentRide?.pickupLocation) || { lat: -15.4167, lng: 28.2833 }
// //   ), [currentRide?.id]); // eslint-disable-line
...
// //       newStatus: rideStatus,
// //       prevRouteRef: routeStatusRef.current,
// //       hadRoute: route.ready,
// //     });
// //     routeStatusRef.current = null;
// //     lastRouteDriverLocRef.current = null;                    // FIX: reset threshold ref on status change
// //     setRoute({ pickup: null, dropoff: null, ready: false }); // FIX: single call replaces three
// //   }, [rideStatus]); // eslint-disable-line

// //   // ── Driver location polling ───────────────────────────────────────────────
// //   useEffect(() => {
// //     if (!rideId || !currentRide || ['completed', 'cancelled'].includes(rideStatus)) {
// //       log('poll-skipped', { rideId, rideStatus, hasCurrentRide });
// //       return;
// //     }
...
// //           // FIX: Route — accepted (driver → pickup).
// //           // Only redraws when driver has moved > ROUTE_UPDATE_THRESHOLD_KM.
// //           // Replaces the old lastRouteKeyRef key-string check which always
// //           // triggered on every tick because driver coords always change.
// //           if (rideStatus === 'accepted') {
// //             const pickup = normalizeCoords(td.ride?.pickupLocation ?? currentRide.pickupLocation);
// //             if (pickup) {
// //               const movedEnough = !lastRouteDriverLocRef.current ||
// //                 calculateDistance(lastRouteDriverLocRef.current, loc) > ROUTE_UPDATE_THRESHOLD_KM;

// //               if (movedEnough) {
// //                 log('route-set-accepted', {
// //                   driverLoc: coordKey(loc),
// //                   pickupLoc: coordKey(pickup),
// //                   prevDriverLoc: coordKey(lastRouteDriverLocRef.current),
// //                 });
// //                 lastRouteDriverLocRef.current = loc;
// //                 setRoute({ pickup: loc, dropoff: pickup, ready: true }); // FIX: single setState
// //               } else {
// //                 log('route-accepted-threshold-not-met', {
// //                   distanceMoved: calculateDistance(lastRouteDriverLocRef.current, loc).toFixed(4),
// //                   thresholdKm: ROUTE_UPDATE_THRESHOLD_KM,
// //                 });
// //               }
// //             } else {
// //               log('route-accepted-no-pickup', {
...
// //           // Route — onboard (pickup → dropoff). Set exactly once.
// //           if (rideStatus === 'passenger_onboard' && routeStatusRef.current !== 'passenger_onboard') {
// //             routeStatusRef.current = 'passenger_onboard';
// //             const pickup = normalizeCoords(td.ride?.pickupLocation ?? currentRide.pickupLocation);
// //             const dropoff = normalizeCoords(td.ride?.dropoffLocation ?? currentRide.dropoffLocation);
// //             if (pickup && dropoff) {
// //               log('route-set-onboard', { pickup: coordKey(pickup), dropoff: coordKey(dropoff) });
// //               setRoute({ pickup, dropoff, ready: true }); // FIX: single setState
// //             } else {
// //               log('route-onboard-missing-coords', {
// //                 hasPickup: !!pickup,
// //                 hasDropoff: !!dropoff,
...
// //     };
// //     // eslint-disable-next-line react-hooks/exhaustive-deps
// //   }, [rideId, rideStatus, hasCurrentRide]);
// //   // FIX: ↑ hasCurrentRide replaces currentRide?.driver?.id.
// //   // driver?.id can change between partial/full responses on slow connections,
// //   // restarting the interval and dropping location data mid-ride.
...
// //       setDriverLocation(loc);
// //       mapControlsRef.current?.updateDriverLocation(loc);
// //       // FIX: functional updaters bail out (no re-render) when the value is
// //       // unchanged. High-frequency bridge events often repeat the same ETA/distance,
// //       // so this eliminates a large share of renders from the native bridge.
// //       if (payload.eta != null) setEta(prev => payload.eta !== prev ? payload.eta : prev);
// //       if (payload.distance != null) setDistance(prev => payload.distance !== prev ? payload.distance : prev);
// //     });
// //     return () => unsub?.();
// //   }, [rnOn]); // eslint-disable-line
...
// //     // eslint-disable-next-line react-hooks/exhaustive-deps
// //   }, [rideId, currentRide?.id]);
// //   // FIX: ↑ currentRide?.id added to deps (was just [rideId]).
// //   // On slow connections currentRide is null on the first render, so the
// //   // guard short-circuits and startTracking is never called. Adding
// //   // currentRide?.id makes the effect re-fire when the ride data arrives
// //   // (null → id), ensuring startTracking is always invoked.
// //   // This is the primary fix for the refresh loop on slow connections.
...
// //     // eslint-disable-next-line react-hooks/exhaustive-deps
// //   }, [currentRide?.rideStatus, rideId]);
// //   // ^ router and stopTracking deliberately excluded — both are stable refs.
// //   // Including them causes the effect to re-fire on every render in some
// //   // Next.js builds due to reference instability.
...
// //         <Box sx={{ position: 'absolute', inset: 0, zIndex: 1 }}>
// //           <MapIframe
// //             center={initialMapCenter}        // FIX: stable — never changes with driver location
// //             zoom={15}
// //             markers={markers}
// //             pickupLocation={route.pickup}    // FIX: from batched route state object
// //             dropoffLocation={route.dropoff}  // FIX: from batched route state object
// //             showRoute={route.ready}          // FIX: from batched route state object
// //             onMapLoad={handleMapLoad}
// //           />
// //         </Box>
...
// // ── DEBUG BREAKERS — remove before production ──────────────────────────────────
// // Toggle any flag in the browser console to isolate the refresh cause:
// //   window.__TRACKING_DEBUG.BREAK_POLL = true
// //   window.__TRACKING_DEBUG.BREAK_RN_BRIDGE = true
// //   window.__TRACKING_DEBUG.BREAK_START_TRACKING = true
// //   window.__TRACKING_DEBUG.BREAK_SEED_DATA = true
// //   window.__TRACKING_DEBUG.BREAK_STATUS_FX = true
// //   window.__TRACKING_DEBUG.BREAK_ROUTE_RESET = true
// if (typeof window !== 'undefined' && !window.__TRACKING_DEBUG) {
//   window.__TRACKING_DEBUG = {
//     BREAK_POLL: false,
//     BREAK_RN_BRIDGE: false,
//     BREAK_START_TRACKING: false,
//     BREAK_SEED_DATA: false,
//     BREAK_STATUS_FX: false,
//     BREAK_ROUTE_RESET: false,
//   };
// }
// const DBG = () => (typeof window !== 'undefined' && window.__TRACKING_DEBUG) || {};
...
// const ROUTE_UPDATE_THRESHOLD_KM = 0.05;
...
//   // ── DEBUG render log ──────────────────────────────────────────────────────
//   log('render', {
//     renderCount: renderCount.current,   // ← how fast this climbs tells you if there's a loop
//     rideId,
//     rideStatus,
//     currentRideId: currentRide?.id,
//     sourceOfRide: activeRide ? 'activeRide' : 'ride',
...
//   // ── EFFECT 2: reset route on status change ────────────────────────────────
//   useEffect(() => {
//     // BREAKER: BREAK_ROUTE_RESET
//     // Flip ON if you suspect rideStatus flickering is tearing down the map route.
//     // Signs: MapIframe remounts repeatedly, route flashes on/off.
//     if (DBG().BREAK_ROUTE_RESET) {
//       console.warn('[DEBUG] BREAK_ROUTE_RESET active — route reset skipped', { rideStatus });
//       return;
//     }
...
//     });
//     routeStatusRef.current = null;
//     lastRouteDriverLocRef.current = null;
//     setRoute({ pickup: null, dropoff: null, ready: false });
//   }, [rideStatus]); // eslint-disable-line

//   // ── EFFECT 3: driver location polling ────────────────────────────────────
//   useEffect(() => {
//     if (!rideId || !currentRide || ['completed', 'cancelled'].includes(rideStatus)) {
//       log('poll-skipped', { rideId, rideStatus, hasCurrentRide });
//       return;
...
//     // BREAKER: BREAK_POLL
//     // Flip ON to stop the 5-second poll entirely.
//     // If the refresh loop stops → state changes from poll results (driverLocation,
//     // eta, distance, route) are causing the re-renders or cascading into useRide.
//     if (DBG().BREAK_POLL) {
//       console.warn('[DEBUG] BREAK_POLL active — polling disabled', { rideId, rideStatus });
//       return;
//     }
...
//                 });
//                 lastRouteDriverLocRef.current = loc;
//                 setRoute({ pickup: loc, dropoff: pickup, ready: true });
//               } else {
//                 log('route-accepted-threshold-not-met', {
//                   distanceMoved: calculateDistance(lastRouteDriverLocRef.current, loc).toFixed(4),
//                   thresholdKm: ROUTE_UPDATE_THRESHOLD_KM,
...
//             if (pickup && dropoff) {
//               log('route-set-onboard', { pickup: coordKey(pickup), dropoff: coordKey(dropoff) });
//               setRoute({ pickup, dropoff, ready: true });
//             } else {
//               log('route-onboard-missing-coords', {
//                 hasPickup: !!pickup,
//                 hasDropoff: !!dropoff,
...
//   // ── EFFECT 4: RN live location bridge ────────────────────────────────────
//   useEffect(() => {
//     // BREAKER: BREAK_RN_BRIDGE
//     // Flip ON to silence all DRIVER_LOCATION_UPDATED events from React Native.
//     // If the refresh loop stops → the native bridge is flooding setState calls
//     // faster than React can batch them, causing cascading re-renders.
//     if (DBG().BREAK_RN_BRIDGE) {
//       console.warn('[DEBUG] BREAK_RN_BRIDGE active — RN location bridge disabled');
//       return;
//     }
...
//   // ── EFFECT 5: startTracking ───────────────────────────────────────────────
//   useEffect(() => {
//     if (!rideId || !currentRide) {
//       log('start-tracking-skipped', {
//         rideId,
//         hasCurrentRide,
...
//     // BREAKER: BREAK_START_TRACKING
//     // Flip ON to prevent startTracking() from being called.
//     // THIS IS THE MOST LIKELY CULPRIT ON SLOW NETWORKS.
//     // If the refresh loop stops → startTracking() is triggering useRide state
//     // changes → activeRide reference updates → currentRide?.id changes →
//     // this effect re-fires → loop. Check if useRide's startTracking is
//     // idempotent (i.e. does it setState on every call even if already tracking?).
//     if (DBG().BREAK_START_TRACKING) {
//       console.warn('[DEBUG] BREAK_START_TRACKING active — startTracking not called', {
//         rideId,
//         currentRideId: currentRide?.id,
//       });
//       return;
...
//     log('start-tracking', { rideId, currentRideId: currentRide?.id });
//     const cleanup = startTracking(rideId, () => { });
//     return () => {
//       log('stop-tracking', { rideId });
//       if (cleanup) cleanup();
...
//   // ── EFFECT 6: seed initial data from currentRide ──────────────────────────
//   useEffect(() => {
//     if (!currentRide) return;

//     // BREAKER: BREAK_SEED_DATA
//     // Flip ON to skip seeding driverLocation/eta/distance/profilePic from currentRide.
//     // If the refresh loop stops → currentRide.id is changing repeatedly (null → value
//     // → null) causing this effect to fire on each change and trigger setState cascades.
//     // Check: is activeRide being reset to null between poll ticks by useRide?
//     if (DBG().BREAK_SEED_DATA) {
//       console.warn('[DEBUG] BREAK_SEED_DATA active — seed skipped', {
//         currentRideId: currentRide?.id,
//         rideStatus: currentRide?.rideStatus,
//       });
//       return;
...
//   // ── EFFECT 7: ride status side-effects (redirects) ────────────────────────
//   useEffect(() => {
//     if (!currentRide) return;

//     // BREAKER: BREAK_STATUS_FX
//     // Flip ON to prevent all redirect/snackbar logic from running.
//     // If the refresh loop stops → rideStatus is oscillating (e.g. null → accepted
//     // → null) and each oscillation is triggering router.replace() or setState,
//     // which causes a full page navigation or re-render cycle.
//     if (DBG().BREAK_STATUS_FX) {
//       console.warn('[DEBUG] BREAK_STATUS_FX active — status side-effects skipped', {
...
export default function TrackingPage() {
  ...
  // Stable map center — only recomputes when ride changes, never when driver moves
  const initialMapCenter = useMemo(() => (
    normalizeCoords(currentRide?.pickupLocation) || { lat: -15.4167, lng: 28.2833 }
  ), [currentRide?.id]); // eslint-disable-line
