...
//         if (backendStatus === 'completed' || backendStatus === 'cancelled' || td.ride?.paymentMethod === "completed") {
//           clearInterval(locPollRef.current);
//         }
...
export default function TrackingPage() {
  ...
  // Sends to the native debugger AND browser console so it works everywhere.
  // All keys are automatically prefixed with "tracking-page-".
  ...
  // ── Log every render — key diagnostic for detecting loops ────────────────
  log('render', {
    rideId,
    rideStatus,
    ...
    const poll = async () => {
      ...
      try {
        ...
        log('poll-response', {
          ...
          // Flag when the backend status differs from what React currently shows —
          // this mismatch is the most common root cause of unexpected redirects
          ...
          if (rideStatus === 'accepted') {
            ...
            if (pickup) {
              log('route-set-accepted', {
                driverLoc: `${loc.lat},${loc.lng}`,
                pickupLoc: `${pickup.lat},${pickup.lng}`,
              });
              setRoutePickup(loc);
              setRouteDropoff(pickup);
              setRouteReady(true);
  ...
  // ── Ride-status side-effects — THE REDIRECT DECISION POINT ───────────────
  useEffect(() => {
    if (!currentRide) return;
    const id           = currentRide.id ?? rideId;
    ...
    // This log fires every time rideStatus changes — look here first when debugging
    // unexpected redirects. The "sourceOfRide" field tells you whether it was
    // activeRide or ride that changed, which can point to the responsible hook.
