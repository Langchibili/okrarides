...
// //   const { getCurrentLocation: getNativeLocation, sendToNative } = useReactNative();
// //   const {
// //     loading: rideLoading,
// //     error: rideError,
// //     getFareEstimate,
...
// //   // FIX: reads geoCodeCounterRef (always current) instead of stale state
// //   const reverseGeocodeCoords = useCallback(async (lat, lng) => {
// //     try {
// //       if (geoCodeCounterRef.current >= 2) return null;
...
// //   // FIX: reads stopGeoCodeRef and geoCodeCounterRef instead of stale state
// //   const fetchServerStoredLocation = async (userId) => {
// //     if (userId) {
// //       if (geoCodeCounterRef.current >= 2) return;
...
//   // ─────────────────────────────────────────────────────────────────────────
//   // FIX: applyLocationFix
//   //
//   // Root cause of the bug:
//   //   fetchAndApplyNativeLocation calls fetchServerStoredLocation (not awaited),
//   //   so both run concurrently. fetchServerStoredLocation may set pickupLocation
//   //   to a geocoded address first, but then applyLocationFix unconditionally
//   //   overwrites it with raw coordinates (e.g. "-15.41234, 28.28340") in its
//   //   first setPickupLocation call. By the time applyLocationFix then tries to
//   //   geocode, geoCodeCounterRef is already at 2 (consumed by the concurrent
//   //   fetchServerStoredLocation calls), so reverseGeocodeCoords returns null
//   //   and the raw coordinates stay displayed permanently.
//   //
//   // Fix:
//   //   A `_geocoded: true` flag is stamped onto every location object that was
//   //   produced by an actual geocoding call (here and in fetchServerStoredLocation).
//   //   applyLocationFix's initial raw-coordinate write checks for that flag and
//   //   bails out early if the existing isCurrentLocation is already geocoded,
...
//     setPickupLocation((prev) => {
//       if (prev && !prev.isCurrentLocation) return prev;
//       // ── FIX: don't overwrite an already-geocoded current location with raw coords ──
...
//   // FIX: reads stopGeoCodeRef and geoCodeCounterRef instead of stale state
//   //      Also stamps _geocoded: true on the location it sets, so applyLocationFix
//   //      won't overwrite it with raw coordinates.
//   const fetchServerStoredLocation = async (userId) => {
//     if (userId) {
//       if (geoCodeCounterRef.current >= 2) return;
//       const res = await apiClient.get(`/users/${userId}`);
...
export default function HomePage() {
  ...
  // ── Controls bottom nav FAB toggle ────────────────────────────────────
  const [fabNavOpen, setFabNavOpen] = useState(false);
  ...
  // ── Hide bottom nav on mount; toggle based on fabNavOpen ─────────────
  useEffect(() => {
    hideNav();
  }, []);

  useEffect(() => {
    if (fabNavOpen) showNav();
    else hideNav();
    return () => showNav();
  }, [fabNavOpen]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <ClientOnly>
      <Box sx={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, width: '100%', height: '100%', overflow: 'hidden', maxWidth: '100vw', boxSizing: 'border-box' }}>
        ...
        {/* ── Floating Menu FAB — opens bottom nav ─────────────────────────── */}
        <AnimatePresence>
          {!fabNavOpen && (
            <motion.div
              key="fab-menu"
              ...
              style={{ position: 'fixed', bottom: 24, right: 16, zIndex: 1400 }}
            >
              <IconButton
                onClick={() => setFabNavOpen(true)}
                sx={{
                  width: 44,
                  height: 44,
                  bgcolor: isDark ? 'rgba(30,41,59,0.92)' : 'rgba(255,255,255,0.96)',
                  backdropFilter: 'blur(12px)',
                  color: 'primary.main',
                  border: '1.5px solid',
                  borderColor: isDark ? alpha(GREEN, 0.25) : 'rgba(0,0,0,0.1)',
                  boxShadow: isDark
                    ? `0 4px 18px rgba(0,0,0,0.4), 0 0 0 1px ${alpha(GREEN, 0.12)}`
                    : '0 4px 18px rgba(0,0,0,0.18)',
                  transition: 'all 0.18s ease',
                  '&:hover': {
                    transform: 'scale(1.1)',
                    boxShadow: isDark
                      ? `0 6px 22px rgba(0,0,0,0.5), 0 0 0 1px ${alpha(GREEN, 0.25)}`
        ...
        {/* ── Floating Close FAB — sits just above the bottom nav ──────────── */}
        <AnimatePresence>
          {fabNavOpen && (
            <motion.div
              key="fab-close"
              ...
              exit={{ opacity: 0, scale: 0.7 }}
              transition={{ type: 'spring', stiffness: 420, damping: 28 }}
              style={{ position: 'fixed', bottom: 120, right: 16, zIndex: 2000 }}
            >
              <IconButton
                onClick={() => setFabNavOpen(false)}
                sx={{
                  width: 32,
                  height: 32,
                  bgcolor: isDark ? 'rgba(239,68,68,0.85)' : 'rgba(239,68,68,0.9)',
