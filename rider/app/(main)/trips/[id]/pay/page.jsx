...
import {
  Money as CashIcon, CheckCircle as CheckIcon,
  PhoneAndroid as PhoneIcon, ContentCopy as CopyIcon, Check as CopiedIcon,
...
function MobileMoneyCard({ entry }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }}>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, p: 1.5, borderRadius: 2.5, border: '1.5px solid', borderColor: 'divider', bgcolor: 'background.paper' }}>
        <Box sx={{ width: 42, height: 42, borderRadius: '11px', bgcolor: style.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
        ...
        <Box sx={{ flex: 1, minWidth: 0 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75, mb: 0.25 }}>
            ...
            <Typography variant="caption" sx={{ fontWeight: 700, color: style.text, textTransform: 'uppercase', letterSpacing: 0.5 }}>
              {style.label}
            </Typography>
          </Box>
          <Typography variant="body1" sx={{ fontWeight: 700, letterSpacing: 0.5, lineHeight: 1.2 }}>{entry.mobileNumber}</Typography>
          <Typography variant="caption" color="text.secondary" noWrap>Registered as: <strong>{entry.name}</strong></Typography>
        </Box>
        <Tooltip title={copied ? 'Copied!' : 'Copy number'} placement="top">
          <IconButton size="small" onClick={handleCopy} sx={{ flexShrink: 0, bgcolor: copied ? 'success.light' : 'action.hover', color: copied ? 'success.dark' : 'text.secondary', transition: 'background-color 0.2s,color 0.2s', '&:hover': { bgcolor: copied ? 'success.light' : 'action.selected' } }}>
...
export default function PayRidePage() {
  ...
  const handleOkraPayPayment = async () => {
    ...
    try {
      ...
      if (result?.paidFromWallet) { handlePaymentDone(); return; }
  ...
  return (
    // ── height:100vh + flex column — header fixed, content scrolls ──────────
    <Box sx={{ height: '100vh', bgcolor: 'background.default', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>

      {/* ── Header ── */}
      <Box sx={{ flexShrink: 0, bgcolor: 'primary.main', color: 'white', pt: 6, pb: 4, px: 3, textAlign: 'center' }}>
      ...
      <Box sx={{
        ...
        {/* CASH */}
        <Paper component={motion.div} whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }}
        ...
        <AnimatePresence>
          {driverNumbers.length > 0 && (
            <motion.div key="mobile-money-section" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              <Paper elevation={2} sx={{ borderRadius: 3, overflow: 'hidden', border: '2px solid', borderColor: 'warning.light' }}>
                <Box sx={{ px: 2.5, py: 2, display: 'flex', alignItems: 'flex-start', gap: 1.5, bgcolor: 'warning.50', borderBottom: '1.5px solid', borderColor: 'warning.light' }}>
                  <Box sx={{ width: 48, height: 48, borderRadius: 2, bgcolor: 'warning.light', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <PhoneIcon sx={{ color: 'warning.dark', fontSize: 26 }} />
                  </Box>
                  <Box>
                    <Typography fontWeight={700} sx={{ lineHeight: 1.3 }}>Send via Mobile Money</Typography>
                ...
                <Box sx={{ p: 2, display: 'flex', flexDirection: 'column', gap: 1.25 }}>
                  {driverNumbers.map((entry, idx) => <MobileMoneyCard key={idx} entry={entry} />)}
                </Box>
                <Divider />
                <Box sx={{ p: 2 }}>
                  <Button fullWidth variant="contained" color="warning" size="large"
                    ...
                    {paying ? <CircularProgress size={22} color="inherit" /> : `I've Sent ${formatCurrency(fare)} — Confirm`}
                  </Button>
