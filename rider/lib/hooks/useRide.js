...
export function useRide(initialRideId = null) {
  ...
  // ═══════════════════════════════════════════════════════════════════════
  // REACT NATIVE BRIDGE LISTENERS (always active, no auto-loading)
  // ═══════════════════════════════════════════════════════════════════════
  useEffect(() => {
    const forCurrentRide = (payload) =>
      !activeRideIdRef.current ||
    ...
    // ── APP_RESUMED ─────────────────────────────────────────────────────
    // NOTE: No automatic reload - caller must handle this
    const unsubResumed = rnOn('APP_RESUMED', () => {
      // Do nothing automatically - caller can implement refresh if needed
      console.log('[useRide] APP_RESUMED received - no automatic action');
    });
  ...
  }, [rnOn, stopPollingRideStatus, stopTracking, router]);

  // ═══════════════════════════════════════════════════════════════════════
  // SOCKET.IO LISTENERS - only active when explicitly started
  // ═══════════════════════════════════════════════════════════════════════
  // NOTE: Socket listeners are NOT automatically set up
  // They must be enabled by calling setupSocketListeners(rideId) manually

  const socketListenersActive = useRef(false);
  const currentSocketRideId = useRef(null);

  const setupSocketListeners = useCallback((rideId) => {
    if (!rideId) return;

    // Clean up existing listeners if any
    if (socketListenersActive.current) {
      socketOff(SOCKET_EVENTS.RIDE.ACCEPTED);
      socketOff(SOCKET_EVENTS.DRIVER.LOCATION_UPDATED);
      socketOff(SOCKET_EVENTS.DRIVER.ARRIVED);
      socketOff(SOCKET_EVENTS.RIDE.TRIP_STARTED);
    ...
    currentSocketRideId.current = rideId;
  ...
  }, [socketOn, socketOff, stopTracking, stopPollingRideStatus]);

  const removeSocketListeners = useCallback(() => {
    if (!socketListenersActive.current) return;
    ...
    currentSocketRideId.current = null;
  }, [socketOff]);
  ...
  const requestRide = useCallback(async (rideData) => {
    ...
    try {
      ...
      setRide(newRide); setActiveRide(newRide);
      if (hasWebSocket.current && window.realtimeService) window.realtimeService.joinRideRoom(newRide.id);
      // NOTE: Polling must be started manually by caller if needed
      return { success: true, data: newRide };
    } catch (err) {
      const msg = err.message || 'Failed to create ride'; setError(msg); return { success: false, error: msg };
    } finally { setLoading(false); }
  ...
  // ============================================
  // STEP 4: Poll for Driver Assignment & Status (MANUAL START)
  // ============================================
  const startPollingRideStatus = useCallback((rideId, maxAttempts = 120, onUpdate = null) => {
    stopPollingRideStatus();
    let attempts = 0;
    pollingIntervalRef.current = setInterval(async () => {
      if (!mountedRef.current) { stopPollingRideStatus(); return; }
  ...
  // ============================================
  // STEP 5: Track Ride (MANUAL START)
  // ============================================
  const startTracking = useCallback((rideId, onUpdate = null) => {
    stopTracking();
    trackingIntervalRef.current = setInterval(async () => {
      if (!mountedRef.current) { stopTracking(); return; }
      try {
  ...
  const loadActiveRide = useCallback(async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        setActiveRide(result.data);
        if (hasWebSocket.current && window.realtimeService) window.realtimeService.joinRideRoom(result.data.id);
        // NOTE: Polling must be started manually by caller
        return result.data;
      }
      return null;
    } catch (err) { console.error('Error loading active ride:', err); return null; }
  ...
  // ============================================
  // window.realtimeService WebSocket handlers (MANUAL)
  // ============================================
  const setupRealtimeServiceHandlers = useCallback((rideId, handlers) => {
    if (!rideId || !hasWebSocket.current || !window.realtimeService) return () => { };

    const rs = window.realtimeService;
    const byId = (fn) => (data) => { if (data.rideId === rideId) fn(data); };

    const onAssigned = byId(handlers.onDriverAssigned || (() => { }));
    const onLoc = byId(handlers.onDriverLocation || (() => { }));
    const onArrived = byId(handlers.onDriverArrived || (() => { }));
    const onStarted = byId(handlers.onTripStarted || (() => { }));
    const onCompleted = byId(handlers.onTripCompleted || (() => { }));
    const onCancelled = byId(handlers.onRideCancelled || (() => { }));

    rs.on('driverAssigned', onAssigned);
    rs.on('driverLocationUpdate', onLoc);
    rs.on('driverArrived', onArrived);
    rs.on('tripStarted', onStarted);
    rs.on('tripCompleted', onCompleted);
    rs.on('rideCancelled', onCancelled);

    return () => {
      rs.off('driverAssigned', onAssigned);
      rs.off('driverLocationUpdate', onLoc);
      rs.off('driverArrived', onArrived);
      rs.off('tripStarted', onStarted);
      rs.off('tripCompleted', onCompleted);
      rs.off('rideCancelled', onCancelled);
    };
  }, []);

  // ============================================
  // Cleanup on Unmount (NO auto-loading)
  // ============================================
  useEffect(() => {
    mountedRef.current = true;
    return () => {
  ...
  }, [activeRide, stopPollingRideStatus, stopTracking, removeSocketListeners]);

  // ═══════════════════════════════════════════════════════════════════════
  // NO AUTOMATIC LOADS - Everything must be called manually
  // ═══════════════════════════════════════════════════════════════════════
  // - loadActiveRide() must be called explicitly
  // - loadRide() must be called explicitly  
  // - startPollingRideStatus() must be called explicitly
  // - startTracking() must be called explicitly
  // - setupSocketListeners() must be called explicitly
  // - setupRealtimeServiceHandlers() must be called explicitly
