...
export function ReactNativeWrapper({ children }) {
  ...
  useEffect(() => {
    ...
    const handleMessage = (event) => {
      try {
        ...
        // Handle event-based messages - Emit to registered handlers
        const handlers = messageHandlersRef.current.get(type);
        if (handlers) {
          console.log(`[ReactNativeWrapper] Emitting ${type} to ${handlers.size} handler(s)`);
          handlers.forEach(handler => handler(payload));
        }

        // Handle specific message types for state updates
        switch (type) {
          case 'PERMISSION_RESULT':
            setPermissions(prev => ({
              ...prev,
  ...
  const sendToNative = useCallback((type, payload = {}) => {
    return new Promise((resolve, reject) => {
      ...
      // Store timeout ID so we can clear it if response comes
      const requestData = pendingRequestsRef.current.get(requestId);
      if (requestData) {
  ...
  const on = useCallback((type, handler) => {
    ...
    console.log(`[ReactNativeWrapper] Registered handler for event: ${type}`);
    ...
    return () => {
      ...
      if (handlers) {
        handlers.delete(handler);
        console.log(`[ReactNativeWrapper] Unregistered handler for event: ${type}`);
      }
    };
  }, []);
  ...
  const initializeNativeServices = useCallback(async (userId, frontendName, socketServerUrl) => {
    ...
    try {
      ...
      console.log('[ReactNativeWrapper] Native services initialized successfully');
  ...
  const value = {
    ...
    currentFrontend,
    sendToNative,
    on, // This is critical - exposes the event subscription system
    requestPermission,
    initializeNativeServices,
    getCurrentLocation,
    startLocationTracking,
