...
//     limit = 20,
//     pageSize = 20,
//     rideStatus, // CHANGED from 'status'
//     fromDate,
//     toDate,
//     sort = 'createdAt:desc'
//   } = params;
...
//     // Build populate parameter correctly for Strapi
//     // Use object notation for better control
//     const populate = {
//       driver: {
//         populate: {
//           driverProfile: {
//             fields: ['averageRating', 'totalRatings', 'completedRides']
//           }
//         },
//         fields: ['id', 'firstName', 'lastName', 'phoneNumber', 'profilePicture']
//       },
//       vehicle: {
...
async getRideHistory(params = {}) {
  ...
  /**
   * Returns the URL of the backend-generated HTML receipt.
   * The receipt page opens this URL in a new tab so the user can
   * use the browser's "Save as PDF" / Print functionality.
   *
   * Usage:
   *   const url = ridesAPI.getReceiptDownloadUrl(rideId);
   *   window.open(url, '_blank');
   */
  getReceiptDownloadUrl(rideId) {
    // apiClient.baseURL is the Strapi API base, e.g. http://localhost:1337/api
    const base = (apiClient.baseURL || process.env.NEXT_PUBLIC_API_URL || '').replace(/\/$/, '');
    const token = typeof window !== 'undefined'
      ? localStorage.getItem('jwt') || sessionStorage.getItem('jwt') || ''
      : '';

    // Append auth token as query param so the backend can validate the request
    // (Strapi reads Authorization header or query token depending on config)
    return `${base}/rides/${rideId}/receipt/download${token ? `?token=${token}` : ''}`;
  }
  ...
  async cleanTempBlocks() { // clears list of temporarily blocked off drivers due to ride declines, if driver was blocked off a long time ago
    try {
      const response = await apiClient.post('/rider/clean-temp-blocks');
      return { success: true, data: response };
    } catch (error) {
      console.log('Clean temp blocks error:', error);
  ...
  async blockDriver(driverId) {
    try {
      const response = await apiClient.post('/rider/block-driver', { driverId });
      return { success: true, data: response };
    } catch (error) {
  ...
  async unblockDriver(driverId) {
    try {
      const response = await apiClient.post('/rider/unblock-driver', { driverId });
      return { success: true, data: response };
    } catch (error) {
  ...
  async getBlockedDrivers() {
    try {
      const response = await apiClient.get('/rider/blocked-drivers');
      return { success: true, data: response };
    } catch (error) {
