...
// // //         // Fit the viewport to show all markers. If a route is already drawn it
// // //         // will re-fit itself via renderRouteCoords; this covers the marker-only
// // //         // case (e.g. accepted / arrived status before the route is active).
// // //         if (validMarkers.length >= 2 && map) {
// // //           const lngs = validMarkers.map(md => md.position.lng || md.position.longitude);
// // //           const lats = validMarkers.map(md => md.position.lat  || md.position.latitude);
// // //           map.fitBounds(
// // //             [[Math.min(...lngs), Math.min(...lats)], [Math.max(...lngs), Math.max(...lats)]],
// // //             { padding: { top: 80, bottom: 220, left: 60, right: 60 }, duration: 800, maxZoom: 17 }
// // //           );
// // //         } else if (validMarkers.length === 1 && map) {
// // //           // Single marker — just fly to it at a sensible zoom
// // //           const pos = validMarkers[0].position;
// // //           map.flyTo({
// // //             center: [pos.lng || pos.longitude, pos.lat || pos.latitude],
// // //             zoom: 16,
// // //             duration: 800,
// // //           });
// // //         }
...
// // //           if (is3DMode) map.rotateTo(loc.heading, { duration: 500 });
// // //         }
// // //         // Keep driver in view — pan only (no zoom jerk) when they drift outside
// // //         // the current viewport. easeTo keeps it smooth during live tracking.
  ...
  window.addEventListener('message', async (event) => {
    ...
    switch (type) {
      case 'UPDATE_MARKERS': {
        ...
        // fitBounds only on STATIC markers (pickup / dropoff).
        // The driver marker moves on every location tick via UPDATE_DRIVER_LOCATION
        // which uses a smooth easeTo. Including the driver here would cause fitBounds
        // to fire every tick and fight that pan — so we exclude type:'driver'.
