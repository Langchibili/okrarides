...
//         if ((rideStatus === 'passenger_onboard' || rideStatus === 'awaiting_payment') && driverLoc) {
//           // Recalculate live from driver's current position → dropoff on every update
//           const dropoff = normalizeCoords(ride.dropoffLocation);
//           if (dropoff) {
//             const dist = calculateDistance(driverLoc, dropoff);
//             if (mountedRef.current) { setDistance(dist); setEta(estimateDuration(dist)); }
...
//                         <Typography variant="caption" sx={{ fontWeight:600 }}>{rideStatus==='accepted' ? 'ETA to Pickup' : 'ETA to Destination'}</Typography>
//                         <Box sx={{ display:'flex', gap:1.5, alignItems:'center' }}>
//                           {distance!=null && <Typography variant="caption" sx={{ fontWeight:700, color:'text.secondary' }}>{typeof distance === 'number' ? distance.toFixed(1) : distance} km</Typography>}
//                           <Typography variant="caption" sx={{ fontWeight:800, color: rideStatus==='accepted' ? '#10B981' : '#FF8C00' }}>{formatETA(eta)}</Typography>
//                         </Box>
//                       </Box>
//                       <LinearProgress variant="indeterminate" sx={{ borderRadius:2, height:5, bgcolor: rideStatus==='accepted' ? 'rgba(16,185,129,0.15)' : 'rgba(255,193,7,0.15)', '& .MuiLinearProgress-bar': { background: rideStatus==='accepted' ? 'linear-gradient(90deg,#10B981,#059669)' : 'linear-gradient(90deg,#FFC107,#FF8C00)' } }} />
...
export default function ActiveRidePage() {
  ...
  useEffect(() => {
    ...
    const updateLocations = async () => {
      ...
      try {
        ...
        if (driverLoc && mountedRef.current) {
          setDriverLocation(driverLoc);
          // Move driver marker only — does NOT touch camera/zoom
          mapControlsRef.current?.updateDriverLocation(driverLoc);
          if (fullScreenMapInitializedRef.current) {
            fullScreenMapControlsRef.current?.updateDriverLocation(driverLoc);
          }
        ...
        // accepted: route driver → pickup (updates every tick so line tracks driver)
        if (rideStatus === 'accepted' && driverLoc) {
          const riderRes = await apiClient.get(`/users/${ride.rider?.id}`);
          const rawRider = riderRes?.data?.currentLocation ?? riderRes?.currentLocation;
          const riderLoc = normalizeCoords(rawRider);
        ...
        // passenger_onboard / awaiting_payment: ETA live, route set ONCE
        if ((rideStatus === 'passenger_onboard' || rideStatus === 'awaiting_payment') && driverLoc) {
          const dropoff = normalizeCoords(ride.dropoffLocation);
          if (dropoff) {
            const dist = calculateDistance(driverLoc, dropoff);
          ...
          // Set route ONCE so fitBounds fires cleanly on status entry
          if (routeStatusRef.current !== 'passenger_onboard') {
            routeStatusRef.current = 'passenger_onboard';
            const pickup      = normalizeCoords(ride.pickupLocation);
            const dropoffNorm = normalizeCoords(ride.dropoffLocation);
  ...
  // Destination for fullscreen nav map
  ...
  return (
    // ── FIX: homologous full-screen layout ────────────────────────────────────
    // Previously: height:100vh flex-column with static AppBar → map was pushed
    //   56px below viewport → iframe's fitBounds fired on wrong coordinate space.
    // Now: map is position:absolute filling 100vh, AppBar floats on top,
    //   SwipeableBottomSheet overlays from bottom — identical to tracking page.
    //   The iframe fills the true viewport, so fitBounds padding (bottom:220)
    //   correctly accounts for the bottom sheet overlay and route zoom works.
    <Box sx={{ height: '100vh', width: '100%', position: 'relative', overflow: 'hidden' }}>
      ...
      {/* ── AppBar — position:absolute, floats over map ──────────────────── */}
      <AppBar
        position="absolute"
        elevation={0}
        sx={{
      ...
      >
        <Toolbar>
          ...
          <Chip
            ...
            sx={{ color: 'white', mr: 1, fontWeight: 700, textTransform: 'capitalize' }}
          />
        </Toolbar>
      </AppBar>

      {/* ── Navigate FAB + popup ─────────────────────────────────────────── */}
      <Box sx={{ position: 'absolute', bottom: 450, right: 16, zIndex: 5 }}>
        <Box sx={{ position: 'relative' }}>
          ...
          <Fab
            variant="extended"
            color="primary"
            sx={{ borderRadius: '24px', boxShadow: '0 6px 24px rgba(0,0,0,0.35)' }}
      ...
      {showNavPopup && (
        <Box sx={{ position: 'fixed', inset: 0, zIndex: 4 }} onClick={() => setShowNavPopup(false)} />
      )}

      {/* ── Bottom Sheet — SwipeableBottomSheet overlaid from bottom ─────── */}
      <SwipeableBottomSheet open initialHeight={420} maxHeight="80%" minHeight={200} draggable>
        ...
        <Box sx={{ px: 2.5, pb: 3, flex: 1, overflowY: 'auto', '&::-webkit-scrollbar': { display: 'none' }, scrollbarWidth: 'none' }}>
                    ...
                    <Box>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 0.5 }}>
                        ...
                        <Box sx={{ display: 'flex', gap: 1.5, alignItems: 'center' }}>
                          ...
                          <Typography variant="caption" sx={{ fontWeight: 800, color: rideStatus === 'accepted' ? '#10B981' : '#FF8C00' }}>
                            {formatETA(eta)}
                          </Typography>
                        </Box>
                      </Box>
