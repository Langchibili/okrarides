...
// 'use client';
// // PATH: driver/app/(main)/active-ride/[id]/page.jsx
// // MAP/MARKER CHANGES ONLY — all ride/payment/socket/UI logic untouched.
// // Changes:
// //   • accepted  → routePickup = driverLoc (updates each 2s poll, no fitBounds spam)
// //                 routeDropoff = ride.pickupLocation (fixed)
// //   • passenger_onboard → routePickup = ride.pickupLocation (fixed)
// //                         routeDropoff = ride.dropoffLocation (fixed), drawn once
// //   • markers stays useMemo(()=>[], []) — driver moved via updateDriverLocation only
// //   • configureDriverMarker({ bg:'#1d4ed8', label:'🚗', size:42 }) called in onMapLoad
...
//   // ── 2-second live location + ETA + route polling ──────────────────────────
//   useEffect(() => {
//     if (!ride || ['completed', 'cancelled'].includes(rideStatus)) return;
...
//         if (driverLoc && mountedRef.current) {
//           setDriverLocation(driverLoc);
//           // Move driver marker smoothly — no re-render, no ghost pins
//           mapControlsRef.current?.updateDriverLocation(driverLoc);
//           if (fullScreenMapInitializedRef.current) {
//             fullScreenMapControlsRef.current?.updateDriverLocation(driverLoc);
//           }
...
//         // ── ACCEPTED: driver heading to rider pickup ─────────────────────────
//         // Route origin (routePickup) = driver's live position — updates every poll.
//         // Route end   (routeDropoff) = ride.pickupLocation    — fixed.
//         // fitBounds fires only on FIRST draw (renderRouteCoords guards on routeLoaded).
//         if (rideStatus === 'accepted' && driverLoc) {
//           // ETA: driver → rider's live position (or static pickup as fallback)
//           const riderRes = await apiClient.get(`/users/${ride.rider?.id}`);
//           const rawRider = riderRes?.data?.currentLocation ?? riderRes?.currentLocation;
...
//           // Update routePickup every poll so route line shifts as driver moves
//           const pickup = normalizeCoords(ride.pickupLocation);
//           if (pickup) {
...
//         // Route = pickup → dropoff (both fixed). Drawn ONCE.
//         // Driver marker moves via updateDriverLocation above.
//         if ((rideStatus === 'passenger_onboard' || rideStatus === 'awaiting_payment') && driverLoc) {
//           const dropoff = normalizeCoords(ride.dropoffLocation);
//           if (dropoff) {
//             const dist = calculateDistance(driverLoc, dropoff);
...
//               setRouteDropoff(dropoffNorm); // fixed — where rider is going
//               setRouteReady(true);
//             }
//           }
//         }
...
//   // ── MAP: markers — empty, driver moved via updateDriverLocation ───────────
//  // ── MAP: markers — driver excluded (moved via updateDriverLocation) ────────
// // For accepted:     show 📍 at ride.pickupLocation (where rider is, driver is heading there)
// //                   suppress the auto-pin that would appear at driverLoc (routePickup prop)
// // For onboard:      show 📍 pickup + 🎯 dropoff (both fixed)
// const markers = useMemo(() => {
//   if (!ride) return [];
...
//   if (rideStatus === 'accepted') {
//     const result = [];
//     // Override 'pickup' → suppresses auto-pin at driverLoc (routePickup = driver's live position)
//     // Places 📍 at the rider's actual pickup location instead
...
//     const result = [];
//     // Both fixed — show route endpoints correctly
//     if (pickupLoc)  result.push({ id: 'pickup',  position: pickupLoc,  type: 'pickup'  });
...
//   const handleStartTrip     = async () => { try { await startTrip(ride.id); loadRideDetails(); } catch (err) { console.error('Error starting trip:', err); } };
...
//       await completeTrip(ride.id, { actualDistance: ride.estimatedDistance, actualDuration: ride.estimatedDuration });
//       router.push('/home');
//     } catch (err) { console.error('Error completing trip:', err); setCompleting(false); setShowCompleteDialog(false); }
//   };
//   const handleConfirmArrival = async () => { try { await confirmArrival(ride.id); loadRideDetails(); } catch (err) { console.error('Error confirming arrival:', err); } };
//   const handleNavigate = () => setShowNavPopup(prev => !prev);
...
//               // Default car icon for ride — configured once, never re-created
...
//           <AnimatePresence>
//             {(rideStatus === 'accepted' || rideStatus === 'passenger_onboard' || rideStatus === 'awaiting_payment') && (
//               <motion.div key={rideStatus + '-eta-banner'} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} transition={{ type: 'spring', stiffness: 280, damping: 24 }}>
//                 <Box sx={{
//                   p: 2, borderRadius: 3, mb: 2,
//                   background: rideStatus === 'accepted'
//                     ? 'linear-gradient(135deg, rgba(16,185,129,0.12) 0%, rgba(5,150,105,0.06) 100%)'
...
//                         borderRadius: 2, height: 5,
//                         bgcolor: rideStatus === 'accepted' ? 'rgba(16,185,129,0.15)' : 'rgba(255,193,7,0.15)',
//                         '& .MuiLinearProgress-bar': {
//                           background: rideStatus === 'accepted'
//                             ? 'linear-gradient(90deg, #10B981, #059669)'
//                             : 'linear-gradient(90deg, #FFC107, #FF8C00)',
...
//                   <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 0.5 }}>
//                     <TimerIcon sx={{ fontSize: 16, color: 'text.secondary' }} />
//                     <Typography variant="caption" color="text.secondary">Complete Trip available in {completeLockSecondsLeft}s — giving rider time to pay</Typography>
//                   </Box>
...
//               {ride.rideStatus === 'awaiting_payment' && (
//                 <Alert severity="info" icon={<PaymentIcon />} sx={{ borderRadius: 2 }}>Waiting for rider to complete payment…</Alert>
//               )}

//               <Button fullWidth variant="contained" color="success" size="large" onClick={handleCompleteTrip}
//                 disabled={completeTripLocked} startIcon={<CheckIcon />}
//                 sx={{ height: 56, fontWeight: 700, borderRadius: 3, opacity: completeTripLocked ? 0.5 : 1 }}>
//                 Complete Trip
//               </Button>
//             </Box>
...
//       <Dialog open={showCompleteDialog} onClose={() => !completing && setShowCompleteDialog(false)}
//         PaperProps={{ sx: { borderRadius: 4, maxWidth: 360, mx: 2, border: '1px solid', borderColor: 'divider' } }}>
//         <DialogTitle sx={{ fontWeight: 700, pb: 0.5 }}>
//           <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
//             <Box sx={{ width: 40, height: 40, borderRadius: 2, flexShrink: 0, background: 'linear-gradient(135deg, #10B981 0%, #059669 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 12px rgba(16,185,129,0.35)' }}>
//               <CheckIcon sx={{ fontSize: 22, color: '#fff' }} />
//             </Box>
//             Complete Trip?
//           </Box>
