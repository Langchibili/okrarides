...
export default function DriverHomePage() {
  ...
  // Light-mode appbar gradient
  const appBarBg = isDark
    ? `linear-gradient(135deg, #1E293B 0%, #0F172A 100%)`
    : `linear-gradient(135deg, #ffffff 0%, #F8FAFC 100%)`;
  const appBarBorder = isDark
    ? `1px solid ${alpha(GREEN, 0.12)}`
    : `1px solid ${alpha('#CBD5E1', 0.8)}`;
  const appBarShadow = isDark
    ? `0 1px 0 ${alpha(GREEN, 0.08)}, 0 4px 20px rgba(0,0,0,0.35)`
    ...
    <ClientOnly>
      <Box sx={{ height: '100vh', display: 'flex', flexDirection: 'column', bgcolor: 'background.default' }}>
        ...
        <AppBar position="static" elevation={0} sx={{
          ...
          <Box sx={{
            height: 2,
            background: `linear-gradient(90deg, ${GREEN} 0%, #3B82F6 50%, ${GREEN} 100%)`,
            backgroundSize: '200% 100%',
          }} />
          <Toolbar sx={{ justifyContent: 'space-between', gap: 1 }}>
            <AnimatedHeaderButton label="APPS" direction="left" icon={<AppsIcon size={22} color={GREEN} />}
              onClick={() => { if (landingPageUrl) router.push(landingPageUrl); }} />
            <Box sx={{ display: 'flex', alignItems: 'center' }}>
              <ThemeToggle isDark={isDark} onToggle={toggleColorMode} />
            </Box>
            <AnimatedHeaderButton label="HELP" direction="right" icon={<HelpIcon size={22} color={GREEN} />}
              onClick={() => router.push('/help')} />
          </Toolbar>
        </AppBar>
          ...
          {loadingDriverProfile ? (
            ...
            <>
              ...
              {driverProfile?.verificationStatus === VERIFICATION_STATUS.APPROVED && (
                <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}>
                  <Paper elevation={0} sx={{
                    ...
                    background: isOnSubscriptionSystem
                      ? isDark
                        ? `linear-gradient(135deg, ${alpha('#10B981', 0.18)} 0%, ${alpha('#047857', 0.1)} 100%)`
                        : `linear-gradient(135deg, ${alpha('#10B981', 0.1)} 0%, ${alpha('#ECFDF5', 1)} 100%)`
                      : isDark
                        ? `linear-gradient(135deg, ${alpha('#3B82F6', 0.18)} 0%, ${alpha('#1D4ED8', 0.1)} 100%)`
                        : `linear-gradient(135deg, ${alpha('#3B82F6', 0.08)} 0%, ${alpha('#EFF6FF', 1)} 100%)`,
                    border: `1px solid ${alpha(isOnSubscriptionSystem ? '#10B981' : '#3B82F6', isDark ? 0.28 : 0.2)}`,
                    boxShadow: isDark ? 'none' : `0 2px 12px ${alpha(isOnSubscriptionSystem ? '#10B981' : '#3B82F6', 0.1)}`,
                    display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                  }}>
                    <Typography variant="body2" sx={{
                      fontWeight: 600, fontSize: 12,
                      color: isOnSubscriptionSystem ? (isDark ? '#34D399' : '#059669') : (isDark ? '#60A5FA' : '#2563EB'),
                    }}>
                      💰{' '}
                      {paymentSystemType === 'subscription_based' ? '0% Commission — Subscription Plan'
                        : paymentSystemType === 'float_based'     ? `Float — ${defaultCommissionPercentage}% Commission`
              ...
              <Box sx={{
                ...
                gridAutoRows: '1fr', gap: 1.5, mb: 1.5,
                '& > *': { minWidth: 0, minHeight: 0 },
              }}>
                {[
                  { el: <EarningsCard title="Today's Earnings" amount={summary.totalEarnings} icon={<EarningsIcon />} color="earnings" onClick={() => router.push('/earnings')} />, delay: 0.08 },
                ...
                {[
                  ...
                  <motion.div key={label} style={{ flex: 1 }}
                    ...
                    <Button variant="outlined" fullWidth startIcon={icon}
                      onClick={() => router.push(path)}
                      sx={{
                        borderRadius: 2.5, height: 46, fontWeight: 600, textTransform: 'none',
                        // Light mode: stronger border + subtle background
                        borderColor: isDark ? undefined : alpha('#10B981', 0.35),
                        background: isDark ? undefined : alpha('#10B981', 0.04),
                        '&:hover': {
                          background: isDark ? undefined : alpha('#10B981', 0.08),
                          borderColor: isDark ? undefined : alpha('#10B981', 0.55),
                        },
                      }}>
                      {label}
                    </Button>
                ...
                <Box sx={{ display: 'flex', gap: 1.5 }}>
                  {isFloatSystemEnabled && (
                    <Button
                      ...
                      onClick={() => router.push('/float')}
                      sx={{
                        borderRadius: 2.5, height: 46, fontWeight: 600, textTransform: 'none', flex: 1,
                        ...(!(isFloatNegative || isFloatLow) && !isDark && {
                          borderColor: alpha('#6B7280', 0.35),
                          background: alpha('#F9FAFB', 1),
                        }),
                      }}>
                      Float: {formatCurrency(floatBalance)}
                    </Button>
                  ...
                  {isSubscriptionSystemEnabled && (
                    <Button
                      ...
                      sx={{
                        borderRadius: 2.5, height: 46, fontWeight: 600, textTransform: 'none', flex: 1,
                        ...(!isSubscriptionExpired && !isSubscriptionExpiringSoon && !isDark && {
                          borderColor: alpha('#6B7280', 0.35),
                          background: alpha('#F9FAFB', 1),
                        }),
                      }}>
