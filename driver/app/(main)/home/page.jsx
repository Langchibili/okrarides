...
export default function DriverHomePage() {
  ...
  const { toggleTheme: toggleColorMode } = useThemeMode();
  const { isAuthenticated } = useAuth();
  const { user } = useAuthGuard({ requireSubscription: true, requireVerification: true });
  const { driverProfile, isOnline, toggleOnline, needsVerification, needsVehicle, needsSubscription, paymentSystemType, loadingDriverProfile } = useDriver();
  const { isNegativeFloatAllowed, negativeFloatLimit, minimumFloatTopup, defaultCommissionPercentage, isFloatSystemEnabled, isSubscriptionSystemEnabled } = useAdminSettings();
  const { incomingRide, acceptRide, declineRide, currentRide } = useRide();
  const { isNative, getCurrentLocation, reconnectDeviceSocket } = useReactNative();
  ...
  useEffect(() => {
    ...
    requestLocationOnMount();
    if (isNative) {
      reconnectDeviceSocket(user?.id, 'driver', process.env.NEXT_PUBLIC_DEVICE_SOCKET_URL);
    }
    apiClient.post('/delivery-driver/toggle-offline');
  }, []);
  ...
  if (!isAuthenticated()) return <HomePageSkeleton />;

  return (
    <ClientOnly>
      <Box sx={{
        height: '100vh', display: 'flex', flexDirection: 'column',
        background: surfaceBg, position: 'relative',
      }}>
        {/* Ambient bg blob */}
        <Box sx={{
          position: 'absolute', top: -60, right: -60, width: 240, height: 240,
        ...
        {/* ── AppBar ── */}
        <AppBar position="static" elevation={0} sx={{
          background: appBarBg, backdropFilter: 'blur(12px)',
          borderBottom: isDark ? `1px solid ${alpha(GREEN, 0.12)}` : `1px solid ${alpha('#047857', 0.2)}`,
          boxShadow: isDark ? `0 2px 16px ${alpha('#000', 0.4)}` : `0 4px 20px ${alpha('#059669', 0.25)}`,
          zIndex: 10,
        }}>
          <Toolbar sx={{ justifyContent: 'space-between', gap: 1 }}>
            <AnimatedHeaderButton label="APPS" direction="left"
