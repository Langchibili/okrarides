      ...
      <motion.div variants={fadeUp} initial="hidden" animate="show">
        <Box sx={{ textAlign: 'center', mb: 3.5 }}>
          <Box sx={{
            ...
            boxShadow: isDark ? `0 8px 32px ${alpha('#10B981', 0.2)}` : `0 8px 32px ${alpha('#10B981', 0.14)}`,
            display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative',
          }}>
            <CarIcon sx={{ fontSize: 48, color: isDark ? '#34D399' : '#059669' }} />
          </Box>
          <Typography variant="h4" sx={{ fontWeight: 800, mb: 0.75, letterSpacing: -0.5 }}>Become a Driver</Typography>
          <Typography variant="body1" color="text.secondary">Start earning with OkraRides</Typography>
        </Box>
      </motion.div>
      ...
      <motion.div variants={stagger} initial="hidden" animate="show">
        <motion.div variants={fadeUp}>
          <Paper elevation={isDark ? 0 : 2} sx={{
            ...
            <Box sx={{ position: 'absolute', top: -20, right: -20, width: 80, height: 80, borderRadius: '50%', background: `radial-gradient(circle, ${alpha('#3B82F6', isDark ? 0.12 : 0.07)} 0%, transparent 70%)`, pointerEvents: 'none' }} />
            <Typography variant="subtitle1" sx={{ fontWeight: 700, mb: 2, letterSpacing: -0.2 }}>What You'll Need</Typography>
            <List disablePadding>
              {requirements.map((req, index) => (
                <Box key={index}>
                  <ListItem sx={{ px: 0, py: 1.25 }}>
                    <ListItemIcon sx={{ minWidth: 46 }}>
                      <Box sx={{ width: 38, height: 38, borderRadius: 2.5, flexShrink: 0, background: isDark ? `linear-gradient(135deg, ${alpha(req.color, 0.22)} 0%, ${alpha(req.color, 0.12)} 100%)` : `linear-gradient(135deg, ${alpha(req.color, 0.14)} 0%, ${alpha(req.color, 0.07)} 100%)`, border: `1px solid ${alpha(req.color, isDark ? 0.25 : 0.15)}`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: req.color, boxShadow: isDark ? 'none' : `0 2px 8px ${alpha(req.color, 0.12)}` }}>{req.icon}</Box>
                    </ListItemIcon>
                    <ListItemText
                  ...
                  {index < requirements.length - 1 && <Divider sx={{ ml: 7, opacity: 0.5 }} />}
                </Box>
              ))}
            </List>
          </Paper>
        ...
        <motion.div variants={fadeUp}>
          <Paper elevation={isDark ? 0 : 2} sx={{
            p: 2.5, mb: 2, borderRadius: 4,
            background: isDark ? `linear-gradient(145deg, ${alpha('#10B981', 0.12)} 0%, ${alpha('#059669', 0.06)} 100%)` : `linear-gradient(145deg, ${alpha('#10B981', 0.08)} 0%, ${alpha('#ECFDF5', 1)} 100%)`,
            border: `1px solid ${alpha('#10B981', isDark ? 0.22 : 0.18)}`,
            boxShadow: isDark ? `0 4px 20px ${alpha('#10B981', 0.1)}` : `0 4px 20px ${alpha('#10B981', 0.08)}`,
            position: 'relative', overflow: 'hidden',
          }}>
            <Box sx={{ position: 'absolute', bottom: -16, right: -16, width: 70, height: 70, borderRadius: '50%', background: `radial-gradient(circle, ${alpha('#10B981', isDark ? 0.15 : 0.1)} 0%, transparent 70%)`, pointerEvents: 'none' }} />
            <Typography variant="subtitle1" sx={{ fontWeight: 700, mb: 1.75, color: isDark ? '#34D399' : '#059669', letterSpacing: -0.2 }}>Why Drive with Us?</Typography>
            <List disablePadding>
              {benefits.map((benefit, index) => (
                <ListItem key={index} sx={{ px: 0, py: 0.6 }}>
                  <ListItemIcon sx={{ minWidth: 28 }}>
                    <Box sx={{ width: 18, height: 18, borderRadius: '50%', background: `linear-gradient(135deg, #10B981 0%, #059669 100%)`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
        ...
        <motion.div variants={fadeUp}>
          <Button fullWidth variant="contained" size="large" onClick={handleStart} disabled={starting}
            sx={{ height: 56, fontSize: '1rem', fontWeight: 700, borderRadius: 4, background: `linear-gradient(135deg, #10B981 0%, #059669 100%)`, boxShadow: `0 6px 20px ${alpha('#10B981', 0.4)}`, textTransform: 'none', '&:hover': { background: `linear-gradient(135deg, #059669 0%, #047857 100%)`, boxShadow: `0 8px 28px ${alpha('#10B981', 0.5)}` } }}>
