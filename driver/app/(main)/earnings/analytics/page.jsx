...
//  *
//  * Charts: Daily earnings trend, hourly heatmap,
//  *         payment method split, ride type split,
//  *         commission vs earnings waterfall.
//  *
//  * Uses recharts (add to your package.json if not present):
...
//   Paper, Grid, Chip, Skeleton, useTheme,
// } from '@mui/material';
...
//     <Typography
//       variant="subtitle2"
//       sx={{ fontWeight: 700, mb: 1.5, color: 'text.secondary', textTransform: 'uppercase', letterSpacing: 0.5 }}
//     >
//       {children}
//     </Typography>
//   );
...
//   return (
//     <Paper elevation={2} sx={{ p: 2, borderRadius: 3, textAlign: 'center', minWidth: 100 }}>
//       <Typography variant="h6" sx={{ fontWeight: 700, color: color ?? 'text.primary', mb: 0.25 }}>
//         {value}
//       </Typography>
...
//           <Typography variant="caption" sx={{ fontWeight: 700 }}>{fmt(entry.value)}</Typography>
//         </Box>
//       ))}
//     </Paper>
//   );
...
//   // Ride type pie (float vs subscription)
//   const rideTypePieData = [
//     { name: 'Float Rides',        value: summary.floatRides        },
//     { name: 'Subscription Rides', value: summary.subscriptionRides },
//   ].filter(d => d.value > 0);

//   // Earnings vs commission bar (for float system or mixed)
//   const earningsVsCommission = dailyBreakdown.map(d => ({
//     date:       d.date.slice(5),        // "MM-DD"
//     Earnings:   parseFloat(d.earnings.toFixed(2)),
//     Commission: parseFloat(d.commission.toFixed(2)),
//   }));

//   // Hourly activity — show rides count
//   const hourlyActivity = hourlyBreakdown.map(h => ({
//     hour:     `${h.hour}:00`,
//     Rides:    h.rides,
//     Earnings: parseFloat(h.earnings.toFixed(2)),
//   }));

//   // Format daily labels for x-axis
...
//             <BackIcon />
//           </IconButton>
//           <Typography variant="h6" sx={{ flex: 1 }}>Analytics</Typography>
//           <Chip
//             label={paymentSystemType === 'float_based' ? 'Float System' : 'Subscription'}
//             size="small"
...
//         >
//           {PERIOD_OPTIONS.map(p => <Tab key={p.value} label={p.label} value={p.value} />)}
//         </Tabs>
//       </AppBar>
...
//           <KpiChip label="Earnings"    value={formatCurrency(summary.totalEarnings)} color={COLORS.earnings}    />
//           <KpiChip label="Rides"       value={summary.completedRides}                                            />
//           <KpiChip label="Avg Ride"    value={formatCurrency(summary.averageRide)}                               />
//           <KpiChip label="Distance"    value={`${summary.totalDistance.toFixed(0)} km`}                          />
//           <KpiChip label="Rating ★"   value={lifetime.averageRating.toFixed(1)}      color={COLORS.earnings}    />
//           {isFloat && (
//             <KpiChip
//               label="Float Balance"
//               value={formatCurrency(balances.floatBalance)}
//               color={balances.floatBalance < 0 ? COLORS.commission : COLORS.cash}
//             />
//           )}
//         </Box>
...
//           <SectionTitle>Daily Earnings Trend</SectionTitle>
//           {loading ? (
//             <Skeleton variant="rectangular" height={200} sx={{ borderRadius: 2 }} />
...
//               <AreaChart data={formattedDaily} margin={{ top: 5, right: 5, left: 0, bottom: 0 }}>
//                 <defs>
//                   <linearGradient id="gradEarnings" x1="0" y1="0" x2="0" y2="1">
//                     <stop offset="5%"  stopColor={COLORS.earnings}   stopOpacity={0.35} />
//                     <stop offset="95%" stopColor={COLORS.earnings}   stopOpacity={0}    />
//                   </linearGradient>
//                 </defs>
//                 <CartesianGrid strokeDasharray="3 3" stroke={theme.palette.divider} />
//                 <XAxis dataKey="date" tick={{ fontSize: 11 }} />
//                 <YAxis tick={{ fontSize: 11 }} tickFormatter={v => formatCurrency(v)} width={65} />
//                 <Tooltip content={<CustomTooltip valueFormatter={formatCurrency} />} />
//                 <Area
//                   type="monotone" dataKey="earnings" name="Earnings"
//                   stroke={COLORS.earnings} fill="url(#gradEarnings)" strokeWidth={2}
//                 />
//               </AreaChart>
//             </ResponsiveContainer>
//           )}
...
//         {/* ── 2. Earnings vs Commission (float) / Earnings only (sub) ─── */}
//         {isFloat && (
//           <Paper elevation={2} sx={{ p: 2, borderRadius: 3, mb: 2 }}>
//             <SectionTitle>Earnings vs Commission</SectionTitle>
//             {loading ? (
//               <Skeleton variant="rectangular" height={200} sx={{ borderRadius: 2 }} />
//             ) : (
//               <ResponsiveContainer width="100%" height={200}>
//                 <BarChart data={earningsVsCommission} margin={{ top: 5, right: 5, left: 0, bottom: 0 }}>
//                   <CartesianGrid strokeDasharray="3 3" stroke={theme.palette.divider} />
//                   <XAxis dataKey="date" tick={{ fontSize: 11 }} />
//                   <YAxis tick={{ fontSize: 11 }} tickFormatter={v => formatCurrency(v)} width={65} />
//                   <Tooltip content={<CustomTooltip valueFormatter={formatCurrency} />} />
//                   <Legend wrapperStyle={{ fontSize: 12 }} />
//                   <Bar dataKey="Earnings"   fill={COLORS.earnings}   radius={[4, 4, 0, 0]} maxBarSize={32} />
//                   <Bar dataKey="Commission" fill={COLORS.commission} radius={[4, 4, 0, 0]} maxBarSize={32} />
//                 </BarChart>
//               </ResponsiveContainer>
//             )}
//             {/* Float balance summary */}
//             <Box sx={{ mt: 1.5, display: 'flex', gap: 1, flexWrap: 'wrap' }}>
//               <Chip size="small" label={`Float Balance: ${formatCurrency(balances.floatBalance)}`}
//                 color={balances.floatBalance < 0 ? 'error' : 'success'} />
...
//                 <Typography variant="caption" color="text.secondary" textAlign="center" display="block" py={4}>
//                   No data
//                 </Typography>
//               ) : (
...
//                       <Cell fill={COLORS.digital} />
//                     </Pie>
//                     <Tooltip formatter={(v, name) => [`${v} rides`, name]} />
...
//           {/* Ride type split (only meaningful when both systems have data) */}
//           <Grid item xs={6}>
//             <Paper elevation={2} sx={{ p: 2, borderRadius: 3, height: '100%' }}>
//               <SectionTitle>Ride Type</SectionTitle>
//               {loading ? (
//                 <Skeleton variant="circular" width={120} height={120} sx={{ mx: 'auto' }} />
...
//                   <PieChart>
//                     <Pie
//                       data={rideTypePieData} cx="50%" cy="50%"
//                       innerRadius={30} outerRadius={55}
//                       labelLine={false} label={renderPieLabel}
//                       dataKey="value"
//                     >
//                       <Cell fill={COLORS.float}        />
//                       <Cell fill={COLORS.subscription} />
//                     </Pie>
//                     <Tooltip formatter={(v, name) => [`${v} rides`, name]} />
//                     <Legend wrapperStyle={{ fontSize: 11 }} />
//                   </PieChart>
...
//               <BarChart data={hourlyActivity} margin={{ top: 5, right: 5, left: 0, bottom: 0 }}>
//                 <CartesianGrid strokeDasharray="3 3" stroke={theme.palette.divider} />
//                 <XAxis
//                   dataKey="hour"
//                   tick={{ fontSize: 9 }}
...
//                 <Tooltip
//                   formatter={(v, name) =>
//                     name === 'Earnings' ? [formatCurrency(v), name] : [`${v} rides`, name]
//                   }
//                 />
//                 <Bar dataKey="Rides" fill={COLORS.digital} radius={[3, 3, 0, 0]} maxBarSize={20} />
//               </BarChart>
//             </ResponsiveContainer>
//           )}
//         </Paper>

//         {/* ── 5. Lifetime Performance Summary ─────────────────────────── */}
//         <Paper elevation={2} sx={{ p: 2, borderRadius: 3 }}>
//           <SectionTitle>Lifetime Overview</SectionTitle>
