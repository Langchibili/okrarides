...
// and fires onSuccess / onError when done.
//
// BUG FIX: The error <Alert> is now rendered OUTSIDE the {!hasNothing &&
// !hasTooLittle} conditional block so it always appears when an error occurs,
// even if the available balance is zero after a failed attempt.
// ─────────────────────────────────────────────────────────────────────────────
...
export default function WithdrawPage() {
  ...
  const handleError = (err) => {
    // Close the modal first, then surface the error message.
    // NOTE: this Alert is rendered OUTSIDE the hasNothing/hasTooLittle guard
    // so it always shows regardless of balance state.
    setModalOpen(false);
    setError(err?.message || 'Withdrawal failed. Please try again.');
  };
  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: 'background.default', pb: 10 }}>
      ...
      <Box sx={{ p: 3 }}>
        ...
        {/* ── Error alert — always visible, not gated behind balance checks ──── */}
        {error && (
          <Alert
            severity="error"
            sx={{ mb: 3 }}
      ...
      {/* ── OkraPayModal ────────────────────────────────────────────────────────
           purpose="withdraw" → modal calls POST /okrapay/request-withdrawal
           and handles the full polling → success/failure flow internally.
           onError fires if the gateway rejects or polling confirms failure.
           Backend also now rolls back the deducted balance on gateway errors.
      ─────────────────────────────────────────────────────────────────────── */}
      <OkraPayModal
        open={modalOpen}
        onClose={() => setModalOpen(false)}
