...
//  *   "971234567"  + "260" → "260971234567"
//  *   "260971234"  + "260" → "260971234" (already prefixed — strip prefix first)
//  */
...
//   // Take last 9 digits (Zambia local numbers are 9 digits)
//   const nineDigit = digits.slice(-9);

//   return `${code}${nineDigit}`;
// }

// // ─── Fetch user with country + currency populated ─────────────────────────────
...
//       metadata   = {},
//       // ── payment channel ────────────────────────────────────────────────────
//       paymentType = 'mobile_money',   // 'mobile_money' | 'card'
//       // ── mobile money ───────────────────────────────────────────────────────
//       phone      = '',
//       operator   = '',
//       // ── card ───────────────────────────────────────────────────────────────
//       customer: cardCustomer,         // { firstName, lastName }
...
//     // ── Resolve user, country and currency ───────────────────────────────────
//     const user = await getUserWithCountry(userId);

//     const userCountry   = user?.country;
//     const countryCode   = (userCountry?.code        || 'zm').toLowerCase();   // 'zm'
//     const phoneCode     = (userCountry?.phoneCode    || '260').replace(/\D/g, ''); // '260'
//     const currencyCode  = (userCountry?.currency?.code || 'ZMW').toUpperCase(); // 'ZMW'

//     // ── Format phone ─────────────────────────────────────────────────────────
//     const formattedPhone = phone
//       ? normalisePhone(phone, phoneCode)
//       : '';

//     // ── Admin settings & gateway ─────────────────────────────────────────────
//     const settings    = await getAdminSettings();
//     const gatewayName = (settings.externalPaymentGateway || 'lencopay') as string;
...
//     // ── Handle IMMEDIATE Lenco failure ────────────────────────────────────────
//     //   e.g. number not registered, insufficient funds, wrong operator, etc.
//     //   Lenco returns { status: true, data: { status: 'failed', reasonForFailure: '...' } }
...
//       // Return structured failure to frontend — NOT a 4xx/5xx so the frontend
//       // can render the appropriate screen (insufficient funds, wrong number, etc.)
//       return ctx.send({
//         success:          false,
...
//     // ── Store gateway reference on success/pending ────────────────────────────
//     await strapi.db.query('api::okrapay.okrapay').update({
//       where: { id: okrapayRecord.id },
//       data: {
//         gatewayReference: result.gatewayReference,
//         // For card 3DS, mark as processing immediately
...
//         lencoStatus:  result.lencoStatus,  // 'pay-offline' | '3ds-auth-required' | 'successful'
//         redirectUrl:  result.redirectUrl,  // 3DS URL
//         gatewayName,
...
//     });
//   } catch (err: any) {
//     strapi.log.error('[OkraPay:initiate] Error:', err);
...
//     // ── Balance check ─────────────────────────────────────────────────────────
//     const userWithBalance = await strapi.db.query('plugin::users-permissions.user').findOne({
//       where:    { id: userId },
//       populate: { driverProfile: { select: ['id', 'floatBalance'] } },
//     });

//     if (!userWithBalance?.driverProfile) {
//       return ctx.badRequest('Driver profile not found');
//     }

//     const currentBalance = parseFloat(userWithBalance.driverProfile.floatBalance || '0');
...
//     await strapi.db.query('driver-profiles.driver-profile').update({
//       where: { id: userWithBalance.driverProfile.id },
...
//     strapi.log.info(
//       `[OkraPay:requestWithdrawal] Withdrawal ${withdrawal.id} initiated for user ${userId}, amount ${numAmount}`,
//     );
...
//     });
//   } catch (err: any) {
//     strapi.log.error('[OkraPay:requestWithdrawal] Error:', err);
...
async function handleFloatTopupSuccess(
  ...
  try {
    ...
    // ── Load user (just the link to driverProfile id) ─────────────────────────
    const user = await strapi.db.query('plugin::users-permissions.user').findOne({
      where:  { id: userId },
    ...
    // ── Fetch the driver-profile component directly to get current balance ────
...
async function handleSubscriptionPaymentSuccess(
  ...
  try {
    ...
    if (action === 'renew') {
      ...
      // ── Fetch driver-profile component, then update ───────────────────────
...
async function handleWithdrawalFailed(
  ...
  try {
    ...
    // ── Fetch the withdrawal to get balanceBefore + withdrawableMode ──────────
    ...
    if (withdrawal?.balanceBefore != null) {
      ...
      if (user?.driverProfile?.id) {
        ...
        // Determine which field to restore. Default to 'currentBalance' for
        // legacy withdrawal records that pre-date withdrawableMode.
        const balanceField =
...
export default factories.createCoreController('api::okrapay.okrapay', ({ strapi }) => ({
  ...
  // GET /api/okrapay/status/:reference   (auth required)
  //
  // In addition to returning our DB record, this endpoint also actively
  // queries Lenco for the latest status when the payment is still pending /
  // processing.  This means the frontend gets an answer on every poll even
  ...
  async getPaymentStatus(ctx: any) {
    try {
      ...
      // ── If still in-flight, ask Lenco directly ──────────────────────────────
      if (['pending', 'processing'].includes(record.paymentStatus)) {
        try {
          ...
          if (typeof gateway.getCollectionStatus === 'function') {
            ...
            if (lencoStatus === 'successful') {
              ...
              if (record.paymentStatus !== 'completed') {
                ...
                if (parsed) {
                  ...
                  // Run domain handler in background — don't block the response
            ...
            // For pay-offline / 3ds-auth-required / pending — fall through to DB response
          }
        } catch (pollErr: any) {
          // Non-fatal: if Lenco is unreachable we still return the DB state
  ...
  async requestWithdrawal(ctx: any) {
    try {
      ...
      // ── Fetch driver profile component directly ───────────────────────────────
      ...
      // ── Resolve which balance field to deduct from ────────────────────────────
      ...
      const withdrawal = await strapi.db.query('api::withdrawal.withdrawal').create({
        data: {
          ...
          accountName,
          withdrawalStatus: 'processing',
          // Store which balance field was deducted so the refund handler knows
          // exactly which field to restore on failure.
          withdrawableMode,
          balanceBefore:    currentBalance,
          balanceAfter:     newBalance,
          requestedAt:      new Date(),
