...
import { yandexMapService }    from './yandexMapService';
import { localMapService }     from './localMapService';
import { geoapifyMapService }  from './geoapifyMapService'; // ← NEW
...
const DEFAULT_SERVICE_APIS: Record<ProviderName, Record<ServiceType, string | null>> = {
  ...
  apple:      { routing: 'deeplink',    distance: 'mapkit',      eta: null,           autoComplete: 'search',       location: 'geocoder'     },
  yandex:     { routing: 'routeApi',    distance: 'routeApi',    eta: 'routeApi',    autoComplete: 'suggest',      location: 'geocoder'     },
  // ↓ Geoapify — all four services supported natively
  geoapify:   { routing: 'routingApi',  distance: 'routingApi',  eta: 'routingApi',  autoComplete: 'autocomplete', location: 'geocodingApi' },
  waze:       { routing: 'deeplink',    distance: null,          eta: null,           autoComplete: null,           location: null           },
  openstreet: { routing: 'osrm',        distance: 'osrm',        eta: 'osrm',        autoComplete: 'nominatim',    location: 'nominatim'    },
  local:      { routing: 'localOsrm',   distance: 'localOsrm',   eta: 'localOsrm',   autoComplete: 'nominatim',    location: 'nominatim'    },
};
...
async function resolveProvider(
  ...
  const methodFor = (name: ProviderName): string | null =>
    config.serviceApis[name]?.[_serviceTypeFromApiType(apiType)] ??
    DEFAULT_SERVICE_APIS[name]?.[_serviceTypeFromApiType(apiType)] ??
    null;

  // 1. Explicit provider
  if (explicitName && !failed.has(explicitName) && await isUsable(explicitName)) {
    return { name: explicitName, apiMethod: methodFor(explicitName) };
  }
  ...
  for (const name of config.list) {
    ...
    if (await isUsable(name)) {
      return { name, apiMethod: methodFor(name) };
    }
  }
  return null;
}
...
// ─── Shared provider dispatch helpers ─────────────────────────────────────────
// Each helper centralises the if/else chain so every public function stays DRY.

async function dispatchReverseGeocode(
  name: ProviderName, latitude: number, longitude: number,
): Promise<LocationDetails | null> {
  if (name === 'google') {
    const r = await googleMapService.reverseGeocode(latitude, longitude);
    if (r) { await logProviderRequest('google', 'GeocoderHTTPAPI'); return r as LocationDetails; }
  ...
  if (name === 'geoapify') {
    const r = await geoapifyMapService.reverseGeocode(latitude, longitude);
    if (r) { await logProviderRequest('geoapify', 'GeocoderHTTPAPI'); return r as LocationDetails; }
  }
  if (name === 'yandex') {
    const r = await yandexMapService.reverseGeocode(latitude, longitude);
    if (r) { await logProviderRequest('yandex', 'GeocoderHTTPAPI'); return r as LocationDetails; }
...
async function dispatchGeocodeAddress(
  name: ProviderName, address: string,
): Promise<{ latitude: number; longitude: number; placeId: string } | null> {
  if (name === 'google') {
    const r = await googleMapService.geocodeAddress(address);
    if (r) { await logProviderRequest('google', 'GeocoderHTTPAPI'); return r; }
  ...
  if (name === 'geoapify') {
    const r = await geoapifyMapService.geocodeAddress(address);
    if (r) { await logProviderRequest('geoapify', 'GeocoderHTTPAPI'); return r; }
  }
  if (name === 'yandex') {
    const r = await yandexMapService.geocodeAddress(address);
    if (r) { await logProviderRequest('yandex', 'GeocoderHTTPAPI'); return r; }
...
async function dispatchComputeRoute(
  name: ProviderName,
  origin: { lat: number; lng: number },
  destination: { lat: number; lng: number },
): Promise<RouteResult | null> {
  ...
  if (name === 'geoapify') {
    const r = await geoapifyMapService.computeRoute(origin, destination);
    if (r) { await logProviderRequest('geoapify', 'RoutingAPI'); return r; }
  }
  if (name === 'yandex') {
    const r = await yandexMapService.computeRoute?.(origin, destination);
    if (r) { await logProviderRequest('yandex', 'RoutingAPI'); return r as RouteResult; }
...
export async function reverseGeocode(
  ...
  const config = await getPriorityConfig();
  return runWithFallback(
    config, null, 'GeocoderHTTPAPI',
    (name) => dispatchReverseGeocode(name, latitude, longitude),
    ()     => localMapService.reverseGeocode(latitude, longitude) as Promise<LocationDetails | null>,
  );
}
...
export async function geocodeAddress(
  ...
  return runWithFallback(
    config, null, 'GeocoderHTTPAPI',
    (name) => dispatchGeocodeAddress(name, address),
    ()     => localMapService.geocodeAddress(address),
  );
}
...
export async function geocodePlaceId(
  ...
  return runWithFallback(
    config, null, 'GeocoderHTTPAPI',
    async (name) => {
      // Geoapify encodes lat/lng inside the placeId it generates — extract directly
      if (name === 'geoapify') {
        const match = placeId.match(/geoapify_\d+_([-\d.]+)_([-\d.]+)/);
        if (match) {
          const lat = parseFloat(match[1]);
          const lng = parseFloat(match[2]);
          const r   = await geoapifyMapService.reverseGeocode(lat, lng);
          if (r) { await logProviderRequest('geoapify', 'GeocoderHTTPAPI'); return r; }
        }
        return null;
      }
...
export async function computeRoute(
  ...
  const config = await getPriorityConfig();
  return runWithFallback(
    config, config.prioritizedRoute, 'RoutingAPI',
    (name) => dispatchComputeRoute(name, origin, destination),
    ()     => localMapService.computeRoute(origin, destination) as Promise<RouteResult | null>,
  );
}
...
export async function calculateDistance(
  ...
  const config = await getPriorityConfig();
  const route  = await runWithFallback(
    config, config.prioritizedDistance, 'RoutingAPI',
    (name) => dispatchComputeRoute(name, origin, destination),
    async () => null,
  );
...
export async function calculateEta(
  ...
  const config = await getPriorityConfig();
  const route  = await runWithFallback(
    config, config.prioritizedEta, 'RoutingAPI',
    (name) => dispatchComputeRoute(name, origin, destination),
    async () => null,
  );
  if (!route) return null;
